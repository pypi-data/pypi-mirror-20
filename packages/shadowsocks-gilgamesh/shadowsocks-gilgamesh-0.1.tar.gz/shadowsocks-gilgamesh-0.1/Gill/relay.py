...
def R(sock):
    try:
        ...
        print("Socket: error:",e,errno_from_exception(e))
        return False
...
class Connection:
    ...
    def authenticate(self, is_local=False):
        ...
        if is_local:
            ...
            if self._auth_stage == STAGE_A_INIT:
                ...
                self.on_continue_write(self._remote_sock)
        ...
        if pass_hash is not None and self._auth_stage == STAGE_A_HASH_I :
    ...
    def to_sock(self, sock, data):
        ...
        if self._remote_sock == sock:
            L("[%d]:" % len(data),"to server",self._hash(data) ,c= "yellow")
        elif self._local_sock == sock:
            L("[%d]:" % len(data), "to con",self._hash(data),c='yellow', a=['bold'])
    ...
    def from_sock(self, sock, is_remote=False):
        ...
        if not self._auth:
            if sock == self._remote_sock and not is_remote:
                self.authenticate(is_local=ilocal)
        ...
        try:
            ...
            # if len(data) > 0:
                # print("\nRECV",len(data), colored(self._hash(data),"red"))    
        ...
        if self._remote_sock == sock:
            L("[%d]R:" % len(data),"from ",self._target,self._hash(data) ,c= "blue")
        elif self._local_sock == sock:
            if not data:
                l = 0
                ...
                l = len(data)
                L("[%d]L:" % l, "from con ",self._hash(data),c='blue', a=['bold'])
            # print( "remote from  local:", sock==self._local_sock, " remote:", sock==self._remote_sock)
        ...
        if not is_remote:
            ...
            if sock == self._local_sock:
                ...
                tp, info, seq, payload_answer = self.deal_data(data, DE) #  checksum -> decrypt
                if payload_answer:
            ...
            if sock == self._local_sock:
                ...
                L("[%d]:" % len(payload), "extract from con",addr, port,c='grey',a=['bold'])
                ...
                L("[%d]:" % len(real_data), "package to con",self._hash(real_data), c='grey',a=['bold'] )
    ...
    def deal_data(self, data, data_direction=EN):
        ...
        #! 3. decrypt(data). 
            if self._data_check_len is None:
                self._data_check_len = Enuma_len(data)
                # print("\nS: ",colored("%s" %self._data_check_len,"yellow") )

            l = len(data)
            #! combine all data. may be the size would bigger expectly. 
            data = self._data_cached_from_read + data
            self.received_len += l
            print('recv_len',self.received_len, self._data_check_len)
            length_complte = False
            if self.received_len >= self._data_check_len:
                length_complte = True
                self.received_len -= self._data_check_len

            if length_complte:
                ## checksum
                # if checksum()
                tp, raddr, rport, payload = Enuma(data,self.p_hash, self.sec_handler.decrypt)
                self._data_cached_from_read = data[self._data_check_len:]
                if self.received_len > 3:
                    self._data_check_len = Enuma_len(self._data_cached_from_read)
                    print("\bCS: ",colored("%s" %self._data_check_len,"yellow") )
                    if not self._data_check_len:
                        print(self._data_cached_from_read)
                elif self.received_len == 0:
                    #! data recived normal . lei check_len to init.
                    self._data_check_len = None
                return tuple([tp, raddr, rport, payload])
            else:
                self._data_cached_from_read += data
                return None
