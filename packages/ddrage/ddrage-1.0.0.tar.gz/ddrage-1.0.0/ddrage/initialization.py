...
def create_perfect_loci(individuals, args):
    ...
    Arguments:
        individuals (list of individuals): Containing tuples containing barcode pairs,
            inserts, individual name etc. for an individual. For details on the 
    ...
    """
    # get p7 barcode and insert which is the same for all individuals
    (_, p7_bc), (_, p7_ins, *_) = individuals[0]
...
def create_perfect_hrl_loci(args, individuals, nr_hrl_loci):
    ...
    # create one p7 sequence for each locus
    p7_reads = [ProtoReadp7(p7_bc, p7_ins, args.dbr, args.p7_overhang, args.p7_rec_site, args.gc_content, args.read_length) for _ in range(nr_hrl_loci)]
