...
from .initialization import create_perfect_hrl_loci
...
class SingletonGenerator:
    ...
    def reads(self):
        ...
        """
        # get p7 barcode and insert that match the sample
        (_, p7_bc), (_, p7_ins, *_) = self.individuals[0]
        ...
        for i in range(self.nr_singletons):
            ...
            protoread_p7 = ProtoReadp7(p7_bc, p7_ins, self.dbr_seq, self.p7_overhang, self.p7_rec_site, self.gc_content, self.read_length)
            ...
            (barcode_p5, _), (insert_p5, _, individual_name, *_) = random.choice(self.individuals)
            meta_info = [
            ...
            prefix_length_p5 = ProtoReadp5.p5_prefix_length(barcode_p5, insert_p5, self.p5_overhang)
            sequence_p5_truncated = sequence_p5[:self.read_length - prefix_length_p5]
            # create a read object and finalize it
            rad_read = RADRead.from_p7_protoread(barcode_p5, insert_p5, individual_name, self.p5_overhang, self.p5_rec_site, sequence_p5_truncated, protoread_p7, self.gc_content, self.read_length, meta_info)
...
class HighlyRepetitiveLocusGenerator:
    ...
    def log_entries(self):
        """Return log entries for the hrl reads."""
        # add an empty list here to mimic the output of rad_locus.log_entry
        # which returns a list of sequences alongside the locus entries.
        return (self.hrl_logs, [])
