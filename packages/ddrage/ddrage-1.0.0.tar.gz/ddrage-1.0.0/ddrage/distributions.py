...
def initialize_coverage_generators(args):
    ...
    if args.coverage_model.lower() in ("poisson", "pd"):
        ...
        hrl_coverage_generator = DiscreteUniform(normal_coverage_generator.mu_2sigma(), args.hrl_max_cov/args.nr_individuals)
...
def interpret_probabilities(probs, prob_type="event_types"):
    ...
    if prob_type == "event_types":
        ...
        events = ("snp", "insert", "deletion", "na")
    # handle strings and iterables
    if isinstance(probs, str):
        event_probs = {event: float(prob) for event, prob in zip(events, probs.split(" "))}
    else:
        event_probs = {event: float(prob) for event, prob in zip(events, probs)}
...
def distributed_mutation_type(mutation_probs):
...
def incomplete_digestion_coverage(coverage):
    """Determine a low number of ID reads for a given coverage.

    Uses a binomial distribution with p=0.05.
    ...
    cov_ID = np.random.binomial(coverage, 0.20)
