...
try:
    from printargs import show as show_args
    from printargs import formatted as format_args
except ImportError:
...
def generate_file_name(p7_bc, output_path_prefix, name="RAGEdataset"):
    ...
    Arguments:
        ...
        name (str): name for the test dataset. Default: 'RAGEdataset'
    ...
    # now = datetime.datetime.now().isoformat().rsplit(".", 1)[0].replace(":", "_")  # this is not, but needs no escaping on a MAC
    ...
    paths.base_folder = os.path.join(prefix, "generated_files", "{}_{}".format(now, name))
...
def write_ground_truth(loci, paths, args):
    ...
    with open(paths.ground_truth, 'a') as gt_file:
        ...
        meta_info = dict()
        for (barcode_p5, barcode_p7), (insert_p5, insert_p7, individual_name, *_) in loci[0].individuals:
            ...
            meta_info[ind_name]["p5 ins"] = insert_p5.decode()
            meta_info[ind_name]["p7 ins"] = insert_p7.decode()
