# -*- coding: utf-8 -*-
"""This module contains the RADLocus class which is rages pivotal data structure.
...
class RADLocus:
    ...
    def __init__(self, individuals, protoread_p7, locus_name, args):
        ...
        Arguments:
            ...
            overhang_p5 (bytes): The overhang of the used p5 enzyme.
        ...
        Note:
            ...
            ::
                ...
                (p5_barcode, p7_barcode), (p5_insert, p7_ins, individual_name, *meta_info) = individual
        """
        ...
        # create raw read sequence
        sequence_p5 = random_seq(args.read_length, p=self.gc_content, excluded_motif=args.p5_rec_site)
        self.reads = []

        # join this p5 read with a copy of the p7 read
        ...
        for (barcode_p5, _), (insert_p5, _, individual_name, *_) in individuals:
            ...
            prefix_length_p5 = ProtoReadp5.p5_prefix_length(barcode_p5, insert_p5, args.p5_overhang)
        ...
        all_insert_lengths = [len(insert_p5) for _, (insert_p5, *_) in individuals]
        max_p5_spacer_length = max(all_insert_lengths)
        p5_length_variability = max_p5_spacer_length - min(all_insert_lengths)

        # keep bases that are truncated due to variable p4 sapcer lengths
        # example:
        # r1: (aux)A(seq)TT
        # r2: (aux)ACG(seq)
        # due to the shorter spacer (A) in r1 two additional bases are present in the
        # genomic seqeunce. If deletions shorten the read, these need to be added,
        # before the common spare seqeunce can be used.
        shortest_read_end = args.read_length - prefix_length_p5 - 1
        longest_read_end = args.read_length - prefix_length_p5 + p5_length_variability - 1
        # extract the bases that can be 'pushed out' of the read by
        # spacer length variability
        p5_spacer_spare_seq = sequence_p5[shortest_read_end:longest_read_end]

        # compute a mutation model
        self.mutation_model = None
        self._compute_mutation_model(
            args.diversity,
            args.mutation_type_prob_profile,
            len(protoread_p7.sequence),
            args.p5_rec_site,
            protoread_p7.rec_site,
            max_p5_spacer_length,
            p5_spacer_spare_seq,
        )
    ...
    def consensus_sequences(self):
        ...
        Note:
            Sequences size varies due to insert length.
            The consensus sequences are the shortest clean sequence at
            this locus. The legth of the p7 sequences does not change,
            as the inserts are always the same assuming the same p7
            barcode for all individuals.
            The length of p5 sequences can vary by ``max([len(ins) for ins in inserts])``
            which is the length of the longest insert.

        Returns:
            tuple(bytes,bytes): The p5 and p7 consensus sequences of the locus.
        """
        if not self.burned:
            # p7 read has no spacer length variability, hence any seqeunce can be used
            _, p7_consensus_seq = self.initial_reads[0].seqs()
            # find the longest p5 seqeunce
            all_p5_seqs = [read.seqs()[0] for read in self.initial_reads]
            p5_consensus_seq = min(all_p5_seqs, key=len)
    ...
    def get_allele_frequency(self):
        ...
        if self.allele_frequency is not None and self.coverage_frequency is not None:
            ...
            for allele, coverage in allele_coverage_counts.items():
                coverage_frequency[allele] = (coverage, total_coverage, coverage/total_coverage)
    ...
    def _compute_mutation_model(self, diversity, mutation_type_prob_profile,
                                p7_length, rec_site_p5, rec_site_p7, max_p5_spacer_len,
                                p5_spacer_spare_seq):
        """Compute the mutation model and save it for later use."""
        # create new p7 sequence, make sure it is not an accidental incomplete digestion
        self.p7_null_allele_seq = random_seq(p7_length, p=self.gc_content, excluded_motif=rec_site_p7)
        ...
        self.spare_sequence_p5 = random_seq(p7_length, p=self.gc_content, excluded_motif=rec_site_p5)
        self.spare_sequence_p7 = random_seq(p7_length, p=self.gc_content, excluded_motif=rec_site_p7)

        # pick a number of mutated alleles using a ztpd and the diversity parameter
        nr_alleles = zero_truncated_poisson_distribution(l=diversity)
    ...
    def _add_common_reads(self, template_read, coverage):
        ...
        modified_reads = rmh.plain_copies(template_read, coverage)
        ...
        self.assigned_alleles[template_read.individual_name] = ((coverage, 0), (self.mutation_model.common, self.mutation_model.common))
    ...
    def _add_mutation_reads(self, template_read, coverage, args):
        ...
        if heterozygous:
            ...
            self.modified_reads_at_locus.extend(modified_reads)
            self.mutations_added = True
            self.assigned_alleles[template_read.individual_name] = ((0, coverage), (allele, allele))
    ...
    def simulate_individual_events(self, args):
        ...
        for template_read, individual_event_type in zip(template_reads, individual_event_types):
            ...
            if random.random() < args.prob_incomplete_digestion:
                ...
                coverage, id_coverage = distributions.incomplete_digestion_coverage(coverage)
            ...
            if (id_coverage is not None) and (individual_event_type != "dropout"):
    ...
    def add_highly_repetitive_reads(self, args):
        ...
        for template_read in template_reads:
            ...
            modified_reads = rmh.plain_copies(template_read, hrl_coverage, meta_info="type:'HRL copy'")
            self.modified_reads_at_locus.extend(modified_reads)
