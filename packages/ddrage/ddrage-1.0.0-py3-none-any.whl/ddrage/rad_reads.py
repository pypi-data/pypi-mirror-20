...
class RADRead:
    ...
    @classmethod
    def from_p7_protoread(cls, bc_p5, ins_p5, individual_name, overhang_p5,
        ...
        Arguments:
            ...
            ins_p5 (str): The p5 insert used to create this read.
        ...
        """
        pp5 = ProtoReadp5(bc_p5, ins_p5, overhang_p5, rec_site_p5, length, gc_content, sequence=sequence_p5)
...
class ProtoReadp5(ProtoRead):
    ...
    def __init__ (self, barcode_p5, insert_p5, overhang_p5, rec_site_p5,
                  length, gc_content, sequence):
        """Create a single end p5 read.

        Arguments:
            barcode_p5 (str): The barcode sequence.
            insert_p5 (str): The insert sequence.
    ...
    def add_seq_errors(self, prob_seq_error):
        ...
        # end of the split.
        err_barcode, err_insert, err_overhang, err_seq = np.split(errors, np.cumsum(self.fragment_lengths()[:-1]))
        
        # create and overwrite 
        self.barcode = bytes((random.choice(not_this[base]) if error else base for base, error in zip(self.barcode, err_barcode)))
        self.insert = bytes((random.choice(not_this[base]) if error else base for base, error in zip(self.insert, err_insert)))
    ...
    def fix_length(self, locus_spare_seq, length_var_spare_seq, max_len_spacer):
        ...
        if length_difference > 0:
            ...
            # 'out of the read'
            spacer_shortening = max_len_spacer - len(self.insert)

            if spacer_shortening > 0:
                # take as many bases from the 'pushed out bases' as needed
                take_from_spacer_spare = max_len_spacer - len(self.insert)
...
class ProtoReadp7(ProtoRead):
    ...
    def __init__ (self, barcode_p7, insert_p7, dbr_seq, overhang_p7,
        ...
        self.insert = insert_p7
        self.dbr_seq = dbr_seq
        self.overhang = overhang_p7
        self.rec_site = rec_site_p7

        self.length = length
        self.prefix_length = len(self.insert) + len(self.dbr_seq) + len(self.overhang)
    ...
    def fragments(self):
        ...
        return self.insert, self.dbr_seq, self.overhang, self.sequence

    def joined_sequence(self):
        """Return the complete read sequence."""
        return b"".join((self.insert, self.dbr_seq, self.overhang, self.sequence))

    def fragment_lengths(self):
        """The individual length of all fragments. This is EXACTLY only the sum of its parts."""
        return tuple(map(len, (self.insert, self.dbr_seq, self.overhang, self.sequence)))

    def total_length(self):
        """The total length of all fragments. This is EXACTLY only the sum of its parts."""
        return sum(map(len, (self.insert, self.dbr_seq, self.overhang, self.sequence)))
    ...
    def add_seq_errors(self, prob_seq_error):
        ...
        # will run to the end. If the last element is added, there would be an empty slice at the
        # end of the split.
        err_insert, err_dbr, err_overhang, err_seq = np.split(errors, np.cumsum(self.fragment_lengths()[:-1]))
