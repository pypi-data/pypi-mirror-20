...
class SyncMysql(Mysql):
    ...
    def query(self, sql, **kwargs):
        ...
        Retry(self, '{obj}'.format(obj=self), self._query, sql, **kwargs).run_sync()
...
class AsynMysql(Mysql):
    ...
    def __str__(self):
        return '<AsynMysql|{name} {host} {port} {user} {db} {charset}>'.format(
            host=self.host,
            port=self.port,
            user=self.user,
            db=self.db,
    ...
    def query(self, sql, **kwargs):
        ...
        effect = yield Retry(self, '{obj}'.format(obj=self), self._query, sql, **kwargs).run_asyn()
        raise Return(effect)
