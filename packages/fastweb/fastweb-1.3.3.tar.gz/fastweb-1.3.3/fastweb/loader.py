...
from fastweb.manager import SyncManager, AsynManager
    ...
    def load_manager(self, pattern):
        ...
        with timing('ms', 10) as t:
            if pattern is SyncPattern:
                self.manager = SyncManager().setup()
            elif pattern is AsynPattern:
                self.manager = AsynManager()
                ioloop.IOLoop.current().run_sync(self.manager.setup)
