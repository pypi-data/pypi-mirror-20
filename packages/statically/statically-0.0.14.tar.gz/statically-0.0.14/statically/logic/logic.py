...
class Statically():
    ...
    children = ["content", "public", os.path.join("public", "styles"), os.path.join("public", "colorschemes"), "templates"]
    ...
    def update(self):
        ...
        def link(content, file):
            ...
            def replacement(match):
                path = Path(unnest(file)) / Path(match.group(2) + ".html")
        ...
        for file in (self.path / Path("content")).rglob('*.md'):
            ...
            target = self.path / Path("public") / file.relative_to(self.path / Path("content"))
