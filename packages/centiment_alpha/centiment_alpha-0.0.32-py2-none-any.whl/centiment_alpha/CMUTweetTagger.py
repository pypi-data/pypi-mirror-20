...
FILEPATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'ark-tweet-nlp-0.3.2.jar')
...
def _call_runtagger(tweets, run_tagger_cmd=RUN_TAGGER_CMD):
    ...
    # interface
    tweets_cleaned = [re.sub('\n|\r', ' ', tw) for tw in tweets]
    message = "\n".join(tweets_cleaned)
    ...
    args.append('conll')
    po = subprocess.Popen(args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
