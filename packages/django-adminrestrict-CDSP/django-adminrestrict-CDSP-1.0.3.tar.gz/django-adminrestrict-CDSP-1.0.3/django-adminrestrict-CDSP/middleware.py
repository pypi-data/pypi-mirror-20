"""
forked adminrestrict middleware
"""
import re
...
"""
On the base of adminrestrict middleware from "Robert Romano (rromano@gmail.com)"
Modified by Alexandre Chevallier (alexandre.chevallier@sciencespo.fr)
"""
...
try:
    allow_ips = settings.CMS_INTERNAL_IPS
except:
    try:
        allow_ips = settings.ALLOWED_ADMIN_IPS
    except:
        allow_ips = []
...
class RestrictAdminDjangoCMSMiddleware(object):
    """
    ...
    def process_request(self, request):
        ...
        if request.path.startswith(reverse('admin:index') or "cms-toolbar-login" in request.build_absolute_uri()) or request.GET.get('edit', False):
            ...
            for filt in [ip for ip in allow_ips if ip.endswith('*')]:
                if re.match(filt.replace("*", ".*"), request_ip):
                    return None

            # Otherwise check if the IP address is in the table. If not, deny access
            if request_ip not in allow_ips:
