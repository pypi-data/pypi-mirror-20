...
class Authz(object):
    ...
    def write(self):
        ...
        for repo_name in self.repositories:
            paths = self.repositories[repo_name]
            for path in paths:
                fh.write("\n[%s:%s]\n" % (repo_name, path))

                if paths[path]['RO']:
                    fh.write(''.join('%s = ro\n' % t for t in paths[path]['RO']))

                if paths[path]['RW']:
                    fh.write(''.join('%s = rw\n' % t for t in paths[path]['RW']))
...
class Repository(object):
    ...
    def __init__(self, group, config):
        ...
        self.path_raw = str(group.path)

        self.m_access = re.match(self.config['patterns']['access_pattern'], self.group_raw, re.IGNORECASE)
        self.m_path = re.match(self.config['patterns']['repo_pattern'], self.path_raw)
    ...
    def path(self):
        ...
        """
        try:
            return self.m_path.group('repo_path')
        except IndexError:
...
def main():
    ...
    repo_group += AttrDef(config['mappings']['svn_path'], key='path')
