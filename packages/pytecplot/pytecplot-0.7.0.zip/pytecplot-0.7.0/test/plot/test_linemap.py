...
class TestLinemap(unittest.TestCase):
    ...
    def test_show_in_legend(self):
        for val in [LegendShow.Yes,LegendShow.No,LegendShow.Auto]:
    ...
    def test_sort(self):
        with self.assertRaises(TecplotLogicError):
            self.lmap.sort_mode = LineMapSort.SpecificVar
        self.lmap.sort_variable = self.dataset.variable(0)
        for val in [LineMapSort.IndependentVar, LineMapSort.DependentVar,
                    LineMapSort.SpecificVar]:
