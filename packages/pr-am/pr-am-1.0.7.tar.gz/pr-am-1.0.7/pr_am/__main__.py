...
import os,pathlib,zipfile,struct,io,math,re
...
class Application:
    ...
    def load_maps(self):
        self.maps = {}
        for p in pathlib.WindowsPath(os.environ["ProgramFiles(x86)"] + "\\Project Reality\\Project Reality BF2\\mods\\pr\\levels\\").iterdir():
            if p.is_dir():
                ...
                with zipfile.ZipFile(server_zip_path) as z:
                    with z.open("heightmapprimary.raw") as zf:
                        ...
                        #Get map size from heightmap size * terrain.con terrain.primaryWorldScale.
                    ...
                    with z.open("terrain.con") as zf:
                        ...
                        #Search for lines starting with terrain.primaryWorldScale
    ...
    def calculate_trajectory(self):
        ...
        if primary_pos_values != None:
            ...
            self.minimap_canvas.tk.call(self.minimap_canvas._w,"moveto", \
                ...
                self.minimap_primary[0] - self.oval_radius, \
                self.minimap_primary[1] - self.oval_radius)
        if secondary_pos_values != None:
            self.minimap_canvas.itemconfig(self.minimap_target_oval,state = "normal")
            self.minimap_canvas.tk.call(self.minimap_canvas._w,"moveto", \
                self.minimap_target_oval, \
                self.minimap_secondary[0] - self.oval_radius, \
                self.minimap_secondary[1] - self.oval_radius)
