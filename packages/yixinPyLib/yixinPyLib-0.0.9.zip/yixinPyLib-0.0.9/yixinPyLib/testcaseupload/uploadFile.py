...
def checkPath(level, path):  
    ...
    for dl in dirList:  
        if(i_dl == 0):  
            ...
            checkPath((int(dirList[0]) + 1), path + '/' + dl)  
    for fl in fileList:  
        # 打印文件  
    ...
    for index in range(len(lineList)):
        ...
        if line.find("#TestCase") > -1:
            ...
            while True:
                ...
                if l[0] == "#":
                    test.strList.append(l[1:])
                else:
                    test.strList.append(l)
                    break
                count+=1
            s = line.strip(' ')
            l = s.split(' ')
...
def upload(case):
    ...
    for index in range(len(case.strList)):
        if case.strList[index].find("模块名称:") > -1:
            upload.moduleName = case.strList[index].replace("模块名称:","")
        if case.strList[index].find("方法:") > -1:
            upload.caseName = case.strList[index].replace("方法:","")
        if case.strList[index].find("描述:") > -1:
            upload.caseInfo = case.strList[index].replace("描述:","")
        if case.strList[index].find("依赖:") > -1:
            upload.preOrder = case.strList[index].replace("依赖:","")
        if case.strList[index].find("顺序号:") > -1:
            upload.caseOrder = case.strList[index].replace("顺序号:","")
