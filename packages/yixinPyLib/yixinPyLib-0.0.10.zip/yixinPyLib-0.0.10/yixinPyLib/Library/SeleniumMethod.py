...
class seleniumMethod():
    ...
    def getScreenshot(self, name, form='png'):
        time.sleep(2)
        self.driver.get_screenshot_as_file(CommonMethod.getProPath()+"/ErrorScreen/"+CommonMethod.getnow()+name+"."+form)
    '''=============================================='''      
    """
        键盘事件
    """
    ...
    def DragAndDrop(self,loc1,loc2):
        '''鼠标拖动
        @loc 元素地址，格式：元组(By.ID,"地址")'''
        ActionChains(self.driver).drag_and_drop(self.driver.findElement(*loc1), self.driver.findElement(*loc2)).perform()
    ...
    *********切换窗口************
    ...
    def isElementExist(self,locator):
        ...
        try:
            if locator == 'id':
                self.driver.findElement(*locator)
                flag = True
        except Exception,ex:
            print Exception,":",ex
        return flag
    ...
    def toElement(self,loc):
        ...
        self.driver.execute_script("arguments[0].scrollIntoView();", self.driver.findElement(*loc))  
    def toTop(self):
