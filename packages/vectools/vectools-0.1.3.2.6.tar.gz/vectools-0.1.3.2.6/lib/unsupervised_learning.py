...
import sys
import numpy as np
from lib.inputoutput import ParseVectors, _shared_params, _slice_list, Vectors, VectorIO
...
def DBSCAN(arg_parser):
    ...
    vectors_obj = Vectors(
        ...
        has_row_names=args.row_titles,
        only_apply_on=args.only_apply_on)
    vectors_obj.parse(sources, args.delimiter)
    ...
    #matrices.only_apply_on_vectors()
    """

    # X = StandardScaler().fit_transform(X)
    # db = DBSCAN(eps=args.epsilon, min_samples=args.min_samples).fit(vectors_obj.get_selected_matrix())
    ...
    vec_parser.out(data_frame)

    #print(data_frame.to_csv())

    #print(sliced_frames)

    # Column title printing is handled by the vector class.
    #vectors_obj.add_column("Clusters", db.labels_, 0)
    #vectors = vectors_obj.get_matrix()
    #print(vectors)
    exit()
    for i in range(len(db.labels_)):
        if (args.omit and db.labels_[i] != -1) or (not args.omit):
            ...
            #    None,  # row_titles[i],
            #    np.append(db.labels_[i], vectors[i]),
            #    column_titles=None
            #)
