...
class VectorIO:
    ...
    """
    def __init__(self, only_apply_on="", delimiter=None, has_col_names=False, has_row_names=False):
    ...
    def out(self, data_frame):
        print(data_frame.to_csv(
            sep=self.delimiter,
            header=self.has_col_names,
            index=self.has_row_names), end="")
    ...
    def _yield_file_names(self, parse_str_or_list):
        ...
        for file_name in parse_str_or_list:
            file_obj = file_name if file_name != 'sys.stdin' else sys.stdin
            yield file_obj

        # Check for STDIN. This is non-blockling so same be missed sometimes.
        while sys.stdin in select([sys.stdin], [], [], 0)[0]:
            if sys.stdin.readable():
                parse_str_or_list.append("sys.stdin")
                yield sys.stdin
            break

        # Check for STDIN last as non-blocking IO can miss it sometimes at the start of the program.
        # If no parse strings passed then check for missed non-blocking IO.
        # Note: Do not use parse_str_or_list is []:
        if parse_str_or_list == []:
            import time
            time.sleep(1)
            ...
            while sys.stdin in select([sys.stdin], [], [], 0)[0]:
                if sys.stdin.readable():
                    ...
                    yield sys.stdin
                break
            if parse_str_or_list is []:
                assert False, "No arguments passed."
    ...
    def parse_vectors(self, parse_str_or_list):
        ...
        return pd.concat(data_frames), pd.concat(sliced_frames)

    def yield_matrices(self, parse_str_or_list):
    ...
    def parse_matrices(self, parse_str_or_list):
        ...
        for df, sliced_frame in self.yield_matrices(parse_str_or_list):
