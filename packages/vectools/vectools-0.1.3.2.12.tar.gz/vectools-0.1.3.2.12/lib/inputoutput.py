...
class VectorIO:
    ...
    def iterative_out(self, row_title, vector, sliced_col_titles=False, column_titles=None, output_type='sys.stdout'):
        ...
        if self.col_titles is not None and self.has_col_names and not self._column_have_been_printed:
            ...
            tmp_col_titles = self.sliced_col_titles if sliced_col_titles else self.col_titles

            tmp_col_titles = list(tmp_col_titles)

            assert (tmp_col_titles is not [] and tmp_col_titles is not None), "Error: No column titles available."

            if self.has_row_names:
                if self.column_row_title is None:
                    # tmp_col_titles.insert(0, "")
                    tmp_col_titles = np.insert(tmp_col_titles, 0, "")
                else:
                    tmp_col_titles = np.array([self.column_row_title] + list(tmp_col_titles))
                    # np.insert(tmp_col_titles, [0], [self.column_row_title])
            output_type.write(self.delimiter.join(tmp_col_titles) + '\n')
    ...
    def _yield_file_names(self, parse_str_or_list):
        ...
        for file_name in parse_str_or_list:
            ...
            if not os.path.isfile(file_name) and file_name != 'STDIN':
    ...
    def parse_vectors(self, parse_str_or_list):
        ...
        return pd.concat(data_frames), pd.concat(data_frames if sliced_frames is [None] else sliced_frames)
