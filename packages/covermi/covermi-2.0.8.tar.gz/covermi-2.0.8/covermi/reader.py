...
class IlluminaManifest(object):
    def __init__(self, path, trim_probes=True):
        self.path = path
        self.trim_probes = trim_probes
        ...
        with FileContext(self.path, "rb") as f:
            for line in csv.reader(f, delimiter="\t"):
                ...
                if heading_row:
                    ...
                    probes[line[col["Target ID"]]] = (len(line[col["ULSO Sequence"]]), len(line[col["DLSO Sequence"]])) if self.trim_probes else (0, 0)
        ...
        with FileContext(self.path, "rb") as f:
            for line in refflatreader(f, self.needed, self.canonicaldict, self.doublecheck):
                ...
                if what == RefFlat.TRANSCRIPTS:
                    yield TranscriptEntry(chrom, int(line[4])+1-self.splice_site_buffer, int(line[5])+self.splice_site_buffer,  name, strand, transcript)
                    ...
                    for start, stop, exon in izip(line[9].rstrip(",").split(","), line[10].rstrip(",").split(","), exon_numbers):
                        yield ExonEntry(chrom, int(start)+1-self.splice_site_buffer, int(stop)+self.splice_site_buffer, name, strand, exon)
...
class Bai(object):
    ...
    def __init__(self, fn, chr_2_ref):
        try:
            ...
            with open(fn, "rb") as bai:
                if bai.read(4) != "BAI\1":
                    raise CoverMiException("{} is not a BAI file!".format(fn))
            ...
            raise CoverMiException("IOError reading {}".format(fn))
    ...
    def chunks(self, locations):
        ...
        for loc in locations:
            ...
            lower_bound = self.intervals[ref_id][(loc.start-1)//16384]
            for bin in allbins(reg2bin(loc.start, loc.stop)):
    ...
    def __init__(self, fn, index=True):
        ...
        try:
            ...
            if self.bam.read(4) != "BAM\1":
                self.bam.close()
                raise CoverMiException("{} is not a BAM file!".format(fn))
            ...
            if self.bam: self.bam.close()
            raise CoverMiException("{} has a truncated header".format(fn))
             
        if index:
            if BGZF:
                try:
                    self.bai = Bai(fn+".bai", chr_2_ref)
                    print "Using index file {}".format(os.path.basename(fn))
                except CoverMiException as e:
                    print repr(e)
            else:
                print "Biopython not installed therefore cannot use bam indexes"
...
#                    supplementary = flag & 0x800
    ...
    def read(self, reference=None, locations=()):
        try:
            for vstart, vstop in self.bai.chunks(locations) if locations and self.bai else ((0, 0),):
                if vstart:
                    self.bam.seek(vstart)
