...
def plot(coverage, panel, identifier, fn, cumulative=False):
    ...
    with PdfPages(fn) as pdf:
        for entry in panel.transcripts.sorted_by_name:
            ...
            exons = panel.exons.touched_by(transcript).subset(entry.name) if ("exons" in panel) else Gr()
            ...
            max_intron_size = amplicons.base_count / amplicons.number_of_components if not amplicons.is_empty else 200
            ...
            xlocations = []
            for exon in exons:
                ax.add_patch(Rectangle((plot.scalex(exon.start), -0.3), exon.stop-exon.start+1, 0.2, edgecolor="black", facecolor="dimgrey"))
                xlabels += [exon.exon]
            ...
            if "variants_mutation" in panel:
                ...
                y = [plot.scaley(panel.depth if ("depth" in panel) else 0)] * len(x)
