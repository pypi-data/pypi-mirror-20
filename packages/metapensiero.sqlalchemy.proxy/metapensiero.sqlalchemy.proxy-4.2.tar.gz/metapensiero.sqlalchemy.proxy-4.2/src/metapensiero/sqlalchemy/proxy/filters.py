...
class Operator(Enum):
    ...
    "The field value must start with the given value, case less."

    CONTAINED = ('~', lambda c, t, v, a: c.ilike('%' + v + '%'))
    "The field value must contain the given value, case less."
            ...
            if not isinstance(operator, Operator):
                for op in Operator:
                    if op.operator == operator:
                        result.append(Filter(fcol, fvalue, op))
                        break
                else:
                    raise ValueError('Unrecognized operator: %r' % operator)
            else:
                result.append(Filter(fcol, fvalue, operator))
