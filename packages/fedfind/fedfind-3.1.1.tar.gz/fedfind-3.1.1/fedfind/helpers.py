...
def pdc_query(path, params=None):
    ...
    cache directory.
    """
    logger.debug("pdc_query: path is {0}".format(path))
    ...
    if path.startswith('compose-images/'):
        logger.debug("pdc_query: caching result for {0}".format(path))
    ...
    if writecache:
        if not os.path.exists(cachedir):
            logger.debug("pdc_query: creating cachedir %s", cachedir)
