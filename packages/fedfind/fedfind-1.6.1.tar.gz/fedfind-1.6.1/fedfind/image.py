...
class Image(object):
    ...
    def __init__(self, url='', path='', arch='', imagetype='', flavor='',
        ...
        # Handle some particularly tricky special cases.
        if re.match(r'Fedora-(x86_64|i386)-\d{2,2}-\d{8,8}'
                    r'(\.\d|)-sda\.(qcow2|raw.xz)', self.filename):
            # F19/20-era cloud image, before we had Cloud flavor.
            self.loadout = 'cloud'
            # This is a bit of a picked-out-of-the-air number.
            self.weight = 90

        if re.match(r'(Fedora-|F)\d{1,2}-(x86_64|i686|Live)'
                    r'-(x86_64|i686|Live)\.iso', self.filename):
            # < F14 Desktop lives. Seriously, who did this naming. We
            # can look for lives with no payload and call 'em
            # 'desktop', but then we start calling newly-added lives
            # 'desktop' and it's a bit tricky to spot when a new one
            # has shown up and we need to add it.
            self.loadout = 'desktop'
            self.weight = 200
        ...
        if not self.imagetype:
            if checkfile == 'boot.iso':
                self.imagetype = 'boot'
            # ISO types
            elif any(checkfile.endswith(ext) for ext in fedfind.const.ISO_EXTS):
                for isotype in fedfind.const.ISO_TYPES:
                    if any(val in checkurl for val in isotype.values):
                        self.imagetype = isotype.name
            # Disk types
            elif any(checkfile.endswith(ext)
                     for ext in fedfind.const.DISK_EXTS):
                self.imagetype = 'disk'
                for sub in fedfind.const.DISK_SUBTYPES:
                    if sub in checkfile:
                        self.imagesubtype = sub
            # Filesystem types
            elif any(checkfile.endswith(ext)
                     for ext in fedfind.const.FILESYSTEM_EXTS):
                for fstype in fedfind.const.FILESYSTEM_TYPES:
                    if fstype in checkfile:
                        self.imagetype = fstype
                if not self.imagetype:
                    self.imagetype = filesystem
            # Vagrant types
            elif any(checkfile.endswith(ext)
                     for ext in fedfind.const.VAGRANT_EXTS):
                self.imagetype = 'vagrant'
                for sub in fedfind.const.VAGRANT_SUBTYPES:
                    if sub in checkfile:
                        self.imagesubtype = sub
                ...
                if any(val in checkurl for val in flav.values):
                    ...
                    if flav.subs:
                        for sub in flav.subs:
                            # This is quite simple: works for now but
                            # may break in future, watch out
                            if sub in checkurl:
                                self.subflavor = sub
        ...
        if self.flavor:
            if self.subflavor:
                self.payload = '{0}_{1}'.format(self.flavor, self.subflavor)
            else:
        ...
        for elem in elems:
            ...
            try:
                ...
                if not self.compose and self.imagetype == 'iso':
                    # It's not really safe to make this guess for non
                    # ISO images, as they often have a date stamp for
                    # non-nightly releases, including milestone and
                    # stable releases which don't have a 'compose'.
                    self.compose = elem
    ...
    @property
    def shortdesc(self):
        """A short description of the image (no arch)."""
        if self.imagesubtype:
            subtypetext = ' {0}'.format(self.imagesubtype)
        else:
            subtypetext = ''
        return '{0} {1}{2}'.format(
            self.payload, self.imagetype, subtypetext)

    @property
    def desc(self):
        """A description of the image."""
        if self.arch:
            return "{0} image for {1}".format(self.shortdesc, self.arch)
        else:
            return "{0} image".format(self.shortdesc)
    ...
    def size(self):
        ...
        # This is actually not safe because self.url could be rsync,
        # but in practice at present it never is. I should fix that
        # though.
        try:
            # Using the mirror system for this seems error-prone.
            return fedfind.helpers.get_size(self.direct_url)
        except AttributeError:
            return fedfind.helpers.get_size(self.url)
