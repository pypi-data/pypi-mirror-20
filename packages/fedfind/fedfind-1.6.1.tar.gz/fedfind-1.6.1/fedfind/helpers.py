...
def get_size(url):
    ...
    while not headers and tries:
        try:
            ...
            tries -= 1
    if not headers:
        raise ValueError("URL {0} not valid or not reachable!".format(url))
...
def get_current_release():
    ...
    while not resp and tries:
        try:
            ...
            tries -= 1
    if not resp:
        raise ValueError("get_current_release: cannot contact server!")
    ...
    collections = json.loads(rawjson)['collections']
