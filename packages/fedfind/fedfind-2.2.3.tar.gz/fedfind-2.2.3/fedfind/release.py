...
def get_release(release='', milestone='', compose='', respin=None,
    ...
    def parse_url(url):
        ...
        if match:
            return (match.group(1), 'Postrelease', match.group(2),
                    match.group(3))
        else:
    ...
    if milestone and compose:
        ...
        # FIXME no syncing for Pungi 4 candidates yet, so we can't try
        # this:
        # rel = Compose(release, milestone, compose, respin)
        # if rel.exists:
        #   return rel
        # instead we find the compose ID and return a Production for
        # that, which is the fallback path anyhow
        if not label:
            comp = '.'.join((compose, str(respin)))
            label = '-'.join((milestone, comp))
...
class Release(object):
    ...
    def check_expected(self):
        ...
        for exptup in self.expected_images:
            for imgdict in self.all_images:
                imgtup = (imgdict['payload'].lower(), imgdict['type'],
                          imgdict['arch'])
                if imgtup == exptup:
    ...
    def get_package_versions(self, packages):
        ...
        the compose doesn't exist, or it can't reach the server. We
        should probably use the packages metadata for this for Pungi
        4 composes, but it's *huge*.
        """
        verdict = dict()
        ...
        for i in initials:
            url = '{0}/x86_64/os/Packages/{1}/'.format(
                self.https_url_generic, i)
            index = fedfind.helpers.urlopen_retries(url)
            text += index.read()
...
class Production(Pungi4Release):
    ...
    be instantiable as a Compose (see below) (FIXME: well, not yet).
    NOTE: Production instances have their *real* milestone, compose
    and respin as self.milestone, self.compose, and self.respin, not
    not 'Production', the date, and the respin relative to date. This
    is pretty much arbitrary. You can always parse self.cid if needed.
    """
    def __init__(self, release, compose, respin):
        ...
        path = "{0}/Fedora-{0}-{1}.{2}/compose".format(
            release, compose, respin)
        url = '/'.join((fedfind.const.NIGHTLY_BASE, path))
        super(Production, self).__init__(url)
        self._release = release
        # We do not set _compose or _milestone as we want those to be
