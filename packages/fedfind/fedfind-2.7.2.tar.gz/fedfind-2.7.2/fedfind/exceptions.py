...
class PreviousRC1Error(Exception):
...
class RsyncError(Exception):
    """Exception raised when a necessary rsync operation fails. 'path'
    is the path we tried to rsync. 'retcode' is the return code.
    """
    def __init__(self, path, retcode):
        self.retcode = retcode
        self.path = path

    def __str__(self):
        return "Path: {0} Return code: {1}".format(self.path, self.retcode)
