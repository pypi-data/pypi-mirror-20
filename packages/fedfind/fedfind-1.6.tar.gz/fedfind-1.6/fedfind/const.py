...
# flavors.
#
# For IsoTuple, each entry in ISO_TYPES is an image type for ISOs.
...
IsoTuple  = collections.namedtuple('IsoTuple', 'name values')
...
# this is a bit icky; really 'raw' and 'qcow' should be image types
# and 'vfat' should be a subimagetype for 'raw', but we've been using
# disk long enough that I don't want to change, so all these are
# considered subimagetypes of 'disk' imagetype
DISK_SUBTYPES = ('raw', 'qcow', 'vfat')
VAGRANT_EXTS = ('.box',)
VAGRANT_SUBTYPES = ('libvirt', 'virtualbox')
FILESYSTEM_EXTS = ('.tar.gz', '.tar.xz')
ISO_EXTS = ('.iso',)
IMAGE_EXTS = DISK_EXTS + FILESYSTEM_EXTS + VAGRANT_EXTS + ISO_EXTS
FILESYSTEM_TYPES = ('docker',)
ISO_TYPES = (
    # 'canned' images are traditional installer images with no kind
    # of payload choice (they install a 'canned' payload). For now
    # the only images of this type are Atomic installer images.
    IsoTuple('canned', ('atomic',)),
    IsoTuple('dvd', ('dvd',)),
    IsoTuple('live', ('live',)),
    IsoTuple('netinst', ('netinst',)),
    IsoTuple('disc',
             ('disc1', 'disc2', 'disc3', 'disc4', 'disc5', 'disc6', 'disc7')),
    IsoTuple('rescuecd', ('rescuecd',)))


# PAYLOADS
FLAVORS = (
    LoadTuple('cloud', ('cloud', 'docker-base'), ('atomic',), 200),
