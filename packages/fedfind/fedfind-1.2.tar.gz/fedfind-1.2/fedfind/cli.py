...
def _release(rel):
    ...
    rawhide.
    """
    if isinstance(rel, int) or rel.lower() == 'rawhide':
        return rel
    else:
        raise ValueError("Release must be integer or Rawhide.")

# _release = lambda x: isinstance(x, int) or x.lower() == 'rawhide'
...
class Cli(object):
    ...
    def images(self):
        ...
        try:
            ...
            logger.debug("Release instance: {0}".format(rel))
            logger.info(
                "Finding images for: Fedora {0}".format(rel.version))
        except ValueError as err:
            sys.exit("Invalid arguments. {0}".format(err[0]))
        if self.args.generic_url:
        ...
        logger.debug("Queries: {0}".format(queries))
        imgs = rel.find_images(queries)
        for img in imgs:
            logger.debug(repr(img))
            logger.debug(img.version)
            logger.debug(img.payload)
            print(img)
    ...
    def parse_args(self):
        ...
        parser_images.add_argument(
            ...
            "Beta)", choices=['Alpha', 'Beta', 'Final', 'Branched', 'Rawhide'],
            required=False)
        parser_images.add_argument(
        ...
        parser_images.add_argument(
            ...
            "upgrade.img, PXE boot kernel/initramfs and so on.",
    ...
    def run(self):
        ...
        function."""
        loglevel = getattr(
            logging, self.args.loglevel.upper(), logging.INFO)
