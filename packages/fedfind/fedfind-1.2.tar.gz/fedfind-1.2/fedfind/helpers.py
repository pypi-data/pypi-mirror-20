...
def url_exists(url):
    ...
    if 'http' in scheme:
        logger.debug("url_exists: checking {0} as HTTP".format(url))
        try:
            urlopen(url)
            return True
        ...
        logger.debug("url_exists: checking {0} as rsync".format(url))
...
def ci_match(val1, val2, exact, neg):
    ...
    """
    logger.debug(
        "ci_match: comparing {0} and {1}, exact {2}, neg {3}".format(
            val1, val2, exact, neg))
