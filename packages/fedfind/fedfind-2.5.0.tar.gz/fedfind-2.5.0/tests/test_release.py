...
class TestRelease:
    ...
    def test_get_release_cid_atomic_old(self):
        """Test that old 2 Week Atomic 'compose IDs' are handled.
        openQA uses the directory name as the 'compose ID', it looks
        like 23-20160427 or 23-20160427.1 for a respin.
        """
        ret = fedfind.release.get_release(cid='23-20160428')
        assert isinstance(ret, fedfind.release.PostRelease)
        assert ret.release == '23'
        assert ret.compose == '20160428'
        assert ret.respin == '0'
        ret = fedfind.release.get_release(cid='23-20160428.1')
        assert isinstance(ret, fedfind.release.PostRelease)
        assert ret.release == '23'
        assert ret.compose == '20160428'
        assert ret.respin == '1'
    ...
    def test_get_release_compose(self):
        ...
        """
        ret = fedfind.release.get_release(24, 'Beta', 1, 1)
        assert isinstance(ret, fedfind.release.Compose)
        assert ret.release == '24'
        assert ret.milestone == 'Beta'
        assert ret.compose == '1'
        assert ret.respin == '1'
        assert ret.label == 'Beta-1.1'
