...
def get_release(release='', milestone='', compose='', respin=None,
    ...
    def guess_compose(release='', milestone='', compose=''):
        ...
        returns Fedora 22).
        """
        snaps = ('rawhide', 'branched', 'postrelease', 'atomic', 'production')
    ...
    def guess_release(release='', milestone='', compose=''):
        ...
        elif milestone.lower() in ('postrelease', 'atomic'):
...
#                '/fedora/linux/releases/{0}'.format(str(release)))
    ...
    def get_nightly(release, milestone, date, respin=None):
        ...
        if milestone.lower() == 'atomic':
            ...
            if respin is not None:
                ...
                branched = guess_nightly_respin(release, milestone, date)
            if branched.exists:
                return branched
            else:
                if respin is not None:
                    ...
                    atomic = guess_nightly_respin(release, milestone, date)
                if atomic.exists:
                    return atomic
                return branched
    ...
    def parse_cid(cid):
        ...
        """
        # Handle old 2 Week Atomic 'compose ID' (openQA BUILD, same
    ...
    def check_expected(self):
        ...
        for exptup in self.expected_images:
            for imgdict in self.all_images:
                imgtup = fedfind.helpers.identify_image(
                    imgdict, out='tuple', lower=True)
                if imgtup == exptup:
