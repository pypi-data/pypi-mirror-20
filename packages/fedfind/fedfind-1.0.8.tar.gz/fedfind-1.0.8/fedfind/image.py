...
class Image(object):
    ...
    def __init__(self, url='', path='', arch='', imagetype='', flavor='',
        ...
        # We need to keep track of this for the later URL hack...
        is_tcrc = False
        try:
        ...
        self.url = url
        if not url:
            # Bit icky, but we want to use dl.fp.o for TC/RC images,
            # not download.fp.o, as the mirrors don't sync fast enough
            if is_tcrc:
                self.url = '{0}/{1}'.format(fedfind.const.HTTPS_DL, path)
            else:
                self.url = '{0}/{1}'.format(fedfind.const.HTTPS, path)
