...
class ClientSession(koji.ClientSession):
    ...
    def find_nightly_tasks(self, date=None, release=None, opts=dict()):
        ...
        nightlies for that release, by checking another request
        ...
        'createdAfter' and 'createdBefore' if you pass date (so just do
        one or the other), and 'owner' (which is hardwired to 2745,
        ...
        # Now find and filter the results. The request parameters - which we
        # get as a list under the key 'request' in the task dict - are not
        # *absolutely* consistent for the different methods, so we have to make
        # some assumptions. We assume that the datestamp and the release are
        # somewhere in the first four parameters.
        ...
        if date:
            ...
            # help filter out any non-nightlies)
            return [t for t in tasks
                     if any(fedfind.helpers.datecheck(p)
                            for p in t['request'][:4])]
