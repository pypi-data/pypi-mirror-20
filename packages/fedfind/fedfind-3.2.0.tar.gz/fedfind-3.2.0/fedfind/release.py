...
def get_release(release='', milestone='', compose='', respin=None,
    ...
    passed. Note that milestone 'RC' / 'Final' will be treated the
...
#                '/fedora/linux/releases/{0}'.format(str(release)))
    ...
    # RespinRelease case, handled early to pre-empt guess_compose and
    # guess_release. We only care for milestone here; passing the
    # other values is actually nonsense so we warn about it
    if str(milestone).lower() == 'respin':
        if release or compose:
            logger.warning("get_release(): Specifying release and/or compose "
                           "for 'Respin' milestone is invalid! Ignored.")
        return RespinRelease()
...
class MirrorRelease(Release):
    ...
    def __init__(self, release, milestone='', compose=''):
        ...
        self._fakecid = "Fedora-{0}-19700101".format(self.release)
    ...
    def all_paths(self):
        ...
        ret = []
        # file types we look for
        exts = ['.{0}'.format(ext) for ext in SUPPORTED_IMAGE_FORMATS]
        ...
        for flist in filelists:
            with fedfind.helpers._get_filelist(flist) as fh:
...
class RespinRelease(MirrorRelease):
    ...
    the date, so you can only instantiate this class with no args and
    you'll get whatever the current available respin is. We use the
    milestone 'Respin' to denote this release type, and the release
    value is the date of the current respin release.
    """
    def __init__(self):
        milestone = 'Respin'
