...
class Release(object):
    ...
    def __init__(self, release='', milestone='', compose=''):
        ...
        # hack up a cid that at least returns the 'release' in the
        # same way as pungi4's. FIXME: we could make this a *bit*
        # more convincing at least
        self.cid = '-'.join(('Fedora', self.release, self.milestone,
                             self.compose))
    ...
    def all_images(self):
        ...
        """
        if not 'images' in self.metadata:
            return []
        images = []
