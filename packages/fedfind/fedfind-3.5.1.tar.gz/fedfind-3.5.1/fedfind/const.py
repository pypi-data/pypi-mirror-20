...
KNOWN_PREFIXES = (HTTPS, 'http://download.fedoraproject.org/pub',
                  ...
                  # PPC compose location, see RHBZ #1435953
...
SUBVARIANTS = (
    ...
    LoadTuple('Cloud', ('cloud', 'docker-base')),
    ...
    LoadTuple('Docker_Base', ('docker-base',)),
...
ARCHES = (
    ...
    ArchTuple('i386', ('i386', 'i486', 'i586', 'i686'), 'intel', True),
...
RESPINRE = re.compile(r'F(?P<release>\d+)-(?P<subv>[^-]+)-(?P<arch>[^-]+)-(?P<date>\d{8,8})\.iso',
                      re.I)
