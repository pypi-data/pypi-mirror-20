...
class Release(object):
    ...
    def _relnum(self):
        ...
        set it up in __init__. Note the public property release_number
        provides the *real* release number; this faux version is kept
        around as it requires no remote trips.
        """
        try:
    ...
    def release_number(self):
        """The release's associated release number (as an int, unlike
        self.release, which is a string). For all releases except
        Rawhide nightlies, this is identical to self.release, it is
        provided so you can use the same property for all classes if
        appropriate. See the RawhideNightly class's implementation for
...
class RawhideNightly(Pungi4Release):
    ...
    @cached_property
    def release_number(self):
        """The true 'release' for Rawhide composes is...'rawhide'.
        Rawhide really is Rawhide, it's not 'Fedora 26' or 'Fedora 27'
        or whatever, it's a single, perpetually-rolling distribution.
        QA, release engineering etc. mostly agree on this. But there
        is, to some extent, a release number 'associated' with any
        given Rawhide compose; it's the number being assigned to
        packages built for Rawhide at that time, etc. There are
        sometimes good reasons to want to know it. So, this property
        provides it. Implementation may change, don't rely on it. As
        noted in the Release class, the same property is available for
        all other Release instances, where it is just self.release as
        an integer.
        """
        collections = fedfind.helpers._get_collections()
        # assumption: there's only ever one release other than devel
        # (which is Rawhide) that is 'Under Development', and it's
        # Branched
        branched = [coll for coll in collections
                    if coll['status'] == 'Under Development'
                    and coll['version'].isdigit()]
        if not branched:
            # this is easy: any time there's no Branched, the number
            # for any available Rawhide compose will be (current + 1)
            return fedfind.helpers.get_current_release(branched=False) + 1
        else:
            branched = branched[0]
            # if there's a Branched, we have to check if the Rawhide
            # compose pre-dates the branch point
            branchdate = branched['date_created'].split()[0].replace('-', '')
            branchdate = fedfind.helpers.date_check(branchdate, out='obj')
            ourdate = fedfind.helpers.date_check(self.compose, out='obj')
            delta = ourdate - branchdate
            if delta.total_seconds() > 0:
                # we're newer than the branch date, so our number is
                # Branched + 1
                return int(branched['version']) + 1
            else:
                # we're older than the branch date, so our number is
                # Branched
                return int(branched['version'])
