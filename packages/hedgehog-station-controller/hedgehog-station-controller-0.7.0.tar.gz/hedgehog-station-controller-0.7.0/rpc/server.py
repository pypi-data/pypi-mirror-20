    ...
    def on_execute(self, channel, method, props, body):
        ...
        try:
            ...
            if ('period' in request):
                schedule.every(self.synchInterval).seconds.do(self.tick_job, request['alias'], request['domain'], request['command']).tag(request['job'])
            else:
                schedule.every(request['period']).seconds.do(self.tick_job, request['alias'], request['domain'], request['command']).tag(request['job'])
