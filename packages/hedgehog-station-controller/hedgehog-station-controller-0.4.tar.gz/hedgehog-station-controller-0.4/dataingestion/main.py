...
def main():
    ...
    if commandLineArguments.verbose:
        root = core.setupLogging(logging.DEBUG)
    else:
        root = core.setupLogging(logging.INFO)
    ...
    server = rpcserver.VisaRpcServer(
        ...
        visa_library=configuration['assetadapter']['backend'],
        exchange=configuration['mq']['exchange']
    )
