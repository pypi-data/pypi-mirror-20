...
class VisaRpcServer(rpc.RpcServer):
    ...
    def on_execute(self, channel, method, props, body):
        ...
        try:
            ...
            schedule.every(self.synchInterval).seconds.do(self.tick_job, request['alias'], request['domain'], request['command']).tag(request['domain'])
    ...
    def on_close_channel(self, channel, method, props, body):
        ...
        try:
            self.data_channels.append(request)
            logging.info('LOCAL.close: ' + json.dumps(
                request) + ' close.')
    ...
    def tick_job(self, alias, domain, command):
        logging.info('TICK ' + alias + ':' + domain + '------------------------------------------------------------------')
        ...
        try:
            ...
            logging.warning('Exception tick query ' + command + ' ...')
            logging.error(e)
