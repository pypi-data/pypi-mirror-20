...
class OutboundApi(object):
    ...
    def get_events(self, **kwargs):
        ...
        all_params = ['page_size', 'page_number', 'filter_type', 'sort_by', 'sort_order']
        all_params.append('callback')
    ...
    def get_events_event_id(self, event_id, **kwargs):
        """
        Get Dialer Event
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_events_event_id(event_id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_events_event_id" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        if ('event_id' not in params) or (params['event_id'] is None):
            raise ValueError("Missing the required parameter `event_id` when calling `get_events_event_id`")
