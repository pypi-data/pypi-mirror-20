...
class OrganizationApi(object):
    ...
    def patch_features_featurename(self, feature_name, enabled, **kwargs):
        """
        Update organization
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.patch_features_featurename(feature_name, enabled, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method patch_features_featurename" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        if ('feature_name' not in params) or (params['feature_name'] is None):
            raise ValueError("Missing the required parameter `feature_name` when calling `patch_features_featurename`")
        # verify the required parameter 'enabled' is set
        if ('enabled' not in params) or (params['enabled'] is None):
            raise ValueError("Missing the required parameter `enabled` when calling `patch_features_featurename`")
    ...
    def put_me(self, **kwargs):
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.put_me(callback=callback_function)
