...
class RoutingApi(object):
    ...
    def get_queues_queue_id_mediatypes_mediatype_estimatedwaittime(self, queue_id, media_type, **kwargs):
        """
        Get Estimated Wait Time
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_queues_queue_id_mediatypes_mediatype_estimatedwaittime(queue_id, media_type, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_queues_queue_id_mediatypes_mediatype_estimatedwaittime" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        # verify the required parameter 'queue_id' is set
        if ('queue_id' not in params) or (params['queue_id'] is None):
            raise ValueError("Missing the required parameter `queue_id` when calling `get_queues_queue_id_mediatypes_mediatype_estimatedwaittime`")
        # verify the required parameter 'media_type' is set
        if ('media_type' not in params) or (params['media_type'] is None):
            raise ValueError("Missing the required parameter `media_type` when calling `get_queues_queue_id_mediatypes_mediatype_estimatedwaittime`")
