...
class PankoClient(object):
    """Panko Client for tempest-lib

    This client doesn't use any authentication system
    """

    def __init__(self):
        self.cli_dir = os.environ.get('PANKO_CLIENT_EXEC_DIR')
        self.endpoint = os.environ.get('PANKO_ENDPOINT')
        self.user_id = uuidutils.generate_uuid()
        self.project_id = uuidutils.generate_uuid()

    def panko(self, action, flags='', params='',
             fail_ok=False, merge_stderr=False):
        creds = ("--os-auth-plugin panko-noauth "
                 "--user-id %s --project-id %s "
                 "--panko-endpoint %s") % (self.user_id,
                                          self.project_id,
                                          self.endpoint)

        flags = creds + ' ' + flags

        return base.execute("panko", action, flags, params, fail_ok,
                            merge_stderr, self.cli_dir)


class ClientTestBase(base.ClientTestBase):
    """Base class for pankoclient tests.

    Establishes the pankoclient and retrieves the essential environment
    information.
    """
    ...
    def retry_panko(self, retry, *args, **kwargs):
        result = ""
        while not result.strip() and retry > 0:
            result = self.panko(*args, **kwargs)
            if not result:
                time.sleep(1)
                retry -= 1
        return result

    def panko(self, *args, **kwargs):
        return self.clients.panko(*args, **kwargs)
