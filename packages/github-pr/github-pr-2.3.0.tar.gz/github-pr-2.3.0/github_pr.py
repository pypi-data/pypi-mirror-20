...
def check_required_fields(required, **args):
    for i in required:
        if args[i] is None:
            logging.error(' %s parameter not set', i.upper())
            sys.exit(1)
...
def _print_prs_table(prs, **args):
    ...
    if 'noheaders' in args and args['noheaders']:
        ...
        logging.debug("HEADERS - %s", table_headers)
...
def _load_pr(**args):
    ...
    logging.debug(" PR: %s", pull_request)
    return pull_request
...
def _load_issue(**args):
    ...
    logging.debug(" ISSUE: %s", issue.number)
    return issue
...
def _load_prs_by_branch(**args):
    ...
    if len(prs) is not 1:
        logging.error("Probable error, found %s pull(s) from %s -> %s (expected 1)", len(prs), args['head'], args['base'])
        _print_prs(prs, **args)
    return prs
...
def _return_specific_owner_prs(all_prs, filters):
    ...
    if _validate_list_of_dict(all_prs):
        ...
        logging.debug("OWNER PRS: %s", [pr['pr'].number for pr in owner_prs])
        return owner_prs
    else:
        return []
...
def _return_specific_labeled_prs(all_prs, filters):
    ...
    if _validate_list_of_dict(all_prs):
        ...
        for pull_request in all_prs:
            ...
            logging.debug("MATCHED LABELS: %s", matched_labels)
            if matched_labels:
    ...
    if _validate_list_of_dict(all_prs):
        ...
        for pull_request in all_prs:
            ...
            if statuses:
                pr_issue.append({'pr':pull_request['pr'], 'issue':pull_request['issue']})
                statuses = []
        logging.debug("SPECIFIC PR STATUSES: %s", [pr['pr'].number for pr in pr_issue])
        return pr_issue
    else:
        return []
...
def _return_specific_comment_prs(all_prs, filters):
    ...
    if _validate_list_of_dict(all_prs):
        ...
        for pull_request in all_prs:
            ...
            logging.debug("COMMENTS: %s", [comment.body for comment in comments])
            matched_comments = [comment.body for comment in comments if re.search(filters['comment'], comment.body)]
            if matched_comments:
                pr_issue.append({'pr':pull_request['pr'], 'issue':pull_request['issue']})
        logging.debug("MATCHED COMMENTS: %s", matched_comments)
        return pr_issue
    else:
        return []
...
def github_check_condition(**args):
    ...
    logging.debug("MERGE COMMENTS %s", [[comment.user.login, comment.body] for comment in issue_comments])

    if not merge_comment_users:
        raise NoMergeCommentError("There are no merge comments associated with this PR")

    logging.debug("PR OWNER: %s", pr.user.login)
    logging.debug("MERGER: %s", merge_comment_users)
    if args.get('condition_non_owner_merger'):
        merge_comment_users = _check_owner_cannot_ship(pr.user.login, merge_comment_users)
    if args.get('condition_approved_mergers_file'):
        merge_comment_users = _check_approved_mergers_file(args['approved_mergers_file_path'], merge_comment_users)
