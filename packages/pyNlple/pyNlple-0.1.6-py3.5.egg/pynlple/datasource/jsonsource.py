...
from pynlple.datasource.datasource import Source, SkippingSource
from pynlple.exceptions import DataSourceException


class ElasticJsonDataSource(SkippingSource):

    def __init__(self, url_address, port, index, query, keys=None, fill_na_map=None, authentication=None, cluster_timeout=10, max_retries=5):
        ...
        self.es = Elasticsearch([{'host': self.url, 'port': self.PORT}], timeout=self.cluster_timeout, max_retries=max_retries, retry_on_timeout=True)
...
class YSProdElasticSource(ElasticJsonDataSource):
    ...
    def __init__(self, index, query, keys=None, fill_na_map=None, authentication=None, cluster_timeout=10, max_retries=5):
        super().__init__(YSProdElasticSource.URL, YSProdElasticSource.PORT, index, query, keys, fill_na_map, authentication, cluster_timeout, max_retries)
