...
def _check_isfile(filename):
    if not isfile(filename):
        print(_("Error: '{filename}' is not a file").format(filename=filename))
        return False
    return True
...
def post(config, status_text, attachment, reply_to_status):
    ...
    if attachment:
        if not _check_isfile(attachment):
            return
        args['media'] = open(attachment, 'rb')
    if reply_to_status:
...
def upload(config, files: list):
    for f in files:
        if not _check_isfile(f):
            return
        link = media.upload(
            server_url=config['server_url'], username=config['username'],
            password=config['password'], media=open(f, 'rb'),
        )[1]
        print('%s: %s' % (f, link))
