...
class Collection(object):
    ...
    def select_model(cls):
        ...
        print('\nlook at self.__class__.__name__ to see if it is Collection')
        print('\nWhat happens when I call this method on a Notes object??')
        if selected:
            if cls.__name__ == 'Collection':
                return eval('rh.{}'.format(selected[0]['name']))

            #
            # Need to do stuff with Notes.select_model()
            #
