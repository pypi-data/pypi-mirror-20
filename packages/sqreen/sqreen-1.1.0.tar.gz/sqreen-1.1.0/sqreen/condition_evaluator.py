...
def hash_key_includes(patterns, iterable, min_value_size, max_iterations=1000):
    """ Tries to find if a nested-dict key is composed by one of the pattern.
    The min_value_size is used to avoid comparison on small strings. For example
    there is no possible Mongo injection below 1 characters.
    """
    iteration = 0
    ...
    while len(remaining_iterables) != 0:
        ...
        iterable_dict = remaining_iterables.pop(0)
        ...
        for key, value in iterable_dict.items():
            ...
            if isinstance(value, dict):
                remaining_iterables.append(value)
