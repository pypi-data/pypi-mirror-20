...
class MakeTarget(Make):
	...
	def get_libpaths(self):
		...
		libpaths = list(set(self.bld.env.LIBPATH + [self.bld.env.LIBDIR]))
...
install: build
	...
	cp $(OUTPUT) $(LIBDIR)
ifneq ($(VNUM),)
	ln -s -f $(LIBDIR)/$(LIBNAME) $(LIBDIR)/$(LIB)
endif
ifneq ($(MAJOR),)
	ln -s -f $(LIBDIR)/$(LIBNAME) $(LIBDIR)/$(LIB).$(MAJOR)
ifneq ($(MINOR),)
	ln -s -f $(LIBDIR)/$(LIBNAME) $(LIBDIR)/$(LIB).$(MAJOR).$(MINOR)
endif
endif
...
ifneq ($(MAJOR),)
	rm -f $(LIBDIR)/$(LIB).$(MAJOR)
ifneq ($(MINOR),)
	rm -f $(LIBDIR)/$(LIB).$(MAJOR).$(MINOR)
endif
endif
	rm -f $(LIBDIR)/$(LIB)
