...
class Database(object):
    def __init__(
        ...
        self._max_hosts_for_query = max_hosts_for_query
        ...
        self._load_hosts(self._database)
    ...
    def query(
        ...
        left_hosts = self._max_hosts_for_query
        while left_hosts > 0:
            try:
                ...
                self._host_manager.cooldown(target_host, self._cooldown_time)
                left_hosts -= 1
                error_log.error(
            ...
            except NoAvailableHostsException as ex:
                error_log.error(ex.message)
                raise ex
        raise NoAvailableHostsException('No available hosts')
    ...
    def _load_hosts(self, new_hosts):
        ...
        if self._host_manager.hosts_set() != 0 and type(new_hosts) in {set, str, list}:
