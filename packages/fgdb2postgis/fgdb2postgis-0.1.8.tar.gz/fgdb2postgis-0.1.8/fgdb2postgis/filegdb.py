...
class FileGDB:
    ...
    def create_index(self, table, field):
        ...
        str_index = "CREATE UNIQUE INDEX IF NOT EXISTS \"%s\" ON \"%s\" (\"%s\"); \n" % (idx_name, table, field)
    ...
    def create_foreign_key_constraint(self, table_details, fkey, table_master, pkey):
        ...
        str_constraint_drop = 'ALTER TABLE "%s" DROP CONSTRAINT IF EXISTS "%s";' % (table_details, fkey_name)
        self.write_it(self.f_create_constraints, str_constraint_drop)
