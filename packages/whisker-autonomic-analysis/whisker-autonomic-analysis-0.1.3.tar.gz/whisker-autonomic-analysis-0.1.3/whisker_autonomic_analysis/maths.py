...
def s_from_ms(values_ms: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    return values_ms / 1000


def ms_from_s(values_s: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    return values_s * 1000


def hr_bpm_from_rr_ms(rr_interval_ms: Union[float, np.ndarray]) \
...
def per_minute_from_freq_hz(freq_hz: Union[float, np.ndarray]) \
        -> Union[float, np.ndarray]:
    return 60 * freq_hz
...
def mean(values: Union[np.ndarray, Iterable[Optional[float]]],
         remove_none: bool = True) -> Optional[float]:
    ...
    if len(values) == 0:
        return None
    result = np.mean(values)  # type: float
    return result
...
def minus(a: Optional[float], b: Optional[float]) -> Optional[float]:
    if a is None or b is None:
        return None
    return a - b


def dictminus(a: Dict[str, Optional[float]],
              b: Dict[str, Optional[float]]) -> Dict[str, Optional[float]]:
    result = {}  # type: Dict[str, Optional[float]]
    for k in a.keys():
        if k in b:
...
def to_db_float(x: Union[None, float, np.float64]) -> Optional[float]:
    """
...
def rotation_matrix(theta_degrees: float) -> np.matrix:
    """
