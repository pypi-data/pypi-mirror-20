...
public:
  ...
  //everything in m_details beyond m_usedLength is ignored. This allows the Input vector allocations to be reused.
  //do this with Polynomial too?
  std::vector<std::pair<std::vector<Input>,double>> m_details;
  size_t m_usedLength=0;
};

void printCoefficient(const Coefficient& d, std::ostream& o){
  for(size_t ic = 0; ic<d.m_usedLength; ++ic){
...
void productCoefficients3 (Coefficient& a, const Coefficient& b, const Coefficient& c){
  ...
  out.reserve(a.m_usedLength*b.m_usedLength*c.m_usedLength);
  for(size_t ix = 0; ix<a.m_usedLength; ++ix){
    const auto& x = a.m_details[ix];
    for(size_t iy = 0; iy<b.m_usedLength; ++iy){
      const auto& y = b.m_details[iy];
      for(size_t iz = 0; iz<c.m_usedLength; ++iz){
...
void sumCoefficients(Coefficient& lhs, Coefficient& rhs){//make the lhs be lhs+rhs, no care about rhs
  ...
  size_t neededSize = lhs.m_usedLength + rhs.m_usedLength;
  if(a.size()<neededSize)
    a.resize(neededSize);
  std::move(b.begin(),b.begin()+rhs.m_usedLength,a.begin()+lhs.m_usedLength);

  //size_t ss = a.size();
  //a.reserve(a.size()+b.size());
  //std::move(b.begin(),b.end(),std::back_inserter(a));
  std::inplace_merge(a.begin(),a.begin()+lhs.m_usedLength,a.begin()+neededSize);

  using A = std::pair<std::vector<Input>,double>;
  auto i = amalgamate_adjacent_pairs(a.begin(), a.begin()+neededSize,
                                     [](const A& a, const A& b){return a.first==b.first;},
                                     [](A& a, const A& b){
...
std::ostream& printPolynomial(Polynomial& poly, std::ostream& o){
  for(auto &l : poly.m_data){
    for(auto& m : l){
      ...
      o<<" ";
      for(size_t ic = 0; ic<m.second.m_usedLength; ++ic){
        const auto& c = m.second.m_details[ic];
        for(const auto& i : c.first){
          o<<"["<<i.m_index<<"]";
        }
        o<<c.second<<"\n";
  ...
  */
  ...
  a.erase(amalgamate_adjacent_pairs(a.begin(), a.end(),
                                    ...
                                    [](Term& a, Term& b){ 
                                      ...
                                      return a.second.m_usedLength!=0;})
          ,a.end());
}
  ...
  if(check){
    ...
    if(s.lexicographicLess(&b,&a)){
      ...
      if(x){
        for(auto& l : x->m_data)
          for(auto&i : l)
            for(size_t j=0; j<i.second.m_usedLength; ++j)
              i.second.m_details[j].second *= -1;
      }
      return x;
    }
  }
  ...
  */
  ...
  for(int i=2; i<bchTable.m_totalLengths[m-1];++i){
    ...
    for(auto& l : p->m_data)
      for (auto&m : l)
        for(size_t ic = 0; ic< m.second.m_usedLength; ++ic)
          m.second.m_details[ic].second *= row.m_coeff; //when m_coeff is zero, we're keeping this in
