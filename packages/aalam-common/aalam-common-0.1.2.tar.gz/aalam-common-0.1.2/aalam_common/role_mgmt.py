...
def install_permissions(provider, app, permissions_map):
    doc = parse_yaml_dict(permissions_map)

    def on_failure(group):
        zutils.request_local_server(
            "DELETE",
            "/aalam/base/permissions/%s/%s/%s" % (provider, app, group))

    for group, desc in doc['permission-groups'].items():
        perm_group = {"name": group, "description": desc}
        resp = zutils.request_local_server(
            "PUT",
            "/aalam/base/permissions/%s/%s" % (provider, app),
            json=perm_group)
        if resp.status_code != 200:
            logging.critical("Unable to create permissions group %s" % group)
            on_failure(group)

        perms = [{"name": k, "description": v} for k, v in doc[group].items()]
        resp = zutils.request_local_server(
            "PUT",
            "/aalam/base/permissions/%s/%s/%s" % (provider, app, group),
            json={"permissions": perms})
        if resp.status_code != 200:
            logging.critical("Unable to create permissions for group %s" %
                             group)
            on_failure(group)
...
def is_client_authorized(request, grp, name):
    return request.user_perms and get_id(grp, name) in request.user_perms
