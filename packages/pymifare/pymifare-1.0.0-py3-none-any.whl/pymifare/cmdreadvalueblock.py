...
class CmdReadValueBlock(CommandBaseSpv1):

    def __init__(self):
        super().__init__(spv1handler, spv1handler.dll.spv1_create_cmdreadvalueblock)

        self.spv1_command_build_api = spv1handler.dll.spv1_build_cmdreadvalueblock
        self.spv1_command_build_api.restype = STRUCT_SPV1FRAME
        self.spv1_command_build_api.argtypes = (ctypes.c_ulong, CmdReadValueBlock.PARAMS_READ_VALUE_BLOCK)

        self.spv1_get_response_api = spv1handler.dll.spv1_get_response_cmdreadvalueblock
    ...
    class PARAMS_READ_VALUE_BLOCK(PARAMS_MIFARE_BLOCK_NO_BASE):
        def __init__(self):
            super().__init__()


    def build(self, commandParams:PARAMS_READ_VALUE_BLOCK, nodeAddress=0):
