    ...
    def save(self, *args, **kwargs):
        ...
        try:
            ...
            try:
                ...
                logger.debug("Got captcha! Q: %s ID: %s", question, captchaid)
                # This 'eval' could be quite dangerous if EVIL PPL
                # took over a wiki server. So let's try and make sure
                # all we're going to do is solve a math question.
                if re.match(r'\d\d?(\+|-)\d\d?', question):
                    answer = eval(question)
                    logger.debug(
                        "Answering captcha! Q: %s A: %s", question, answer)
                    ret = super(Page, self).save(*args, captchaword=answer,
                                                 captchaid=captchaid, **kwargs)
                else:
                    # Er. Dunno what's going on here. Retry?
                    logger.warning(
                        "Captcha solve failed! Question: %s", question)
                    ret = super(Page, self).save(*args, **kwargs)
            except (IndexError, KeyError):
                # We got some other mwe.EditError. Hrm. Retry?
                ret = super(Page, self).save(*args, **kwargs)
        # clear the caches
        self._sections = None
        return ret
...
class ValidationPage(Page):
    ...
    def find_resultrow(self, testcase='', section='', testname=''):
...
class DownloadPage(Page):
    ...
    """
    def __init__(self, site, event, cid='', info=None):
        ...
        self.event = event
        # FIXME: this is part of a hack to allow explicit setting of
        # the relevant compose ID (hence location) at page creation
        # time, since we may want to create a ComposeEvent before the
        # compose is in PDC and hence before we can discover its
        # location from the milestone/compose. This hack can go away
        # if we only create ComposeEvents for mirrored releases after
        # they get mirrored, or if we create them for non-mirrored
        # releases but make sure we do it after they're in PDC (we
        # can't do this yet as there is no fedmsg from PDC).
        if cid:
            self.ffrel = fedfind.release.get_release(cid=cid)
        else:
            self.ffrel = event.ff_release
    ...
    def seedtext(self):
        ...
        arches = set()
        imagedict = dict()
        for img in self.ffrel.all_images:
            if img['arch']:
                arches.add(img['arch'])
            ...
            for arch in arches:
                ...
                for img in imgs:
                    if img['arch'] == arch:
                        ...
                        url = '/'.join((self.ffrel.location, img['path']))
