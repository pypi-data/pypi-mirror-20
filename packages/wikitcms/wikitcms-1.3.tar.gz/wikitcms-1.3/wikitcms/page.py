...
class ComposePage(Page):
    ...
    def get_resultrows(self, statuses=None, transferred=True):
        ...
        for i, sec in enumerate(sections):
            try:
                ...
                nextsec = None
            section = sec['anchor'].replace('_', ' ')
            secid = sec['index']
            if nextsec:
                sectext = pagetext[sec['byteoffset']:nextsec['byteoffset']]
            else:
                sectext = pagetext[sec['byteoffset']:]
            secrows = result.find_resultrows(sectext, statuses, transferred)
            rows.extend([(section, secid, r) for r in secrows])
        return rows
    ...
    def add_result(self, restext, row, env, secid=None):
        """Adds a result to the page. Must be passed the text to be
        inserted - which can be provided by result.result_text() - the
        result.ResultRow() object representing the row into which a
        result will be added, and the name of the environment for which
        the result is to be reported. May be passed the index of the
        section in which the row appears. Works by replacing the first
        instance of the row's text encountered in the page or page
        section. The wrong row may be replaced if used without
        specifying a secid on a page where the same testcase and test
        name are used in more than one section. Expected to be used
        together with get_resultrows, which provides the ResultRow()
        objects and section IDs.
        """
        noneres = result.result_text('none')
        oldsec = self.wikipage.edit(section=secid)
        oldrow = row.text
        cells = oldrow.split('\n|')
        rescell = cells[row.columns.index(env)]
        if noneres in rescell:
            rescell = rescell.replace(noneres, restext)
        elif '\n' in rescell:
        ...
        self.site.write_page(self.wikipage, newsec, summary, section=secid,
                             force=True)
