...
class Wiki(mwclient.Site):
    ...
    def login(self, username=None, password=None, *args, **kwargs):
        ...
        if not self.credentials and (not username or not password):
            ...
            try:
                credfile = open(
                    os.path.expanduser('~/.fedora/credentials'), 'r')
    ...
    def allresults(self, prefix=None, start=None, redirects='all'):
        ...
        gen = li.TcmsPageList(self, prefix=prefix, start=start,
                              namespace=116, redirects=redirects)
        return gen

    def alltestdays(self, prefix=None, start=None, redirects='all'):
        ...
        """
        gen = li.TcmsPageList(self, prefix=prefix, start=start,
                              namespace=114, redirects=redirects)
        return gen
    ...
    def _check_compose(self, compose):
        ...
        if fedfind.helpers.date_check(compose, fail_raise=False):
            ...
            if patt.match(compose.upper()):
                ...
                raise ValueError("Compose must be a TC/RC identifier (TC1, "
                                 "RC3...) or a date in YYYYMMDD format.")
    ...
    def _check_release_milestone(self, release, milestone):
        ...
        """
        if not milestone:
            # None, False give undesired string sanitization results
            milestone = ''
        ...
        if not release:
            # Guess release, sanitize milestone, and return.
    ...
    def get_validation_event_url(self, url):
        """Get a validation event based on a compose URL. For things
        that want to listen to fedmsg for compose messages and map
        from the compose to a validation event.
        """
        # for now we just handle nightlies, since that's all we have
        # to work with. use fedfind helpers for parsing.
        cid = fedfind.helpers.find_cid(url)
        if not cid:
            raise ValueError("get_validation_event_url(): cannot parse URL "
                             "(no compose ID found!)")
        (release, date, typ, _) = fedfind.helpers.parse_cid(cid)
        # FIXME: we have no idea yet what to do for non-nightly types
        if not typ == "nightly":
            raise ValueError("get_validation_event_url(): cannot guess event "
                             "for non-nightly compose yet!")

        # we're gonna need to do some checking against the current
        # event release whatever, so let's find it now
        evrel = self.current_compose['release']

        if release == "rawhide":
            # the release number for Rawhide events should always be
            # 1 above the highest current 'real' release (branched or
            # stable)
            rel = fedfind.helpers.get_current_release(branched=True) + 1
            return ev.NightlyEvent(self, rel, "Rawhide", date, url=url)

        else:
            # for now all non-Rawhide nightly composes will be
            # Branched, but that may change, so let's try to guard a
            # little bit
            if int(release) < int(evrel):
                raise ValueError("get_validation_event_url(): this looks like "
                                 "a post-release nightly! We don't do those.")
            return ev.NightlyEvent(self, release, "Branched", date, url=url)
    ...
    def report_validation_results(self, reslist, allowdupe=False):
        ...
        iterable containing up to 13 values in the following order:
        (testtype, release, milestone, compose, testcase, section,
        testname, env, status, user, bugs, comment, bot). They can
        also be any mapping type (e.g. a dict) with enough of those 12
        keys set (see below for requirements).
        ...
        * 'testtype', 'release', 'milestone' and 'compose' must
        ...
        *'status' must indicate the result status.
        ...
        for resitem in reslist:
            ...
            # ResultRow from the testcase, section, and testname, and
            # produce a Result from status, username, bugs, and
            # comment. The keys in resdict are ResultRow instances, so
            # we check if there's already one that's 'equal' to the
            # ResultRow for this restup. If so, we append the (env,
            # Result) tuple to the list that is the value for that
            # key. If not, we add a new entry to the resdict. It's
            # vital that the resdict contain only *one* entry for any
            # given ResultRow, or else the edit will go squiffy.
