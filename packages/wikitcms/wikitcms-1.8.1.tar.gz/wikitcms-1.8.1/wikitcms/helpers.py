...
def strip_tags(string):
    ...
    if not string:
        return string
    tag_patt = re.compile('<.*?>')
    char_patt = re.compile('[&"<>]')
    # First strip complete tags - this is neater than just replacing the
    # < and > characters because you don't get the stripped tags hanging
    # around in section names and stuff
    string = tag_patt.sub('', string)
    # but also strip < and > chars or else you can probably split them
    # across strings and attack that way. also strip known-dangerous & and
    # ".
    return char_patt.sub('', string)
...
def rreplace(string, old, new, occurrence):
    ...
    Taken from https://stackoverflow.com/questions/2556108/how-to-replace-the-last-occurence-of-an-expression-in-a-string
    """
