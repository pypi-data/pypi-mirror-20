...
class Result(object):
    ...
    """
    def __init__(self, status='', user='', bugs=[], comment=''):
        self.status = hl.strip_tags(status)
        self.user = hl.strip_tags(user)
        self.bugs = [hl.strip_tags(b) for b in bugs]
        self.comment = comment
        self.safe_comment = hl.strip_tags(comment)
    ...
    def __str__(self):
        ...
        if self.bugs:
            ...
            bugs = ''
        if self.safe_comment:
            comment = ', comment: ' + self.safe_comment
        else:
            comment = ''
...
class TestInstance(object):
    ...
    def __init__(self, testcase, milestone='', results=dict()):
        self.testcase = hl.strip_tags(testcase)
        self.name = hl.strip_tags(testcase)
        self.milestone = hl.strip_tags(milestone)
...
class ResultRow(TestInstance):
    ...
    def __init__(self, testcase, columns, section='', secid=None, milestone='',
        ...
        self.columns = [hl.strip_tags(c) for c in columns]
        self.origtext = origtext
        self.section = hl.strip_tags(section)
        self.secid = secid
