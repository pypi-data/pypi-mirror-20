...
class Wiki(mwclient.Site):
    ...
    def current_compose(self):
        """A dict of the key / value pairs from the CurrentFedoraCompose
        page which is the canonical definition of the 'current' primary
        arch validation testing compose. You can usually expect keys
        full, release, date, milestone, compose and rawhide. The page
        is normally written by ValidationEvent.update_current().
        """
        currdict = dict()
        valpatt = re.compile('^\| *?(\w+?) *?= *(.*?) *$', re.M)
    ...
    def walk_category(self, category):
        """Simple recursive category walk. Returns a list of page objects that
