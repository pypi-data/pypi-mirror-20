...
class Page(mwp.Page):
    ...
    def sections(self):
        ...
        section.
        """
        return self.site.api(
            'parse', page=self.name, prop='sections')['parse']['sections']
...
class ComposePage(ValidationPage):
    ...
    def __init__(self, site, release, testtype, milestone, compose, info=None):
        ...
        super(ComposePage, self).__init__(
            site, release, testtype, milestone=milestone, compose=compose,
            info=info)
...
class NightlyPage(ValidationPage):
    ...
    """
    def __init__(self, site, release, testtype, milestone, date, info=None):
        self.version = "{0} {1} {2}".format(str(release), milestone, date)
        self.shortver = date
        super(NightlyPage, self).__init__(
            site, release, testtype, milestone=milestone, date=date, info=info)

        # String that will generate a clean copy of the page using the
        # test page generation template system.
        self.seedtext = ("{{{{subst:Validation results|testtype={0}"
                         "|release={1}|milestone={2}|date={3}}}}}").format(
            testtype, release, milestone, date)
