...
class ValidationEvent(object):
    ...
    def creation_date(self):
        ...
        for the event, or "" if it doesn't exist. This will hit the
        wiki for ComposeEvents, not for NightlyEvents.
        """
        try:
    ...
    def create(self, testtypes=None, force=False, current=True, check=False,
        ...
        if cid:
            downpage = page.DownloadPage(self.site, self, cid=cid)
        else:
            downpage = self.download_page
        pages.extend((self.summary_page, downpage, self.category_page,
