...
class ResTuple(
    ...
    'section testname env status user bugs comment')):
    ...
    def __new__(cls, testtype, release='', milestone='', compose='',
        ...
        return super(ResTuple, cls).__new__(
            ...
            testname, env, status, user, bugs, comment)
...
class Wiki(mwclient.Site):
    ...
    def report_validation_results(self, reslist, allowdupe=False):
        ...
        iterable containing up to 12 values in the following order:
        (testtype, release, milestone, compose, testcase, section,
        testname, env, status, user, bugs, comment). They can also
        be any mapping type (e.g. a dict) with enough of those 12
