...
def find_resultrows(text, statuses=None, transferred=True):
    ...
    for row in rows:
        ...
        if tcmatch:
            ...
            # results.
            resrow = ResultRow(columns, tcmatch.group(1), row, statuses,
                               transferred)
            if resrow.results:
                resultrows.append(resrow)
    return resultrows

def result_text(status, user=None, bugs=None, comment=''):
    """Produces the wiki text for a test result, with the provided
    status and (optionally) user, bug list, and 'comment' (which will
    be appended to the template proper, inside <ref> </ref>).
    """
    bugtext = ''
    commtext = ''
    usertext = ''
    if bugs:
        bugtext = "|" + '|'.join(bugs)
    if comment:
        commtext = "<ref>" + comment + "</ref>"
    if user:
        usertext = "|" + user
    text = "{{{{result|{status}{usertext}{bugtext}}}}}{commtext}".format(
...
class ResultRow(object):
    ...
    """
    def __init__(self, columns, testcase, row, statuses=None,
                 transferred=True):
    ...
    def _parse_row(self, row, statuses, transferred):
        ...
        for i, cell in enumerate(cells):
            ...
            if res or oldres:
                ...
                # column it is in
                self.envs.append(self.columns[i])
                try:
