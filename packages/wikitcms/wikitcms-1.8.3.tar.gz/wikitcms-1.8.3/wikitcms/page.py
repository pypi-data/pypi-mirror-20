...
class Page(mwp.Page):
    ...
    def sections(self):
        ...
        section.
        """
        return self.site.api('parse', page=self.name,
    ...
    def save(self, *args, **kwargs):
        ...
        remote roundtrip."""
        if kwargs['oldtext'] and args[0] == kwargs['oldtext']:
            return(dict(nochange=''))
        try:
            ret = super(Page, self).save(*args, **kwargs)
        except mwe.EditError as e:
...
class ValidationPage(Page):
    ...
    """
    def __init__(self, site, release, testtype, info=None):
...
class ComposePage(ValidationPage):
    ...
    def __init__(self, site, release, testtype, milestone, compose, info=None):
        ...
        super(ComposePage, self).__init__(site, release, testtype, info)

        # String that will generate a clean copy of the page using the test
        # page generation template system.
        self.seedtext = ("{{{{subst:Validation results|testtype={0}|release={1}"
...
class NightlyPage(ValidationPage):
    ...
    """
    def __init__(self, site, release, testtype, tree, date, info=None):
        self.date = date
        self.version = "{0} {1} {2}".format(str(release), tree, date)
        self.shortver = date
        self.tree = tree
        super(NightlyPage, self).__init__(site, release, testtype, info)

        # String that will generate a clean copy of the page using the test
        # page generation template system.
        self.seedtext = ("{{{{subst:Validation results|testtype={0}|release={1}"
                        "|milestone={2}|date={3}}}}}").format(
                testtype, release, tree, date)

    @property
    def sortname(self):
        # The parent class version sorts 'Nightly' ahead of 'Rawhide'.
        # Our 'date' is always going to sort correctly anyhow, so just
        # use it.
        return self.shortver
...
class SummaryPage(Page):
    ...
    def __init__(self, site, event, info=None):
        ...
        self.seedtext = ("Fedora {0} [[QA:Release validation test plan|release " 
...
class TestDayPage(Page):
    ...
    cases as the column headers and users as the rows - each row is one
    user's results for all of the test cases in the table.
    """
    ...
    def fix_app_results(self):
        """The test day app does its own bug references outside the result
        template, instead of including them as the final parameters to the
        template like it should. This fixes that, in a fairly rough and
        ready way.
        """
        badres = re.compile('({{result.*?)}} {0,2}(<ref>{{bz\|\d{6,7}}}</ref>) ?'
                            '(<ref>{{bz\|\d{6,7}}}</ref>)? ?'
                            '(<ref>{{bz\|\d{6,7}}}</ref>)? ?'
                            '(<ref>{{bz\|\d{6,7}}}</ref>)? ?'
                            '(<ref>{{bz\|\d{6,7}}}</ref>)? ?'
    ...
    def long_refs(self):
        """People tend to include giant essays as <ref> notes on test day
        results, which really makes table rendering ugly when they're
        dumped in the last column of the table. This finds all <ref>
        notes over 150 characters long, moves them to the "long"
        group, and adds a section at the end of the page with all the
        "long" notes in it. The 'end of page discovery' is a bit hacky,
        it just finds the last empty line in the page except for trailing
        lines and sticks the section there, but that's usually what we
        want - basically we want to make sure it appears just above the
        category memberships at the bottom of the page. It does go wrong
        *sometimes*, so good idea to check the page after it's edited.
        """
        text = oldtext = self.edit()
        ...
        if found:
            text = helpers.rreplace(text.strip(), '\n\n',
