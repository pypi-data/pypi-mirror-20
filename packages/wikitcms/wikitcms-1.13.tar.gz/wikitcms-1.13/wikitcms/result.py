...
# separator (newline starting with a |).
# current line ($). The re.M is vital or this breaks.
...
def _filter_results(results, statuses=None, transferred=True, bot=True):
    ...
    results = [r for r in results if not r.user or
               r.user.lower() not in ('sampleuser', 'exampleuser', 'example')]
    if statuses:
...
def find_results(text, statuses=None, transferred=True, bot=True):
    ...
    included. If bot is False, results from automated test bots (results
    with 'bot=true') will not be included.
    """
    results = list()
...
class Result(object):
    ...
    def result_template(self):
        ...
        tmpl = ("{{{{result|{status}{usertext}{bugtext}{bottext}}}}}"
                "{commtext}").format(
            status=status, usertext=usertext, bugtext=bugtext, bottext=bottext,
            commtext=commtext)
        return tmpl
