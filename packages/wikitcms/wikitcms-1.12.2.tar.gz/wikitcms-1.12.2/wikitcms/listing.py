...
class TcmsGeneratorList(mwl.GeneratorList):
    ...
    def _get_tcms(self, name, info=()):
        ...
        if (name.startswith(u'Test Results:') or
                (name.startswith(u'Test Day:') and not
                 name.endswith(u'/ru') and not
                 'metadata' in name.lower() and not
                 'rendercheck' in name.lower()) or
                (name.startswith(u'Category:'))):
            nightly_patt = re.compile(u'Test Results:Fedora (\d{1,3}) '
                                      '(Rawhide|Nightly|Branched) '
                                      '(\d{8,8}|\d{4,4} \d{2,2}) (.+)$')
            ...
            if match:
                if not match.group(2):
                    page = ValidationCategory(self.site, match.group(1), info=info)
                    return self._check_page(name, page)
...
class TcmsPageList(mwl.PageList, TcmsGeneratorList):
    ...
    def __init__(self, site, prefix=None, start=None, end=None, namespace=0,
                 redirects='all'):
