...
class ValidationPage(Page):
    ...
    pages, containing common properties and methods.
    """
    def __init__(self, site, release, testtype):
        self.release = release
        self.testtype = testtype
        super(ValidationPage, self).__init__(site)

        # Wiki name the page should have, according to the naming convention.
        self.wikiname = "Test Results:Fedora {0} {1}".format(self.version,
                                                           self.testtype)
        if int(release) == 12:
            # handle slightly wacky old page names
            if self.milestone is 'Alpha':
                self.wikiname = self.wikiname.replace('Test Results', 'QA')
                self.wikiname = self.wikiname + " Test Results"
            if self.milestone is 'Final':
    ...
    def results_wikitext(self):
        ...
        """
        pagetext = self.wikipage.edit().encode('utf-8')
    ...
    def from_wikipage(cls, wikipage):
        """Given a wikipage, returns an instance of the correct child
        class for the page, or None on failure. So, if the page is for
        a nightly compose it returns a NightlyPage instance, if the
        page is for a TC/RC it returns a ComposePage instance.
        """
        if not wikipage.exists:
            return None
        if ('Alpha' in wikipage.name or 'Beta' in wikipage.name or 'Final'
            in wikipage.name):
            return ComposePage.compose_from_wikipage(wikipage)
        else:
            return NightlyPage.nightly_from_wikipage(wikipage)
    ...
    def add_result(self, result, row, env):
        ...
        oldsec = self.wikipage.edit(section=row.secid).encode('utf-8')
        oldrow = row.origtext
        cells = oldrow.split('\n|')
...
class ComposePage(ValidationPage):
    ...
    def __init__(self, site, release, testtype, milestone, compose):
        ...
        super(ComposePage, self).__init__(site, release, testtype)

        # String that will generate a clean copy of the page using the test
        # page generation template system.
        self.text = ("{{{{subst:Validation results|testtype={0}|release={1}"
                        "|milestone={2}|compose={3}").format(
                testtype, release, milestone, compose)

    @classmethod
    def compose_from_wikipage(cls, wikipage):
        """Get a ComposePage object from a mediawiki Page object. This
        may seem rather roundabout, but it's useful for e.g. for
        getting existing validation pages when you don't know what
        composes and test types exist - you can get the wikipage
        objects from a Release instance and then run them through this
        to get wikitcms objects.

        Works by matching a regexp based on the naming convention for
        validation pages. If it thinks the wiki page name matches,
        it'll try and instantiate the ComposePage object. If that
        succeeds, as a further sanity check, it checks that the name
        properties match - we don't want you to get a ComposePage
        that actually writes to a different wiki page than the one you
        based it on. If any of this fails, it returns None.

        Assumes that the milestone will be Alpha, Beta or Final. In
        practice, this gets the pages you want. Will match releases
        from 1 to 999 and pretty much anything for 'compose' and
        'testtype'.
        """
        if not wikipage.exists:
            return None
        # This does not work for most Fedora 12 pages. Fixing it fully
        # would be very ugly, and not worth the work.
        pattern = re.compile('Test[_ ]Results:Fedora[_ ](\d{1,3})'
                                     '[_ ](Alpha|Beta|Final)[_ ]([\w .]+?)'
                                     '[_ ]([\w .]+)')
        match = pattern.match(wikipage.name)
        if match:
            try:
                page = cls(wikipage.site, match.group(1), match.group(4),
...
class NightlyPage(ValidationPage):
    ...
    """
    def __init__(self, site, release, testtype, date, rawhide=False):
        self.date = date
        ...
        if rawhide is True:
            ...
            self.rawhide = ""
        super(NightlyPage, self).__init__(site, release, testtype)

        # String that will generate a clean copy of the page using the test
        # page generation template system.
        self.text = ("{{{{subst:Validation results|testtype={0}|release={1}"
                        "|date={2}|rawhide={3}}}}}").format(
                testtype, release, date, self.rawhide)

    @classmethod
    def nightly_from_wikipage(cls, wikipage):
        """Get a NightlyPage object from a mediawiki Page object. This
        may seem rather roundabout, but it's useful for e.g. for
        getting existing validation pages when you don't know what
        composes and test types exist - you can get the wikipage
        objects from a Release instance and then run them through this
        ...
        Works by matching a regexp based on the naming convention for
        validation pages. If it thinks the wiki page name matches,
        it'll try and instantiate the NightlyPage object. If that
        succeeds, as a further sanity check, it checks that the name
        properties match - we don't want you to get a NightlyPage that
        actually writes to a different wiki page than the one you
        based it on. If any of this fails, it returns None.

        Accepts any 8-digit sequence as the date (expects 20141208
        format). Will match releases from 1 to 999 and almost anything
        for 'testtype'.
        """
        if not wikipage.exists:
            return None
        ...
        match = pattern.match(wikipage.name)
        if match:
            if "rawhide=true" in wikipage.edit():
                rawhide = True
            else:
                rawhide = False
            try:
                page = cls(wikipage.site, match.group(1), match.group(3),
                           date=match.group(2), rawhide=rawhide)
            except:
                return None
        if page:
            if (page.wikiname.replace(' ', '_') !=
...
class SummaryPage(Page):
    ...
    def __init__(self, site, event):
        ...
        self.wikiname = "Test Results:Fedora {0} Summary".format(event.version)
        self.summary = ("Relval bot-created validation results summary for "
                        "{0}").format(event.version)
        self.text = ("Fedora {0} [[QA:Release validation test plan|release " 
...
class CategoryPage(Page):
    ...
    def __init__(self, site, release, milestone=None, nightly=False):
        ...
        if nightly is True:
            self.wikiname = ("Category:Fedora {0} Nightly Test "
            ...
            self.wikiname = "Category:Fedora {0} {1} Test Results".format(
                    release, milestone)
            self.text = ("{{{{Validation results milestone category|release="
            ...
            self.wikiname = "Category:Fedora {0} Test Results".format(release)
            self.text = ("{{{{Validation results milestone category|release="
                         "{0}}}}}").format(release)
            self.summary = ("Relval bot-created validation result category page"
