...
class Page(mwp.Page):
    ...
    def results_wikitext(self):
        ...
        """
        pagetext = self.text().encode('utf-8')
    ...
    def text(self, section=None, expandtemplates=False):
        """Same as the original, except cache the results until the
        next page edit.
        """
        key = (section, expandtemplates)
        if not key in self._textcache:
            self._textcache[key] = super(Page, self).text(*key)
...
class ValidationPage(Page):
    ...
    def add_results(self, resultsdict, allowdupe=False):
        ...
        dupes = list()
        newtext = oldtext = self.text().encode('utf-8')
        for (row, results) in resultsdict.items():
            ...
            secoff = [sec['byteoffset'] for sec in self.sections if
                      sec['index'].encode('utf-8') == row.secid][0]
            if secoff:
        ...
        if len(resultsdict) > 3:
            testtext = ', '.join(row.name for row in resultsdict.keys()[:3])
    ...
    def add_result(self, result, row, env, allowdupe=False):
        ...
        return self.add_results(resdict)
