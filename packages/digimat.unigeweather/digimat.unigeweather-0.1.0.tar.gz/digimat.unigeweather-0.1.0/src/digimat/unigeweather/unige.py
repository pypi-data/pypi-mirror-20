...
# Parser/extractor for weather unige data files
...
class UnigeWeatherData(object):
                ...
                if not data:
                    try:
                        ...
                        if r and r.status_code==200:
                            ...
                            csvfile=CommentedFile(StringIO(zfile.read(zfile.namelist()[0])))
                    except:
                        pass
                ...
                for row in reader:
                    ...
                    if rdata:
                        if self._headers:
                            ...
                            self._headers=[s.lower() for s in rdata]
    ...
    def valuesFromTo(self, name, dt1, dt2):
        values=[]
        year=dt1.year
        while year<=dt2.year:
            self.load(year)
    ...
    def min(self, name, dt1, dt2):
        return min(self.valuesFromTo(name, dt1, dt2))

    def max(self, name, dt1, dt2):
        return max(self.valuesFromTo(name, dt1, dt2))

    def mean(self, name, dt1, dt2):
        values=self.valuesFromTo(name, dt1, dt2)
        if values:
            count=len(values)
