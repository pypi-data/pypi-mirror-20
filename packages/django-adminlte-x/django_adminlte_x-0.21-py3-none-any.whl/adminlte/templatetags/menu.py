...
class _Menu:
    ...
    def render(self, context, menus={}):
        ...
        if len(menus) <= 0:
            ...
            r = '<li><a href="' + reverse('admin:index') + '"><i class="fa fa-home"></i> <span>Home</span></a></li>'
    ...
    def admin_apps(self, context, r):
        ...
        for app in context['available_apps']:
            ...
            try:
                if self.apps_icon[app['name'].title()] != '':
                    if re.match(r'\<([a-z]*)\b[^\>]*\>(.*?)\<\/\1\>', self.apps_icon[app['name'].title()]):
                        icon = self.apps_icon[app['name'].title()]
                    else:
                        icon = '<i class="%s"></i>' % (self.apps_icon[app['name'].title()])
            except:
                pass

            if app['app_url'] in current_url:
                r += '<li class="treeview active"><a href="#">%s <span>%s</span><span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a><ul class="treeview-menu">\n' % (
                    icon, app['name'])
            else:
                r += '<li class="treeview"><a href="#">%s <span>%s</span><span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a><ul class="treeview-menu">\n' % (
                    icon, app['name'])
    ...
    def set_app_icon(self, app_name, icon):
        self.apps_icon[app_name.title()] = icon

    def get_app_icon(self, context):
        print(context['app']['name'])
        icon = '<i class="fa fa-circle"></i>'
        if context['app']['name'].title() in self.apps_icon:

            if self.apps_icon[context['app']['name'].title()] != '':
                if re.match(r'\<([a-z]*)\b[^\>]*\>(.*?)\<\/\1\>',
                            self.apps_icon[context['app']['name'].title()]):
                    icon = self.apps_icon[context['app']['name']]
                else:
                    icon = '<i class="%s"></i>' % (self.apps_icon[context['app']['name'].title()])
...
@register.simple_tag(takes_context=True)
def app_icon(context):
    return Menu.get_app_icon(context)
