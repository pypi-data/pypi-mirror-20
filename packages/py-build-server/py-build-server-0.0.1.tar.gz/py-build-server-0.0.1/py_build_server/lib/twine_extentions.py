...
class Twine(object):
    ...
    def upload(self, call, tag):
        if LatestTagFileParser.is_tag_in_file(self.repo, tag):
            return
        os.popen('cd {}; {}'.format(call.repo.working_dir, call.repo.config.build_command)).read()
        result = UploadCallResult(os.popen(str(call)))
        os.popen('rm -rf {}/dist'.format(call.repo.working_dir)).read()
        print(result)
...
class UploadCall(object):
    def __init__(self, repo):
        self.dist = '{}/dist/*'.format(repo.working_dir)  # type: str
        self.repo = repo
        ...
        self.load_yaml_config(repo.config.twine_conf)
