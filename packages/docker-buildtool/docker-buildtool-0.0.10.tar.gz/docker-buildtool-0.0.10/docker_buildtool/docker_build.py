...
class DockerBuild(object):
    def __init__(self, dockerfile, build_root, include, docker_repo, workdir=None, ignore=None, tag=None, default_ignore=None, include_version_file=False, variables=None, no_fetch=True, no_ignore_override=False, quiet=False):
...
""".format(self.workdir))
    ...
    def write_version_string(self, outfile, timestamp, dryrun=False):
        ...
        versions = v.run(no_fetch=(dryrun or self.no_fetch))
    ...
    def run(self, dryrun=False):
        ...
        try:
            ...
            if self.include_version_file:
                ...
                logger.info("Including version file. Temporary file at %s", versionfile)
            ...
            utils.execute_command(dryrun, ['docker', 'build', '-f', custom_dockerfile.name] + tag + compress + [self.build_root], **kwargs)
