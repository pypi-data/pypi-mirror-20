...
"""
    ...
    The `ast2` module helps Python applications to process trees of the Python
    abstract syntax grammar.  The abstract syntax itself might change with
    each Python release; this module helps to find out programmatically what
    the current grammar looks like and allows modifications of it.  The `ast2`
    module is similar to the builtin `ast` module on Python 2.7, except `ast2`
    ...
    classes all inherit from `ast2.AST`.
...
def parse(source, filename='<unknown>', mode='exec'):
    ...
    """
    return _ast2.parse(source, filename, mode)
