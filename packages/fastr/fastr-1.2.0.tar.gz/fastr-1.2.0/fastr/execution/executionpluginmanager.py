...
from fastr.execution.job import Job, JobState, SourceJob
...
class ExecutionPlugin(Plugin):
    ...
    * :py:meth:`_release_job <fastr.execution.executionpluginmanager.ExecutionPlugin._release_job>`
    ...
    def __init__(self, finished_callback=None, cancelled_callback=None, status_callback=None):
        ...
        self.number_of_source_jobs_queued = 0
    ...
    def queue_job(self, job):
        ...
        if isinstance(job, SourceJob):
            if self.number_of_source_jobs_queued < self.source_job_limit:
                fastr.log.debug('Claiming source job slot {} -> {}'.format(self.number_of_source_jobs_queued,
                                                                           self.number_of_source_jobs_queued + 1))
                self.number_of_source_jobs_queued += 1
            else:
                # Keep the job in a temporary queue
                fastr.log.debug('Putting sour job {} in source queue due to limitations!'.format(job.id))
    ...
    def cancel_job(self, job):
        ...
        # Write the final state of the job to the logfile
        ...
        if isinstance(job, SourceJob):
            if len(self.source_job_queue) > 0:
                ...
                if self.SUPPORTS_HOLD_RELEASE:
                    ...
                    self._dispatch_job(new_source_job)
            else:
                fastr.log.debug('Releasing source job slot {} -> {}'.format(self.number_of_source_jobs_queued,
                                                                            self.number_of_source_jobs_queued - 1))
                self.number_of_source_jobs_queued -= 1
    ...
    def _job_finished_body(self, job, errors):
        ...
        if errors is not None and len(errors) > 0:
            job.info_store['errors'].extend(errors)
        ...
        if job_id in self.held_queue:
            ...
            for held_job_id in self.held_queue[job_id]:
                ...
                if action == JobAction.queue:
                    ...
                    if held_job_id in self.job_dict:
                        held_job = self.job_dict[held_job_id]
                        self.job_dict[held_job_id] = held_job
                ...
                for hold_id in job.hold_jobs:
                    if hold_id in self.held_queue:
                        with self.held_queue_lock:
                            ...
                            if ready_job_id in required_job:
                                required_job.remove(ready_job_id)
                            else:
                                fastr.log.warning('Could not remove {} from dependencies {} of {}'.format(ready_job_id, required_job, hold_id))

            with self.held_queue_lock:
                del self.held_queue[job_id]
    ...
    def check_job_requirements(self, job_id):
        ...
        status_list = [self.check_job_status(job_id) for job_id in job.hold_jobs]
