...
class SampleItemBase(tuple):
    ...
    def __new__(cls, index, id_, data, jobs=None, failed_annotations=None):
        ...
        if not isinstance(jobs, (set, tuple, list)):
            raise exceptions.FastrTypeError("Jobs should be a set, tuple or list of jobs or job ids, found {}".format(type(jobs).__name__))
        ...
        if not isinstance(failed_annotations, set):
            raise exceptions.FastrTypeError("failed_annotations should be a set, found {}".format(type(failed_annotations).__name__))
...
class SampleCollection(MutableMapping):
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(value, SampleItem):
            ...
            if len(value) == 3 and isinstance(value[0], SampleValue) and isinstance(value[1], set) and isinstance(value[2], set):
                # Create the SampleItem
                value = SampleItem(sample_index, sample_id, value[0], value[1], set())
            else:
