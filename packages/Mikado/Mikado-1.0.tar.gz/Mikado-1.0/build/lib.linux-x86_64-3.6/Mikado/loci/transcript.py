...
class Transcript:
    ...
    @property
    @functools.lru_cache()
    def internal_orf_boundaries(self):

        """This property returns a list of tuples containing the CDS boundaries of internal ORFs."""
        ...
        for orf in self.internal_orfs:
            ...
            for segment in [_ for _ in orf if _[0] == "CDS"]:
                start = min(start, segment[1][0])
                end = max(end, segment[1][1])
