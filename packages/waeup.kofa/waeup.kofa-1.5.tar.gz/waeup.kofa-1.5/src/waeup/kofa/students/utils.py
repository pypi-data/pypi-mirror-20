## $Id: utils.py 14473 2017-01-27 15:53:33Z henrik $
##
...
class StudentsUtils(grok.GlobalUtility):
    ...
    def renderPDFTranscript(self, view, filename='transcript.pdf',
                  ...
                  tableheader=None,
                  no_passport=False):
        """Render pdf slips for transcripts.
        """
        portal_language = getUtility(IKofaUtils).PORTAL_LANGUAGE
        # XXX: tell what the different parameters mean
        style = getSampleStyleSheet()
    ...
    def renderPDFCourseticketsOverview(
            self, view, session, data, lecturers):
        ...
        if data[1]:
            ...
            pdf_data += [Paragraph(
                ...
                mapping = {'a':data[1][1],'b':data[1][2]})), style["Normal"]),]
            pdf_data += [Paragraph(
        ...
        pdf_stream = creator.create_pdf(
            pdf_data, None, doc_title, author, doc_title + ' -'
            )
        return pdf_stream

    def maxCredits(self, studylevel):
        """Return maximum credits.
        At some universities maximum credits is not constant, it
