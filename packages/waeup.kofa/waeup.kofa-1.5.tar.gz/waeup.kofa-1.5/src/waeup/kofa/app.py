## $Id: app.py 14511 2017-02-07 08:33:05Z henrik $
##
...
class University(grok.Application, grok.Container, Logger):
    ...
    def updatePlugins(self):
        """Lookup plugins and call their `update()` method to
        upgrade Kofa's database since software version 1.4 (2016-01-14).
        """
        getUtility(IKofaPluggable, name='faculties').update(
            self, 'faculties', self.logger)
        getUtility(IKofaPluggable, name='departments').update(
            self, 'departments', self.logger)
        getUtility(IKofaPluggable, name='certificates').update(
            self, 'certificates', self.logger)
        return
