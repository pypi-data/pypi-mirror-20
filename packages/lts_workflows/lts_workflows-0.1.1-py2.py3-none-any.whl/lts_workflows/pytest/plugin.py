...
def addoption(parser, engine="snakemake"):
    ...
    if engine == "snakemake":
        parser.addoption("-2", "--python2-conda", action="store",
                         ...
                         dest="python2_conda")
        parser.addoption("-C", "--use-conda", default=False, action="store_true",
...
def namespace(engine="snakemake", **kwargs):
    m = re.search("--conda-install-dir\s+(?P<conda_install_dir>\w+)", " ".join(sys.argv))
    install_dir = m.group("conda_install_dir") if m else CONDA_DEFAULT_INSTALL_DIR
    python2 = conda_create_env(name="python2", envlist=kwargs.get('py2', []), install_dir=install_dir, name_only=True)
    python3 = conda_create_env(name="python3", envlist=kwargs.get('py3', []), install_dir=install_dir, name_only=True)
    envlist = []
    if engine == "snakemake":
        retval = {
        ...
        nextflow = conda_create_env(name="nextflow", envlist=kwargs.get('nextflow', []), install_dir=install_dir, name_only=True)
        retval = {
...
def slow_runtest_setup(item):
    ...
    if getattr(item.obj, 'no_slow', None) and item.config.getvalue('no_slow'):
...
def _snakemake_report_header(config):
    try:
        ...
        py2 = [x for x  in envs['envs'] if re.search("{}{}$".format(os.sep, config.option.python2_conda), x)][0]
    ...
    if not pytest.config.option.enable_test_conda or pytest.config.option.use_conda:
        ...
        conda = ["conda environments:"] + ["  {}: {}".format(name, path) for (name, path, env) in pytest.conda_env_list]
...
def _nextflow_report_header(config):
    try:
        output = sp.check_output(['conda', 'env', 'list', '--json'])
        envs = ast.literal_eval(output.decode("utf-8"))
        nf = [x for x  in envs['envs'] if re.search("{}{}$".format(os.sep, config.option.nextflow_conda), x)][0]
...
# Create environments
##############################
def conda_create_env(name, envlist, install_dir=CONDA_DEFAULT_INSTALL_DIR, disable=False, name_only=False, package_path=None, **kwargs):
    ...
    Args:
      ...
      package_path: package path of calling test module to be added to conda.pth
    """
    if disable:
        return None
    ...
    try:
        version = sp.check_output(["conda", "--version"], stderr=sp.STDOUT).decode().split()[1]
    ...
    env_path = os.path.join(install_dir, "_".join([name, md5hash.hexdigest()[:8]]))
    if name_only:
        return env_path
    if len(envlist) == 0:
        return None
    ...
    if not os.path.exists(env_path):
        ...
        conda_args = ["env", "create", "--file", env_file, "--prefix", env_path]
...
def conda_environments(request):
    ...
    for name, path, envlist in pytest.conda_env_list:
        conda_create_env(name, envlist, install_dir=pytest.config.option.conda_install_dir, package_path=package_path)
