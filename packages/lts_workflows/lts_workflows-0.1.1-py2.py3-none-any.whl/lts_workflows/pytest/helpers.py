...
SNAKEMAKE_LAYOUTS = {
    ...
    "sample_project_run" :  {'runfmt':'{SM}/{BATCH}/{PU}/{SM}_{PU}', 'samplefmt':'{SM}/{SM}'},
    "pop_sample" : {'runfmt':'{POP}/{SM}/{SM}_{PU}', 'samplefmt':'{POP}/{SM}/{SM}'},
    "pop_sample_run" :  {'runfmt':'{POP}/{SM}/{PU}/{SM}_{PU}', 'samplefmt':'{POP}/{SM}/{SM}'},
    "pop_sample_project_run" :  {'runfmt':'{POP}/{SM}/{BATCH}/{PU}/{SM}_{PU}', 'samplefmt':'{POP}/{SM}/{SM}'},
}
...
def run(cmd, env=None, stdout=sp.PIPE, stderr=sp.STDOUT):
    ...
    if env:
        path = ":".join([os.path.join(p, "bin") for (n, p, env) in pytest.conda_env_list])
...
def snakemake_list(snakemakedata, results, env=None, **kwargs):
    ...
    output, err = run(cmd, env, sp.PIPE, sp.PIPE)
    print("\n", err.decode("utf-8"))
    assert(err.decode("utf-8").find(results) > -1)
...
def snakemake_run(d, env=None, **kwargs):
    ...
    output, err = run(cmd, env, kwargs.get("stdout", None), kwargs.get("stderr", None))
