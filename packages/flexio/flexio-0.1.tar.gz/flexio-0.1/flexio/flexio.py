...
class FlexioPipe:
    ...
    def run(self, params={}, files={}):
        response = requests.post('https://{0}/api/v1/pipes/{1}/run'.format(self.connection.host,self.identifier), data=params, files=files, verify=False, headers={'Authorization': "Bearer %s" % self.connection.api_key})
        print(response.text)
