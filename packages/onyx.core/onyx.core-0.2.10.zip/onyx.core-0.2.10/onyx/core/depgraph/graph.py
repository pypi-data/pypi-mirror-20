...
class PropertyVT(BaseVT):
    def __init__(self, fget):
        ...
        if fget.__code__.co_argcount > 2:
            raise GraphError("A Property VT doesn't accept "
...
class CallableVT(BaseVT):
    """
    Descriptor used for callable ValueTypes. Notice that for this ValueType
    the input arguments are part of the node_id so that the same ValueType
    called with different arguments will produce different nodes in the graph.
    The limitation is that, for efficiency reason, only positional arguments
    are supported.
    """
...
class PropSubGraphVT(BaseVT):
    """
    Descriptor used to represent a subgraph with property-like nodes. The input
    arguments are treated as children of the node so that when called with
    different arguments the node in the graph will be invalidated.
    """
    def __init__(self, func):
        self.func = func
    ...
    def __call__(self, *args, **kwds):
        ...
        return self.func(self.instance, graph, *args, **kwds)
