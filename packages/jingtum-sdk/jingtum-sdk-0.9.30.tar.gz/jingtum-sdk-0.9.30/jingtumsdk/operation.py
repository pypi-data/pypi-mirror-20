...
class PaymentOperation(Operation):
    ...
    def getBatchPara(self):
        ...
        para = {}
        para["payment"] = _payment
        para["type"] = "Payment"
        para["account"] = self.src_address

        return para, self.src_secret
...
class SettingsOperation(Operation):
    ...
    def getBatchPara(self):
        para = {}
        para["type"] = "AccountSet"
        para["account"] = self.src_address
        para["settings"] = self.settings
...
class RelationsOperation(Operation):
    ...
    def getBatchPara(self):
        para = {}
        para["type"] = self.relation_type
        para["counterparty"] = self.counterparty
        para["amount"] = self.amt
...
class BatchOperation(Operation):
    def __init__(self, wallet):
        super(BatchOperation, self).__init__(wallet)

        self.operations = []
        self.signers = []

    def setOperation(self, opobj):
        para, secret = opobj.getBatchPara()
        self.operations.append(para)
        self.signers.append({"secret": secret})


    def oper(self):
        _para = {}
        _para["secret"] = self.src_secret
        _para["signers"] = self.signers
        _para["operations"] = self.operations
        _para["client_resource_id"] = self.client_resource_id

        if self.is_sync:
          url = 'accounts/{address}/operations?validated=true'
        else:
