...
from libs.payments.weixin.request import WxpayRequest
from libs.payments.weixin.response import WxpayResponse
...
class WeixinBaseParam(object):
    ...
    def __init__(self, account):
        self.appid = account.app_id
        self.mch_id = account.mch_id
...
class WechatBaseApi(object):
    def __init__(self, account, response_class=None, business=None):
        self._host = getattr(settings, 'WEIXIN_PAY_V4_API_HOST', None)
        self._account = account
        self._business = business
        if not response_class:
            self.response_class = WxpayResponse
        else:
    ...
    def post(self, post_data, resource, bussiness):
        req = self.get_request(resource)
        resp = req.post(post_data)
        wx_resp = self.response_class(bussiness, resp)
        return wx_resp

    def get(self, resource, bussiness):
        req = self.get_request(resource)
        resp = req.get()
        wx_resp = self.response_class(bussiness, resp)
        return wx_resp

    def delete(self, resource, bussiness):
        req = self.get_request(resource)
        resp = req.delete()
        wx_resp = self.response_class(bussiness, resp)
        return wx_resp
