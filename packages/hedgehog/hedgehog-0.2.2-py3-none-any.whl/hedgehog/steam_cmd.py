...
class SteamCMD:
    ...
    def unpack_bin(self):
        ...
        try:
            os.makedirs(self.steam_cmd_dir)
    ...
    def check_steam_cmd(self):
        steam_bin = os.path.join(self.steam_cmd_dir, 'steamcmd.sh')

        if not os.path.exists(steam_bin):
            return False

        return launch_external_program([steam_bin, '+quit'])
