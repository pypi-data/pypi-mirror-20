...
class Intervallometre(threading.Thread):
    ...
    def run(self):
        ...
        while self.encore:
            ...
            val=self.parent.progressBar.value()
            if val+10>self.parent.progressBar.maximum():
                self.parent.progressBar.setValue(0)
            else:
                self.parent.progressBar.setValue(val+10)
...
class scanPlot(QtGui.QDialog):
    ...
    def StartAuto(self):
        self.th=Intervallometre(2.0,self.OnClickRefresh,self)
        self.th.start()
