...
class PartialNode(object):
        ...
        if isinstance(upper_args[0], str):
            ...
            def __decorator(func):
                def __inner(request):
                    n_args = func.func_code.co_argcount
                    if "self" in func.func_code.co_varnames[:n_args]:
                        return self.__class_service(
                            func, request, service_name)
                    else:
                        return self.__function_service(
                            func, request, service_name)
            ...
            raise ValueError(
                "First argument to service must be service name as str")
    ...
    def __class_subscriber(self, func, msg, topic_name):
        if func.func_code.co_argcount == 2 and self.slf is not None:
            return func(self.slf, msg)
        elif func.func_code.co_argcount == 3 and self.slf is not None:
    ...
    def __class_service(self, func, request, service_name):
        if func.func_code.co_argcount == 2:
            ...
            raise NotImplementedError(
                "Should not happen, but now you can "
                + "search for this exception in __class_service")
    ...
    def __function_service(self, func, request, service_name):
        if func.func_code.co_argcount == 1:
            ...
            raise NotImplementedError(
                "Should not happen, but now you can "
                + "search for this exception in __function_service")
        # elif func.func_code.co_argcount == 2:
