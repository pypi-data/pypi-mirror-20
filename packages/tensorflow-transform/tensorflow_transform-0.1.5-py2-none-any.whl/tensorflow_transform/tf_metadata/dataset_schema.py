...
def _from_parse_feature(parse_feature):
  ...
  if isinstance(parse_feature, tf.FixedLenFeature):
    domain = dtype_to_domain(parse_feature.dtype)
    shape = _tf_shape_to_logical_shape(parse_feature.shape)
    representation = FixedColumnRepresentation(parse_feature.default_value)
    return ColumnSchema(domain, shape, representation)
...
def infer_column_schema_from_tensor(tensor):
  ...
  if isinstance(tensor, tf.SparseTensor):
    ...
    domain = dtype_to_domain(tensor.dtype)
    shape = [Axis(None)]
    representation = ListColumnRepresentation()
  else:
    domain = dtype_to_domain(tensor.dtype)
    shape = _tf_shape_to_logical_shape(tensor.get_shape(),
