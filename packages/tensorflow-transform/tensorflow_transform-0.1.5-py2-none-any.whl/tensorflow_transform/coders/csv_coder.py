...
class _FixedLenFeatureHandler(object):
  ...
  def __init__(self, name, feature_spec, index, reader=None, encoder=None):
    ...
    if self._reader is None and len(feature_spec.shape):
      if len(feature_spec.shape) == 1 and feature_spec.shape[0] == 1:
        self._is_singleton = True
        self._default_value = [self._default_value]
      else:
        raise ValueError(
            'Columns with no secondary delimiter should have shape () or (1,) '
            'but shape={}'.format(feature_spec.shape))
  ...
  def encode_value(self, string_list, values):
    if self._encoder:
      ...
      string_list[self._index] = str(values[0])
    else:
...
class CsvCoder(object):
  ...
  def decode(self, csv_string):
    ...
    # An empty string when we expect a single column is potentially valid.
