...
class BaseIrclient(irc.client.SimpleIRCClient, ConnectionStates, Variables):
    ...
    def start(self):
        ...
        try:
            ...
            network_error = u"Failed to connect to Server"
        except Banned:
            self.logger.log(u"Failed to connect due to a ban", LOG.BANNED)
            network_error = u"Failed to connect due to a ban"
        except Exception, e:
            try:
                self.quit()
            except (Disconnect, irc.client.ServerNotConnectedError):
                pass
            if unicode(type(e)) != u"<class 'xdcc_dl.xdcc.layers.irc.BaseIrcClient.Disconnect'>":
