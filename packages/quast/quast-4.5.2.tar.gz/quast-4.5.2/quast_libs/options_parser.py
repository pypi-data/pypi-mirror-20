...
def parse_options(logger, quast_args, is_metaquast=False):
    ...
    if qconfig.test or qconfig.test_no_ref or qconfig.test_sv:
        ...
        if any(not isfile(fpath) for fpath in contigs_fpaths):
            logger.error('Test files are not found.\n'
                         'You are probably running an installation via pip. To run tests, please download the data '
                         'from our repository at https://github.com/ablab/quast/tree/master/test_data, ' +
                         'or install quast from source instead:\n   > git clone https://github.com/ablab/quast && cd quast && setup.py install')
            sys.exit(2)
