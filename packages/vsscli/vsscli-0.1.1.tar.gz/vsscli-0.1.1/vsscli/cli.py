...
@click.option('-p', '--parent', type=str,
              ...
              required=True)
@click.pass_context
def compute_folder_mk(ctx, parent_moref, name):
    ...
    try:
        cli_manager = ctx.obj['CLIManager']
        parent_moref = str(parent_moref)
        cli_manager.get_folder(parent_moref)
        request = cli_manager.create_folder(moref=parent_moref,
                                            name=name)
        if not cli_manager.output_json:
            lines = '\n'.join(print_request(request))
        else:
...
def compute_vm_get_floppies(ctx, unit):
    ...
    try:
        ...
        if unit:
            ...
            if cli_manager.output not in ['json']:
                _lines = print_vm_cds_summary(ctx.obj['UUID'], devs)
                lines = '\n'.join(_lines)
            else:
...
@compute_vm_create.command('shell',
                           short_help='Empty virtual machine')
@click.argument('name', type=str, required=True)
