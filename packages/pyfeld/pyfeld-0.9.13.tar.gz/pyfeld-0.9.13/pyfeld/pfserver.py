        ...
        try:
            for upnp in upnp_service.services_list:
                ...
                if not self.__is_subscribed(nl, upnp['eventSubURL'], udn):
                    self.create_new_subscription(local_ip, 8088, nl, upnp['eventSubURL'], udn)
                ...
                try:
                    ...
                    for upnp_service in zone.services.services_list:
                        ...
                        if not self.__is_subscribed(nl, upnp_service['eventSubURL'], media_server.udn):
                            self.create_new_subscription(local_ip, 8088, nl, upnp_service['eventSubURL'], zone.get_udn() )
                    ...
                    for room in zone.rooms:
                        for upnp_service in room.upnp_service.services_list:
                            ...
                            if not self.__is_subscribed(nl, upnp_service['eventSubURL'], room.get_udn()):
                                self.create_new_subscription(local_ip, 8088, nl, upnp_service['eventSubURL'], room.get_udn())
                except Exception as e:
...
def run_subscription_server(port):
    try:
...
def run_main():
    ...
    threads = []
    t = threading.Thread(target=subscription_handler.subscription_thread, args=(60,))
    threads.append(t)
    t.start()
