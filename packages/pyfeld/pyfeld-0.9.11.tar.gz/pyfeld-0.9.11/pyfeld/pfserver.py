...
def play_browe_content(browse_query, onebased_index, room_name):
    uc_media = UpnpCommand(RfCmd.rfConfig['mediaserver'][0]['location'])
...
def play(what, onebased_index, room_name):
    ...
    createzone_if_room_unassigned(room_name)
    if what == 'localradio':
        return play_browe_content("0/RadioTime/LocalRadio", onebased_index, room_name)
    elif what == 'station':
        return play_station(onebased_index, room_name)
    elif what == 'mostplayed':
        return play_station(onebased_index, room_name)
