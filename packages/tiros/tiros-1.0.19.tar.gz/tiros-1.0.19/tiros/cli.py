...
def get_throttle(args):
    throttle = {}
    if args.max_calls_per_minute_ec2:
        throttle['ec2'] = args.max_calls_per_minute_ec2
    if args.max_calls_per_minute_elb:
        throttle['elb'] = args.max_calls_per_minute_elb
    if args.max_calls_per_minute_elb2:
        throttle['elb2'] = args.max_calls_per_minute_elb2
    return throttle
...
server_args.add_argument(
    '--max-calls-per-minute-ec2',
    help='Throttle the elasticloadbalancing endpoints'
)

server_args.add_argument(
    '--max-calls-per-minute-elb',
    help='Throttle the elasticloadbalancing endpoints'
)

server_args.add_argument(
    '--max-calls-per-minute-elb2',
    help='Throttle the elasticloadbalancing endpoints'
)
