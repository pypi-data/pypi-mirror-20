...
class Manager(object):
    ...
    def __init__(self, image, cmd=[], n=1, ports=[], cap_add=[], enable_logging=True):
        self.image = image
        self.cmd = cmd
        self.n = n
    ...
    def _start(self):
        ...
        for _ in range(self.n):
            instance = DockerInstance(self.assigner, self.image, label=self.assigner.next_instance_num(), internal_ports=self.internal_ports, cap_add=self.cap_add, cmd=self.cmd)
        ...
        while True:
            ...
            for instance in self.instances:
                if instance.booted():
                    continue
                keep_waiting += 1
                self.instance.healthcheck(logs=True)
                time.sleep(1)
                delta = time.time() - start
                if delta > 120:
                    raise error.Error('{} did not boot in {}'.format(self.instance.label, delta))
            if keep_waiting == 0:
                break
...
class DockerInstance(object):
    def __init__(self, assigner, image, cmd=[], internal_ports=[], cap_add=[], label='main'):
    ...
    def booted(self):
        return utils.port_open(self.host, self.external_ports[0])
