...
(weight.ndim, ndim))
        ...
        # If the input data array 'fits' in memory, then make sure
        # that it has only one partition
        if (not _preserve_partitions and
            d._pmndim and d.fits_in_memory(d.dtype.itemsize)):
            d.varray
...
#        print 'DCH old.shape =', d.shape
#        print 'DCH new.shape =', new.shape
        for partition in new.partitions.matrix.flat:
...
'''        
    ...
    def var(self, axes=None, squeeze=False, weights=None, mtol=1, ddof=1,
            a=None, i=False, _preserve_partitions=False):
        r'''
