...
from .cfdatetime import Datetime, elements
...
# Default monthl lengths in days
_default_month_lengths = [None, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
...
#            if self.duration >= one_year:
#                self.duration.Units = _calendar_years
#        else:
#            if self.duration >= one_day:
#                self.duration.Units = _days
#            elif self.duration >= one_hour:
#                self.duration.Units = _hours
#            elif self.duration >= one_minute:
#                self.duration.Units = _minutes
...
'''
        ...
        if isinstance(other, (self.__class__, int, long, float)):
...
'''   
    ...
    def _binary_operation(self, other, method, inplace=False):
        ...
        if duration.size != 1:           
            raise ValueError(
"Can't create {0} with more than one value: {1}".format(
    self.__class__.__name__, duration))
            
        if duration < 0:
            raise ValueError(
"Can't create {0} with a negative duration".format(self.__class__.__name__))

        if not duration.Units.equals(self.duration.Units):
            if method not in ('__eq__', '__ne__', '__lt__', '__le__', '__gt__', '__ge__'):
                # Operator is not one of ==, !=, >=, >, <=, <                
                raise ValueError(
"Can't create {0} of {1!r}".format(self.__class__.__name__, duration.Units))
...
'''
        ...
        if units == _calendar_years:
            ...
            if months != months0:
                raise ValueError(
"Fractional months not supported for date calculations: {0}".format(months0))
...
.. versionadded:: 1.4
        '''
        return Datetime(*[(i if j is None else j)
...
:Parameters:
    ...
    calendar: `str`, optional
        By default, calendar is the mixed Gregorian/Julian calendar as
        defined by Udunits.
    ...
    month_lengths: sequence of `int`, optional
        By default, *month_lengths* is ``[None, 31, 28, 31, 30, 31,
...
:Examples 2:
        ...
        elif len(month_lengths) != 13:
            raise ValueError("month_lengths must be a sequence of 13 elements")
...
>>> t.inspect()
<CF TimeDuration: 9 days (from Y-M-D 00:00:00)>
-----------------------------------------------
day: 1
...
#                 minute=None, second=None, calendar=None, end=False,
#                 iso=None):
    def interval(self, dt, calendar=None, end=False, iso=None):
...
*minute* and *second* parameters, but if any of these is unset then
its default value is taken from the attribute of the same name
(i.e. the `!year`, `!month`, `!day`, `!hour`, `!minute` and `!second`
...
:Parameters:
    ...
    calendar: `str`, optional
        Define the CF calendar for the input date-time and output time
        interval. By default the calendar of the input date-time is
        used. If *calendar* is set, then the calendar of the input
        date-time is overridden.
    ...
    months))
        ...
        if int_months is not None:
            if not end:
                ...
                dt0 = dt.copy()
                dt1 = dt.replace(year1, month1, day1)
            else:
                ...
                dt0 = dt.replace(year0, month0, day0)
                dt1 = dt.copy()
        else:
...
#               second=0, calendar=None, direction=True):
    def bounds(self, dt, calendar=None, direction=True):
        '''Return a time interval containing a date-time.

The interval spans the time duration and starts and ends at date-times
consistent with the time duration's offset.

.. seealso:: `cf.dt`, `cf.Datetime`, `interval`
...
:Parameters:
    ...
    dt: date-time-like
        The date-time to be contained by the interval. *dt* may be any
        date-time-like object, such as `cf.Datetime`,
        `datetime.datetime`, `netCDF4.netcdftime.datetime`, etc.

          *Example:*
            To find bounds around 1999-16-1 in the Gregorian calendar
            you could use ``dt=cf.dt(1999, 1, 16)`` or
    ...
    direction: bool, optional
        ...
        are increasing. Note that ``t.bounds(dt, direction=False)`` is
        equivalent to ``t.bounds(dt)[::-1]``.
