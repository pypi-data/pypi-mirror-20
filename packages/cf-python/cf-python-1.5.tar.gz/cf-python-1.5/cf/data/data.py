        ...
        if dt or units.isreftime:
            ...
            if kind == 'S':
                ...
                if not units:
                    ...
                    # reference times
                    units = Units('days since '+str(data[(0,)*data.ndim]), units._calendar)
                    self._Units = units
...
#            else:
            try:
                if method == '__eq__' and data0.Units.isreftime:
                    array0 = numpy_isclose(array0, array1)
                else:
...
#        print '\nDCH self.Units, axes, self._pmshape=', self.Units, axes, self._pmshape
        ndim = d._ndim
        self_axes  = d._axes
        self_shape = d._shape        
...
(weight.ndim, ndim))
        ...
        if (not _preserve_partitions and
            ...
            d.varray
#        print 'DCH 9 d.shape d._pmshape  =', d.shape, d._pmshape
...
#        print 'DCH d.shape d._pmshape  =', d.shape, d._pmshape
#        print 'DCH new.shape new._pmshape =', new.shape, new._pmshape
        for partition in new.partitions.matrix.flat:
...
#            print  'DCH partition.indices = ', partition.indices
            indices = partition.indices[:n_non_collapse_axes] + c_slice
#            print  'DCH indices = ', indices
...
'''   
        ...
        data = self[indices]
#        print 'DCH _collapse_subapce data.shape =', data.shape
        if (not _preserve_partitions and 
...
#        print  'DCH _collapse_subapce data._pmshape =', data._pmshape
        for i, partition in enumerate(data.partitions.matrix.flat):
            array = partition.dataarray(**pda_args)
#            print 'DCH _collapse_subapce i=',i, ', array.shape =',array.shape
...
#                    print "DCH kwargs['weights'].shape =",  kwargs['weights'].shape
            #--- End: if  
            
            p_out = func(array, masked=p_masked, **kwargs)
#            print 'DCH finished p_out = func(array, masked=p_masked, **kwargs)'
...
:Examples:
    ...
    def sd(self, axes=None, squeeze=False, mtol=1, weights=None,
           ddof=1, a=None, i=False, _preserve_partitions=False):
        r'''
...
'''        
    ...
    def var(self, axes=None, squeeze=False, weights=None, mtol=1, ddof=1,
            i=False, _preserve_partitions=False): #a=None, 
        r'''
