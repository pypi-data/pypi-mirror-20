...
cdef class FastStart:
    ...
    cdef bint _moov_is_compressed(self, object moov_atom):
        ...
        # type identifier)
        self._cache.seek(moov_atom['position'] + 8)

        # Step through the moov atom children to see if a cmov atom is
        # among them
        cdef size_t stop = moov_atom['position'] + moov_atom['size']
        cdef Atom child_atom

        while self._cache.tell() < stop:
            child_atom = self.read_atom(self._cache)
            self._cache.seek(self._cache.tell() + child_atom.size - 8)
    ...
    cpdef _patch_moov(self, object atom, long offset):
        ...
        self._cache.seek(atom['position'])
        # Wrap the moov in BytesIO for independent seeking
        moov = io.BytesIO(self._cache.read(atom['size']))
    ...
    def iter_atom_data(self, blacklist=None, whitelist=None):
        ...
        for atom in filter(filter_, self.indexes):
            self._cache.seek(atom['position'])
            yield self._cache.read(atom['size'])
    ...
    def from_stream(self, stream):
        ...
        while CHUNK_SIZE:
            ...
            self._cache.write(chk)
            # Skip bytes we don't need to process because we know they are part
            # of existing atoms
            chunk_skip_to = 0
            if self._cache.tell() < SKIP_TO:
                self._stream_pos = self._cache.tell()
                continue
            else:
        ...
        if offset <= 0:
            ...
            # Yield ftype
            for data in self.iter_atom_data(whitelist=ATOM_WHITELIST):
                yield data
            ...
            for data in self.iter_atom_data(blacklist=ATOM_BLACKLIST):
                yield data
