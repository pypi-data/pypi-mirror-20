...
class JujuVnfm(AbstractVnfm):
    def __init__(self, type):
        super().__init__(type)
        self.juju_command_line_semaphore = threading.Semaphore()
        self.deploy_store_charm_semaphore = threading.Semaphore()
            ...
            for lifecycle_event in vnf_lifecycle_events:
                ...
                with open(tmp_charm_dir + '/actions/' + event, 'w') as action:
                    action.write(
                        ...
                        'echo "$(date +\'%Y-%m-%d %T\') start executing $script" >> /var/log/openbaton/scriptLog\n'
                        'status-set unknown\n'
                        './$script &>> /var/log/openbaton/scriptLog\n'
                        'echo "$(date +\'%Y-%m-%d %T\') finished executing $script" >> /var/log/openbaton/scriptLog\n'
                        'status-set active\n')
    ...
    def instantiate(self, vnf_record, vnf_package, vim_instances):
        ...
        if str2bool(self._map.get("allocate", 'True')):
            ...
            for vim in vdus_by_vim:
                ...
                log.debug("Semaphore is available? %s" % self.juju_command_line_semaphore)
                with self.juju_command_line_semaphore:
