...
def wait_conn(ip, port=22, timeout=60):
    """
...
def wait_conn_thread(queue, ip, port=22, timeout=60):
    """
...
def get_params(vnfr, unit_ip_addresses, lifecycle_script, dependency=None):
...
def check_ips_for_ssh(ips, name, ssh_port=22, ssh_timeout=60):
    """
...
class JujuVnfm(AbstractVnfm):
    ...
    def get_all_ips_for_vnfc(self, vnfc):
        ip_dict = {}
        floating_ip_dict = {}
        vnfc_instance = vnfc.get("vnfc_instance")
        if not vnfc_instance:
            ...
            vnfc_instance = vnfc.get("vnfc")
        for vnf_comp in vnfc_instance:
            # log.debug(vnf_comp)
            # Check if we find the private ips, if not we for sure set the allocate option to false
            if vnf_comp.get("ips"):
                for ip in vnf_comp.get("ips"):
                    ip_dict[ip.get("netName")] = ip.get("ip")
                    # always set the internal_ip on the latest value
                    internal_ip = ip.get("ip")
                for connection_point in vnf_comp.get('connection_point'):
                    floating_ip_dict[
                        connection_point.get("virtual_link_reference") + "_floatingIp"] = connection_point.get(
                        'floatingIp')
                    # always set the floating_ip on the latest value
                    floating_ip = connection_point.get('floatingIp')
        # Throw all available IPs into 1 dictionary
        ip_dict.update(floating_ip_dict)
        return ip_dict
    ...
    def scale_out(self, vnf_record, vnf_component, scripts, dependency):
        ...
        # If we only have 1 ip we can just use the actual value of the internal ip
            ...
            #    raise RuntimeError('Not all VNFC instances of {} have floating ips.
            #       This is a requirement for using the Juju-VNFM with
            #       resource allocation on the NFVO side.'.format(vnf_name))
    ...
    def scale_in(self, vnf_record, vnfc_instance):
        ...
        # else:
        #     env = juju_helper.get_environment(vim_instance_name)
        # TODO we need the name of the vim on which the vnfc runs
    ...
    def instantiate(self, vnf_record, vnf_package, vim_instances):
        ...
        if str2bool(self._map.get("allocate", 'True')):
            ...
            # Assemble all available IPs in one dictionary
            ################################################
            ip_dict = self.get_all_ips_for_vnf_record(vnf_record)
            ...
            for vim in vdus_by_vim:
                ...
                for vdu in vdus:
                    ...
                    for vnfc_instance in vnfc_instances:
                        ...
                        # raise RuntimeError('Not all VNFC instances of {} have floating ips. This is a requirement for using the Juju-VNFM with resource allocation on the NFVO side.'.format(vnf_name))
                        # fip = floating_ip_list[0].get('ip')
                        application_name = vnf_name + vdu.get('id').replace('-', '')
                        manual_provisioner = juju_helper.ManualProvisioner(fip, application_name)
            ...
            if deploy_from_charm_store:
                ...
                if series not in ['precise', 'trusty', 'xenial']:
                    log.warning(
                        '{} is not a valid series for deploying a Charm. Set the VNFR\'s version to precise, '
