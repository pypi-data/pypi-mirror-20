    ...
    def modify(self, vnf_record, dependency):
        ...
        if deploy_from_charm_store:
            ...
            target_app_name = self.get_app_name(
                ...
                vdu_.get('id'))
            status = juju_requestor.get_status(target_app_name)
            log.debug("Status of %s is still %s", target_app_name, status)

            while status is None or status not in ['active', 'blocked']:
                log.debug("Status of %s is still %s", target_app_name, status)
                time.sleep(5)
                status = juju_requestor.get_status(target_app_name)
            ...
            if len(relations) != 0:
                ...
                for source_type in parameters:
                    ...
                    status = juju_requestor.get_status(source_app_name)
                    log.debug("Source app name: %s" % source_app_name)

                    while status is None or status not in ['active', 'blocked']:
                        log.debug("Status of %s is still %s", source_app_name, status)
                        time.sleep(5)
                        status = juju_requestor.get_status(source_app_name)
