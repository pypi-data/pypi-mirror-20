...
class locale():
    ...
    def import_locales(self):
        ...
        if 'locales' not in os.listdir('.'): raise Exception("Dir 'locales' not found")
    ...
    def reimport_fast_transl(self, key):
        self.import_locales()
        return self.fast_translate(key)

    def returnlocales(self):
        if 'locales' not in os.listdir('.'): raise Exception("Dir 'locales' not found")
        output = []
        for file in os.listdir("locales"):
            ...
            if file.endswith(".txt"):
                ...
                output.append(file.replace('.txt', ''))
        return output

    def multiple_t(self, keys):
        if type(keys) != str: raise Exception("Kesys must be str, not {}".format(type(keys)))
        splited = keys.split("^")
        output = ""
        for i in range(len(splited)):
            if splited[i] == " ":
                output+=" "
            else: output+=self.fast_translate(splited[i])
        return output