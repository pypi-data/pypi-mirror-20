...
def connect_to_db(conn_dict):
    conn_dict_key = hashlib.md5(
        json.dumps(conn_dict, sort_keys=True)
    ).hexdigest()
    db = conn_dict.get('db', '')
    user = conn_dict['user']
    password = conn_dict.get('passwd', '')
    host = conn_dict['host']
    port = conn_dict.get('port')
    return connector.connect(
        pool_name=conn_dict_key, pool_size=3, user=user, password=password,
...
def get_sql_result(sql, conn_dict):
    connection = connect_to_db(conn_dict)
