...
def connect_to_db(conn_dict, force_reconnect=False):
    if not hasattr(connect_to_db, 'connection'):
    ...
    if not connection or force_reconnect:
        db = conn_dict.get('db', '')
        user = conn_dict['user']
...
def get_sql_result(sql, conn_dict, retry=True):
    try:
        ...
        if retry:
            ...
            db = connect_to_db(conn_dict, force_reconnect=True)
            execute_sql(sql, conn_dict, retry=False)
        else:
            raise


def execute_sql(sql, conn_dict, params=None, retry=True):
    try:
        db = connect_to_db(conn_dict)
        cur = db.cursor()
