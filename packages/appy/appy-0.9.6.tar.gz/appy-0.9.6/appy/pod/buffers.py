...
EMPTY_STATEMENT_GROUP = 'This note is empty. It must at least contain a Pod ' \
...
class MemoryBuffer(Buffer):
    ...
    def createPodAction(self, actionType, statementName, subExpr, podElem,
                        minus, main=True):
        '''Creates an Action instance, depending on p_actionType'''
        if actionType == 'if':
            r = IfAction(statementName, self, subExpr, podElem, minus)
            if main:
            ...
            r = ElseAction(statementName, self, None, podElem, minus,
                           linkedIfAction)
        elif actionType == 'for':
            ...
            r = ForAction(statementName, self, subExpr, podElem, minus, iter)
        elif actionType == 'with':
            variables = self._getVariables(subExpr)
            r = VariablesAction(statementName, self, podElem, minus, variables)
        elif actionType == 'meta-if':
            r = NullAction(statementName, self, None, podElem, None)
        else: # Null action
            r = NullAction(statementName, self, None, podElem, None)
        return r

    def createPodActions(self, statementGroup):
        '''Tries to create action(s) based on p_statementGroup. If the statement
        ...
        try:
            ...
            if not statementGroup: raise ParsingError(EMPTY_STATEMENT_GROUP)
            # Get the main statement (starting with "do...") and check it
            statement = statementGroup[0]
            aRes = self.Rex.action.match(statement)
            if not aRes:
            ...
            for statement in statementGroup[1:]:
                ...
                # Get any secondary statement
                    ...
                    last = self.createPodAction(actionType, '', subExpr,
                                                podElem, None, main=False)
    ...
    def createPxAction(self, elem, actionType, statement):
        ...
        if actionType == 'for':
            ...
            action= IfAction('if', self, statement, elem, False)
