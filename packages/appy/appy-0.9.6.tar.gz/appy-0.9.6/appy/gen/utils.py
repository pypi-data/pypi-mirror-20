...
def readCookie(request, onResponse=False):
    ...
    res = cookieValue.split(':')
    if len(res) == 2:
        # This way, we are compliant with the format preceding auth contexts
        res.append(None)
    return res
...
class Collapsible:
    ...
    px = Px('''
     <img id=":'%s_img' % collapse.id" align=":dleft" class=":collapse.css"
          var="icons=collapse.icons"
          onclick=":'toggleCookie(%s,%s,%s,%s,%s)' % \
                   (q(collapse.id), q(collapse.display), q(collapse.default), \
                    q(icons.expand), q(icons.collapse))"
          src=":collapse.expanded and \
                url(icons.collapse) or url(icons.expand)"/>''')
