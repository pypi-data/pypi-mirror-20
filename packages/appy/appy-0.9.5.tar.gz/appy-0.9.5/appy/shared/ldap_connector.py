...
class LdapConfig:
    ...
    def __init__(self):
        ...
        # If "authenticate with DN" is True, instead of authenticating to the
        # server with "adminLogin" or with the login as encoded by the user, we
        # authenticate users with their complete DN (Distinguished Name)
        # computed like this:
        #              [loginAttribute]=[login],[baseDn]
        #   * "loginAttribute" and "baseDn" are the attributes defined
        #     hereabove;
        #   * "login" is the login used: either attribute "adminLogin"
        #     hereabove, or the login as encoded by the user in the UI when
        #     logging into the Appy app.
        self.authenticateWithDN = False
    ...
    def getConnectLogin(self, login):
        '''Get the login to use for logging in to the LDAP server'''
        if not self.authenticateWithDN: return login
        # Use the full Distinguished Name
        return '%s=%s,%s' % (self.loginAttribute, login, self.baseDn)

    def test(self, tool):
        '''Connects to the LDAP server to check the connection'''
        connector = LdapConnector(self.getServerUri(), tool=tool)
        login = self.getConnectLogin(self.adminLogin)
        success, msg = connector.connect(login, self.adminPassword)
    ...
    def getUser(self, tool, login, password):
        ...
        login = self.getConnectLogin(data[0][0])
        success, msg = userConnector.connect(login, password)
        if not success: return
