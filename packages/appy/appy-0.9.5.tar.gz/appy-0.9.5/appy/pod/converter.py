...
class Converter:
    ...
    def updateOdtDocument(self):
        ...
        '''
        # 1) Update all indexes
        for index in LoIter(self.doc.getDocumentIndexes()): index.update()
        # 2) Update sections
        self.doc.updateLinks()
        for section in LoIter(self.doc.getTextSections(), reverse=True):
...
class ConverterScript:
    ...
    def run(self):
        ...
        converter = Converter(args[0], args[1],
                              options.port, options.template, optimal)
        try:
            converter.run()
