...
class ToolMixin(BaseMixin):
    ...
    def getLinksTargetInfo(self, klass, back=None):
        ...
        if hasattr(klass, 'popup'):
            res = Object(target='appyIFrame')
            d = klass.popup
            if isinstance(d, basestring):
                params = d[:-2] # Width only
            else:
                params = "%s, %s" % (d[0][:-2], d[1][:-2]) # Width and height
    ...
    def getPortletCollapse(self):
        '''Gets a Collapsible instance for showing/hiding the portlet'''
        return gutils.Collapsible('appyPortlet', self.REQUEST,
                                  default='expanded', icons='showHide')
# ------------------------------------------------------------------------------
