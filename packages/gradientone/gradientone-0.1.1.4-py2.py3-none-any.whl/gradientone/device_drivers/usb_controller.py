...
usb_logger = gateway_helpers.get_logger(log_filename='usb_controller_logs.log')
...
class UsbController(object):
    ...
    def ask(self, instr=None, command=""):
        usb_logger.info("USB ask: %s" % command)
        if not instr:
            instr = self.instr
...
class RawUsbController(object):
    ...
    def write(self, ep=0x02, command='\x14\x05'):
        ...
        usb_logger.info("USB write: %s" % command)
        if not self.device:
            usb_logger.warning("missing device")
            return
