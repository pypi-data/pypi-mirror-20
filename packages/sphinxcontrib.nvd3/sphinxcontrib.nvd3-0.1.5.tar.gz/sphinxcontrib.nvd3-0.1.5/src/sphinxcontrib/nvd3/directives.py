...
class NVD3DirectiveBase(rst.Directive):
    ...
    def run(self):
        ...
        # workaround
        if not self.options.get('focus_enable', False):
            if 'x2Axis' in chart.axislist:
                del chart.axislist['x2Axis']
        ...
        # workaround
        text = text.replace('focusEnable(true)', 'focusEnable = true')
        text = text.replace('focusEnable(false)', 'focusEnable = false')
