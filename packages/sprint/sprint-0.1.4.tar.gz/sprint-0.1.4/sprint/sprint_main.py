...
def main(): 
    ...
    def help_doc():
        ...
        print "         -c       INT              # Remove the fist INT bp of each read (default is 6)"
        print "         -p       INT              # Mapping CPU (default is 1)"
        print "         -cd      INT              # Cluster distance (default is 200)"
    ...
    if 1==1: 
       ...
       # subprocess.Popen('cp '+tmp+'/regular.res '+output+'/SPRINT_identified_regular.res',shell=True).wait()
        ...
        subprocess.Popen('echo "#Chrom\tStart(0base)\tEnd(1base)\tType\tThe_number_of_high_quality_reads_supporting_this_RES\tStrand\tAD:DP" | cat - '+tmp +'/regular.res.depth   > '+output+'/SPRINT_identified_regular.res',shell=True).wait()
       # subprocess.Popen('cp '+tmp+'/hyper.res '+output+'/SPRINT_identified_hyper.res',shell=True).wait()
        ...
        subprocess.Popen('echo "#Chrom\tStart(0base)\tEnd(1base)\tType\tThe_number_of_high_quality_reads_supporting_this_RES\tStrand\tAD:DP" | cat - '+tmp +'/hyper.res.depth   > '+output+'/SPRINT_identified_hyper.res',shell=True).wait()
