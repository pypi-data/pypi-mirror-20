...
class HierarchicalMapping(collections.MutableMapping, metaclass=abc.ABCMeta): 
    ...
    def __getitem__(self, key): 
        if not isinstance(key, str): 
            raise KeyError("{} is not a filepath-like string.".format(key))
        if '/' not in key: 
    ...
    def __setitem__(self, key, value): 
        ...
        if '/' not in key: 
            ...
            if _key =='': 
                raise KeyError("Cannot set unnamed file '' in {}".format(folder))
    ...
    def __delitem__(self, key): 
        ...
        if '/' not in key: 
            ...
            if _key == '': 
                del self._dict[folder]
            else:
                del self._dict[folder][_key]
                if len(self._dict[folder]) == 0: 
                    del self._dict[folder]
