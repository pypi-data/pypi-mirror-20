...
def create_order(
        cart, email=None, phone=None, shipping_method=None, payment_method=None, note=None,
...
def get_shipping_method(name):
    for m in settings.PCART_SHIPPING_METHODS:
        if m['name'] == name or name is None:
            ...
            return _shipping_method_class(config=m['config'])
...
def get_payment_method(name):
    for m in settings.PCART_PAYMENT_METHODS:
        if m['name'] == name or name is None:
            ...
            return _payment_method_class(config=m['config'])
