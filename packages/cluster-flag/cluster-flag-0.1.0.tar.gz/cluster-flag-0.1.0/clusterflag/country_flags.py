...
def cross_flag(npoints=[100, 100, 100, 100, 100], cenx=0.5, ceny=0.5,
    ...
    small_rects_x = [np.random.uniform(0, 1, k) for k in npoints[:4]] * \
                     np.array([[cenx - shift_rectx/2],[cenx - shift_rectx/2],
                               [1 - (cenx + shift_rectx/2)],
                                [1 - (cenx + shift_rectx/2)]]) + \
                               np.array([[0], [0], [cenx + shift_rectx/2],
                                         [cenx + shift_rectx/2]])
    small_rects_y = [np.random.uniform(0, 1, k) for k in npoints[:4]] * \
                     np.array([[1 - (ceny + recty/2)],[ceny - recty/2],
                             [1 - (ceny + recty/2)],[ceny - recty/2]]) + \
                                np.array([[ceny + recty/2], [0],
                                          [ceny + recty/2], [0]])
    output = np.transpose(np.vstack((np.concatenate(small_rects_x)*ratio,
...
def crescent_flag(npoints=[100,100,100,100,100], bcx=0.5, bcy=0.5, 
   ...
   # middle_part[:,0] = middle_part[:,0]*ratio
