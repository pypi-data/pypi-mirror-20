...
examples:
  shadowproxy -v socks://:8527=ssr://aes-256-cfb:password@127.0.0.1:8888                     # socks5 --> shadowsocks
  shadowproxy -v http://:8527=ssr://aes-256-cfb:password@127.0.0.1:8888                      # http   --> shadowsocks
  shadowproxy -v red://:12345=ssr://aes-256-cfb:password@127.0.0.1:8888                      # redir  --> shadowsocks
  ...
  shadowproxy -v tunneludp://:8527#8.8.8.8:53=ssrudp://aes-256-cfb:password@127.0.0.1:8888   # tunnel --> shadowsocks (udp)
  sudo shadowproxy -v tproxyudp://:8527=ssrudp://aes-256-cfb:password@127.0.0.1:8888         # tproxy --> shadowsocks (udp)
'''
...
class ServerBase:
    ...
    async def get_remote_stream(self, remote_conn):
        if self.via_client:
            ...
            try:
                await self.via_client.client_init(remote_stream, self.taddr)
            except Exception:
...
class SSClient:
    ...
    async def client_init(self, stream, taddr):
        await stream._stream.write(stream.encrypter.iv)
        await stream.write(pack_addr(taddr))
...
class HTTPClient:
    ...
    async def client_init(self, stream, taddr):
        headers_str = f'CONNECT {taddr[0]}:{taddr[1]} HTTP/1.1\r\n'
        if self.auth:
            headers_str += 'Proxy-Authentication: Basic {}'.format(
                    base64.b64encode(self.auth[0] + b':' + self.auth[1]).decode())
        await stream.write(headers_str.encode())
        await read_until(stream, b'\r\n\r\n')
