...
class SocksConnection(ServerBase):
    def __init__(self, server_auth=None, via=None):
    ...
    async def interact(self):
        ...
        if self.server_auth and b'\x02' not in methods:
        ...
        if self.server_auth:
            ...
            if (username, password_length) != self.server_auth:
...
class HTTPConnection(ServerBase):
    ...
    async def interact(self):
        ...
        if self.server_auth:
            ...
            httpauth = 'Basic ' + base64.b64encode(b':'.join(self.server_auth)).decode()
            if httpauth != pauth:
...
def uri_compile(uri):
    ...
    if cipher:
        ...
        if url.scheme.startswith('ss'):
            ...
            kw['server_auth'] = (cipher_cls.encode(), password.encode())
        else:
            pass
    if loc:
