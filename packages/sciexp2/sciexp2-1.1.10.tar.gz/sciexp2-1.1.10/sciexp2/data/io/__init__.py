...
def find_files(expression, *filters):
    ...
    For convenience, the elements in *filters* can also be lists of filters.

    The results of multiple calls to `find_files` can be aggregated by
    extending the contents of an `~sciexp2.common.instance.InstanceGroup`
    object.

    You can also set the ``FILE`` variable to a `file`, `StringIO` or any other
    `io.IOBase` subclass instance by either modifying the result of
    `find_files` or by creating an `~sciexp2.common.instance.InstanceGroup` by
    hand.
...
def _probe_fsv_file(fobj, header, delimiter, genfromtxt_args, info=None):
    ...
    with _source.current_position(fobj):
        data = numpy.genfromtxt(fobj, delimiter=delimiter,
                                dtype=None,
                                names=header,
    ...
    default_contents = six.StringIO(("," * len(names)) + "1")
...
def extract_func(source, func, *args, **kwargs):
    ...
    def _extract_func_postprocess(data, source, v2f):
        ...
        for i, ldata in enumerate(data):
            ...
            if len(f2v) > 0:
                for name in f2v:
                    variables.append((name, values[f2v_dict[name]]))
    ...
    for var in v2f:
        ...
        if numpy.issubdtype(tmp.dtype, str):
            vdtype = STRING_DTYPE
        else:
            vdtype = tmp.dtype
...
def extract_txt(source, *fields, **kwargs):
    ...
    * Regular expression to match on each field name.
...
def _extract_txt_probe(source, fields, delimiter, header, genfromtxt_args,
    ...
    info = _probe_fsv_file(source, header=header, delimiter=delimiter,
                           genfromtxt_args=genfromtxt_args, info=info)
    ...
    # make strings long enough
    for i in info:
        if issubclass(i.type, six.string_types):
            i.type = STRING_DTYPE
...
def _extract_txt_file(source, delimiter, x_usecols, x_converters, x_dtype,
                      genfromtxt_args):
    data = numpy.genfromtxt(source, delimiter=delimiter,
                            usecols=x_usecols,
...
def extract_regex(source, *fields, **kwargs):
    ...
    * Regular expression to match on the source contents.

      Variable references in the regular expression will be used to extract a
      value into a field named as the variable.

      In the case of complex patterns, you can use named regular expression
      groups instead of variable references; i.e., ``@var@`` is a shorthand for
      ``(?P<var>.+)``.
...
def _extract_regex_probe(source, regexes, converters, multi_line, state):
    ...
    info = _probe_fsv_file(six.StringIO(contents),
                           header=True,
                           delimiter=",",
                           genfromtxt_args={})
    ...
    if len(data_contents) == 0:
        data_contents = ","
    info = _probe_fsv_file(six.StringIO(data_contents),
                           header=None,
                           delimiter=",",
                           genfromtxt_args={},
                           info=info)
    ...
    state["x_dtype"] = [(i.name, i.type) for i in info]
