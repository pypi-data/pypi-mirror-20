...
class Settings:
    ...
    def ___init__(self, host='localhost', port=28015, db='test', user='admin', password='', auth_key='', ssl=None):
        if not ssl:
            ssl = {}
...
class RethinkDB:
    ...
    async def drop_and_remake(self, *tables):
        ...
        if isinstance(tables, dict):
            table_creators = [r.table_create(table_name, options)
                              for table_name, options in tables.items()]
        else:
            table_creators = [r.table_create(table_name)
                              for table_name in tables]

        await asyncio.wait(table_creators)
