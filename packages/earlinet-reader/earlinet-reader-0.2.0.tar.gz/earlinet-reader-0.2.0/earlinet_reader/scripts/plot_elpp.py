...
def plot_all_variables(args, p):
    ...
    with warnings.catch_warnings():  # Catch warning "Converting masked element to nan"
        ...
        p.plot_data(r_min=args.rmin, r_max=args.rmax, grid=args.grid, errorevery=args.errorevery)
...
def plot_single_variable(args, p):
    ...
    with warnings.catch_warnings():  # Catch warning "Converting masked element to nan"
        warnings.simplefilter("ignore")
        p.plot_data_variable(args.variable, r_min=args.rmin, r_max=args.rmax, grid=args.grid, errorevery=args.errorevery)
...
def create_html_single_file(file_path, args):
    ...
    for variable_name in p.data_variables:
        ...
        with warnings.catch_warnings():  # Catch warning "Converting masked element to nan"
            warnings.simplefilter("ignore")
            p.plot_data_variable(variable_name, r_min=args.rmin, r_max=args.rmax, grid=args.grid, errorevery=args.errorevery, add_suptitle=False)
