...
class PreprocessedFile:
    ...
    def plot_data(self, r_min=0, r_max=20, errorevery=3, fig_width=9, ax_height=4, fig_height=None, grid=False):
        ...
        for n, variable_name in enumerate(self.data_variables):
            ax = plt.subplot(no_variables, 1, n + 1)

            legend = (n == 0)  # Show legend only on first plot

            self.draw_data_variable(ax, variable_name, r_min=r_min, r_max=r_max, errorevery=errorevery, grid=grid,
    ...
    def plot_data_variable(self, variable_name, r_min=0, r_max=20, errorevery=3, figsize=(9, 5), custom_title=None,
                           custom_xlabel=None, legend=True, grid=False):

        fig = plt.figure(figsize=figsize)
        ax = plt.subplot(111)
        self.draw_data_variable(ax, variable_name, r_min=r_min, r_max=r_max, errorevery=errorevery,
                                custom_title=custom_title, custom_xlabel=custom_xlabel, legend=legend, grid=grid)
        plt.tight_layout()
        plt.show()

        return fig

    def draw_data_variable(self, ax, variable_name, r_min=0, r_max=20, errorevery=3, custom_title=None,
                           custom_xlabel=None, legend=True, grid=False):
