...
class EdpClient(object):
    ...
    def list_populations(self):  # type: () -> List[Dict[str, Any]]
        """Lists all visible populations."""
        url = self.config.edp_url + '/rpc/population'
        resp = self._session.get(url)
        _raise_for_error(resp)
        return resp.json()

    def list_population_models(self):  # type: () -> List[Dict[str, Any]]
        """Lists all visible population models."""
        url = self.config.edp_url + '/rpc/population_model'
        resp = self._session.get(url)
        _raise_for_error(resp)
        return resp.json()
