...
def collection_view(request, slug, view='default'):
    ...
    nonfiltered_vendor_counters = {
        ...
        for x in Product.objects.filter(
            ...
            published=True,
            status_id__in=visible_status_ids,
            variants_count=0).values('vendor__title').annotate(Count('id')).order_by()}
    nonfiltered_v_vendor_counters = {
        ...
        for x in ProductVariant.objects.filter(
            ...
            status_id__in=visible_status_ids
            ).values('product__vendor__title').annotate(Count('id')).order_by()}
    ...
    _nonfiltered_tagset = list(
        Product.objects.filter(
            ...
            published=True,
            status_id__in=visible_status_ids,
            variants_count=0).values_list('tags', flat=True))
    ...
    for f in properties_for_filters:
        ...
        _kwargs = {
            'product__collections': collection,
            'product__published': True,
            'product__status_id__in': visible_status_ids,
        }
        if vendors:
    ...
    if _filtered:
        ...
        pa_query = Product.objects.filter(
            tag_expr_without_prices,
            collections=collection,
            published=True,
            status_id__in=visible_status_ids,
            variants_count=0)
