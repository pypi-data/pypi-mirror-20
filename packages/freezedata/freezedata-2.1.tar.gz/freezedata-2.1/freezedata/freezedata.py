...
def freeze_data(obj, *, allow=frozenset()):
    ...
    - modules (if allowed) will unchanged. If root object s a module, it will be converted to
      a namedtuple
    - classes and other instances of 'type' (if allowed) are converted to namedtuples
    - namedtuples are created anew and values recursively made read-only

    If a function is found in 'obj', a ValueError is raised, as functions are not read-only
    (e.g. the function's .__globals__ will be accessible). However, functions are often useful
    to retain in data structures, so if sting 'functions' is found in 'allow',
    ...
    However, if a string 'classes' is in 'allow', these objects will be converted to
    namedtuple instances.

    class instances will raise ValueError by default. if it's class is set in keep_instances_of,
    teh instance will be converted to a namedtuple instances.
...
def freeze_data_inner(obj, *, allow=frozenset()):
    ...
    if isinstance(obj, (str, int, float, bytes, type(None), bool)):
        ...
        return type(obj)(
            *(freeze_data(i, allow=allow) for i in obj))
    elif isinstance(obj, (tuple, list)):
        return tuple(freeze_data(i, allow=allow) for i in obj)
    elif isinstance(obj, (dict, types.MappingProxyType)):
        return types.MappingProxyType(
            {k: freeze_data(v, allow=allow) for k, v in
             obj.items()})
        ...
        for i in (j for j in dir(obj) if not j.startswith('__')):
            val = getattr(obj, i)
            setattr(func, i, freeze_data(val, allow=allow))
        return func
