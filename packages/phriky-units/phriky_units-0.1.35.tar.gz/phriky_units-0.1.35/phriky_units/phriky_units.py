...
def main(
    ...
    parser = argparse.ArgumentParser(description='Apply physical unit inconsistency detection to C++ files for ROS.',
                epilog='Copyright 2017 REDACTED')
    ...
    if not os.path.exists(target_cpp_file + '.dump' ) or not args.use_existing_dump_file:
        ...
        # TEST IF DIRECTORY IS NOT NOTHING -- THIS CATCHES THE CASE WHEN WE'RE ALREADY IN THE RIGHT DIRECTORY
        if target_cpp_file_dir:
        ...
        if not os.path.exists(dump_filename):
            args_cppcheck = ['cppcheck', '--dump', '-I ../include', '--suppress="*"', '-q', target_cpp_file_base_name]
            cppcheck_process = Popen(' '.join(args_cppcheck),  shell=True)
