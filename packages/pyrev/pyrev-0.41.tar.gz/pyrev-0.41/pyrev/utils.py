...
def copy_document(source, dest, logger, **kwargs):
    ...
    '''
    logger.debug('Start running copy_chapter "{}" -> "{}"'
                 .format(source, dest))
    ...
    if os.path.isdir(dest):
        ...
        if not dest_filename.endswith('.re'):
            logger.error('Filename "{}" looks inappropriate. Maybe "{}.re"?'
    ...
    if os.path.samefile(source_dir, dest_dir):
        ...
        if source_filename == dest_filename:
            logger.error('src and dst point to a same file ({}).'
    ...
    if not source_project:
        logger.error('Failed to instanciate Re:VIEW Project ({}).'
                     .format(source_dir))
        return 1
    if not dest_project:
        logger.error('Failed to instanciate Re:VIEW Project ({}).'
