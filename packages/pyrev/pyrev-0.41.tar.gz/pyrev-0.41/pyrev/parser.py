    ...
    def parse_ch(self, ch, pos, logger=None):
        ...
        if self.state == ISM_NONE:
            ...
            if ch == '{':
                ...
                self._error('Wrong charactor at C{} ("{{" != "{}")'
                            .format(pos, ch))
            ...
            if ch in self._inline_escape_allowed:
                ...
                self._info(('Backslash inside inline "{}" is'
                            ' not effective toward "{}".')
                           .format(self.name, ch))
                self.unprocessed.append('\\')
                self.unprocessed.append(ch)
                self.state = ISM_INLINE_CONTENT
...
class BlockStateMachine(object):
    ...
    def parse_line(self, line_num, line, logger=None):
        ...
        if self.state == BSM_NONE:
            if m_end:
                ...
                ret = self._parse_block_start(line_num, line, content,
                                              prefix_len, logger)
                # BSN_NONE ... //footnote[][]
                # BSN_IN_BLOCK ... //list[][]{
    ...
    def _parse_block_start(self, line_num, line, content, 
            ...
            if self.state == BSM_PARSE_NAME:
                if ch == '[':
                    ...
                    self._error(line_num,
                                'Invalid param end at C{}'.format(pos),
                                line,)
...
class Parser(object):
    ...
    def __init__(self,
        ...
        def __block_exist(inline, block_names):
            ...
            self._error(inline.line_num,
                        'Inline for id "{}" found but no block for it.'
                        .format(inline_id),
                        None)
        ...
        def __image_file_exist(block):
            ...
            if not imgs:
                self._error(block.line_num,
                            'Image file for image "{}" does not exist'
                            .format(image_id),
                            block.lines)
                return
    ...
    def parse_file(self, path, base_level, source_name, logger=None):
        ...
        f = None
        try:
            f = file(path, encoding='utf-8-sig')
    ...
    def _parse_file_inter(self, f, base_level, source_name, logger=None):
        ...
        if self.bsm.state != BlockStateMachine.BSM_NONE:
            self._error(None,
                        'Block "{}" is not ended'.format(self.bsm.name),
                        None)
    ...
    def _parse_line(self, line_num, line, logger=None):
        ...
        if self.bsm.state == BSM_IN_BLOCK:
            ...
            # it in the block
            if rstripped[:3] == '#@#':
                self._info(line_num, line,
                           ('Re:VIEW comment in block "{}".'
            ...
            if self._handle_chap(line_num, line):
                ...
                if rstripped[:3] == '#@#':  # Comment
                    ...
                    self._warning(line_num,
                                  ('Unordered list operator ("*") without'
                                   ' a single space'),
                                  line)
    ...
    def _block_firstline_check(self, block):
        ...
        if not block_checkers:
            self._error(block.line_num,
                        'Undefined block "{}" found'.format(block.name),
                        block.lines)
