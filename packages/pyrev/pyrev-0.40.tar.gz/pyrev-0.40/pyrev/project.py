...
def _verify_filename(source_dir, filename, logger=None):
    ...
    if source_dir not in abs_path:
        logger.info('"{}" does not point to file in dir "{}".'
                    .format(filename, source_dir))
        return None
    if not os.path.exists(abs_path):
        logger.info('"{}" does not exist in "{}".'
                    .format(filename, source_dir))
        return None
...
class ProjectImage(object):
    ...
    def __str__(self):
        return '{} (parent: {})'.format(self.rel_path,
...
class ReVIEWProject(object):
    ...
    def init(self,
        ...
        if config_file:
            logger.debug('config_file is specified ("{}"). Try pasing it.'
                         .format(config_file))
        ...
        if catalog_file:
            logger.debug('catalog_file is specified ("{}"). Try parsing it.'
                         .format(catalog_file))
        ...
        if (self.parts is None) and (self.chaps is None):
            self.logger.warn('Failed to recognize book structure in {}.'
        ...
        if os.path.isdir(self.image_dir_path):
            ...
            self.logger.info('"{}"({}) is not a directory'
    ...
    def _find_and_parse_config_file(self, logger=None):
        ...
        for candidate in candidates:
            if self._try_parse_config_file(candidate):
                logger.debug('"{}" is used as Re:VIEW config file.'
                             .format(candidate))
                return True
        return False
    ...
    def _try_parse_config_file(self, candidate, logger=None):
        ...
        try:
            ...
            logger.info('Error during parsing {}: {}'.format(candidate, e))
        return False
            ...
            if yaml_data.get('PREDEF'):
                for filename in [x.strip() for x in yaml_data['PREDEF']]:
                    if not _is_appropriate_file(self.source_dir, filename):
                        logger.info(('Ignoring "{}" because the file looks'
                                     ' inappropriate'
                                     ' (not available, invalid, etc')
                                    .format(filename))
                        continue
    ...
    def parse_single_source_file(self, filename, base_level):
        ...
        f = file(os.path.normpath(
                '{}/{}'.format(self.source_dir, filename)))
        chap_index = 0
        for line in f:
            # BOM matters. Force ignore it..
            line = str(line, 'utf-8-sig').rstrip()
            m = r_chap.match(line)
            if m:
    ...
    def _recognize_image_files(self):
        if not os.path.isdir(self.image_dir_path):
            self.logger.debug('No image_dir ("{}")'
