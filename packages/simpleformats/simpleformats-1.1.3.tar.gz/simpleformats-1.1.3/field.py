...
def parse_int( field, required = True, nonzero = False, positive = False, **kwargs ):
    ...
    try:
        ...
        ctxlogger.exception(
            ParseException,
            'Value "{}" is not an integer'.format(field),
            orig_exc = e
        )
...
def parse_decimal( field, required = True, strict = True, min_precision = None, max_precision = None, **kwargs ):
    ...
    try:
        ...
        ctxlogger.exception(
            ParseException,
            'Could not parse "{}" as a decimal'.format( field ),
            orig_exc = e
        )
...
def parse_boolean( field, required = True, positive = '1', negative = '0', ignore_case = True, **kwargs ):
    """
    ...
    if required:
        ctxlogger.exception(
            ParseException, 'Empty string for required value'
        )
    return False
...
def unparse_string( value, length = None, justify = 'right', filler = ' ', match = None, **kwargs ):
    """
    ...
    try:
        ...
        ctxlogger.exception(
            ParseException,
            'Invalid fill character "{}"'.format(filler),
            orig_exc = e
        )
