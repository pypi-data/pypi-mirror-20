...
def main_gcode(current_layer, filament_width,x2, y, n, layer_height, base_case):
    """This methods prints the gcode for each layer
    """
    increment_size = 1/n    
    rev_per_mm = (10 * (72.0/26.0))   
    ...
    if (current_layer %2  == 1):
        commands.append("G0 X{:.5f} Y{:.5f}".format(0, rev_per_mm*0))
    else:
         commands.append("G0 X{:.5f} Y{:.5f}".format(x2, rev_per_mm*y))

    if (base_case and (current_layer+1) % 2 == 0):
        commands.append("G1 X{:.5f} Y{:.5f}".format(x2, rev_per_mm*y))

    if (base_case and (current_layer+1) % 2 == 1):
        commands.append("G1 X{:.5f} Y{:.5f}".format(0, rev_per_mm*0))

    while (a/n < 1-increment_size and (current_layer+1) % 2 == 0):
        print "in forward loop "
        #print "start point", a
        #print "winding ratio", (a/n)
        commands.append("G1 X{:.5f} Y{:.5f}".format(x2, rev_per_mm*(y+(a/n))))
        a += 1
        #print "start point", a
        #print "winding ratio", (a/n)
        commands.append("G1 Y{:.5f}".format(rev_per_mm*(y+(a/n))))
        commands.append("G1 X{:.5f} Y{:.5f}".format(0, rev_per_mm*(a/n)))
        a += 1
        #print "start point", a
        #print "winding ratio", (a/n)
        commands.append("G1 Y{:.5f}".format(rev_per_mm*(a/n)))
...
def tpg(axial_travel, filament_width_og, printbed_diameter, final_diameter, helix_angle, smear_factor, feedrate):
    ...
    commands.append(";feedrate={}".format(feedrate))
    ...
    while (current_layer <= layers):
        ...
        main_gcode(current_layer, filament_width, x2, y, n, filament_width_og * smear_factor, base_case)
        current_layer += 1
