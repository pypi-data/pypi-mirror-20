...
def run():
    ...
    for filename in files:
        if filename == "-":
            ...
            fp = open(filename, "r")
            # Get tree count if we're doing burnin
            if options.burnin:
                N = 0
                for line in fp:
                    # Skip blank lines
                    ...
                    if (
                        ...
                        N += 1
                fp.seek(0)
                print("Peeked at file, found %d trees" % N)
                burnin = int(round((options.burnin/100.0)*N))
                print("Set burnin to %d" % burnin)

        can_be_fast = (not options.burnin) or filename != "-"
        tree_strings = []
        firstline = True
        for n, line in enumerate(fp):
            ...
            if (
                ...
                if can_be_fast:
                    if n > burnin and (n-burnin)%options.subsample == 0:
                        # HAndle
