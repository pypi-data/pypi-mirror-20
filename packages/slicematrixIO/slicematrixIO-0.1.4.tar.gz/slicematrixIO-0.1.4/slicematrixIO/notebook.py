...
class GraphEngine():
	...
	def drawNetworkGraph(self, network_model, color_map = "RdBuGn", graph_style = "light", graph_layout = "force", width = 1000, height = 600, charge = -100, color_axis = "closeness_centrality", label_color = "#000", label_shadow_color = "#fffff0", min_node_size = 5):
		...
		init_js = "$.data(element, 'graph_id', {});".format(self.graph_count);
		...
		var width = window.graph_data[$.data(element, "graph_id")]['width'];
		var height = window.graph_data[$.data(element, "graph_id")]['height'];

		var label_color = window.graph_data[$.data(element, "graph_id")]['label_color'];
		
		var min_node_size = window.graph_data[$.data(element, "graph_id")]['min_node_size'];
		
		var charge = window.graph_data[$.data(element, "graph_id")]['charge'];

		var colorAxis = window.graph_data[$.data(element, "graph_id")]['color_axis'];
		if(colorAxis == undefined){
			colorAxis = "closeness_centrality";
		}

		var colorMap = window.graph_data[$.data(element, "graph_id")]['color_map'];
		if(colorMap == undefined){
			colorMap = "RdBuGn";
		}

		var graphStyle = window.graph_data[$.data(element, "graph_id")]['graph_style'];
		...
		var graphLayout = window.graph_data[$.data(element, "graph_id")]['graph_layout'];
		if(graphLayout == undefined && window.graph_data[$.data(element, "graph_id")]['got_embed'] == true){
			graphLayout = "embedding";
		}

		require(['d3'], function(d3){
			var graph_id = $.data(element, "graph_id");
			...
			var data = window.graph_data[$.data(element, "graph_id")];
			...
			var canvas = document.querySelector("#mainEvent_" + $.data(element, "graph_id")),
			...
			window.graph_data[$.data(element, "graph_id")]['stats'].forEach(function(row, j){
				//console.log(row);
				var val1 = row[0];
				if(val1 > max_rank){
					max_rank = val1;
			...
			if(graphLayout == "force"){
				d3.select("#spinner_" + $.data(element, "graph_id")).style("visibility", "hidden");
				drawGraph();
				...
				window.graph_data[$.data(element, "graph_id")]['embedding'].forEach(function(row, j){
		...
		"""
		self.graph_count += 1
		return Javascript(init_js + config + javascript + js_code)
