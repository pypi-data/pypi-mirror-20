...
class MockAddon:
    ...
    def on_message(ws, message):
        try:
            ...
            if msg_obj.get(commands.KEY_NAME) == commands.VALUE_START:
                ...
                ws.send(json.dumps(ret_obj))
            elif msg_obj.get(commands.KEY_NAME) == commands.VALUE_WAIT_LINK_FINISH:
                if MockAddon.status_page_opened:
                    ret_obj = {
                        'data': 'okay',
                        'command': 0
                    }
                    ...
                    ret_obj = {
                        'data': 'Profiling page was not opened.',
                        'command': 1
                    }
                ws.send(json.dumps(ret_obj))
            else:
    ...
    def reset_addon(self):
        # reset status
        MockAddon.status_page_opened = False
