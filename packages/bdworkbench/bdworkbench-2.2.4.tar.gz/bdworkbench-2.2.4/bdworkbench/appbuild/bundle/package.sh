...
CATALOG_NAME=bdcatalog-${OS}-${SANITIZED_DISTRO}-${VERSION}

CATALOG_BASE_DIR=${CATALOG_STAGING_DIR}/${CATALOG_NAME}
mkdir -p ${CATALOG_BASE_DIR}
...
if ! $(isUrlOrUndef ${IMAGE}); then
    ...
    cp -f ${IMAGE} ${DEST_IMAGE_DIR}/${IMAGE_FILENAME}
else
...
DEST_DECOMP=${STAGING_DIR}/decompress-${OS}-${SANITIZED_DISTRO}.sh
...
sed -i "s|@@@NAME@@@|${NAME}|" ${DEST_DECOMP}
sed -i "s|@@@DISTRO@@@|${DISTRO}|" ${DEST_DECOMP} ## Not the sanitized version.
sed -i "s|@@@VERSION@@@|${VERSION}|" ${DEST_DECOMP}
sed -i "s|@@@DESCRIPTION@@@|${DESCRIPTION}|" ${DEST_DECOMP}
sed -i "s|@@@INDEPENDET@@@|${INDEPENDENT}|" ${DEST_DECOMP}
sed -i "s|@@@CONFIGAPIVER@@@|${CONFIG_API_VERSION}|" ${DEST_DECOMP}
sed -i "s|@@@CATALOGAPIVER@@@|${CATALOG_API_VERSION}|" ${DEST_DECOMP}
sed -i "s|@@@SOURCEPACKAGE@@@|${SRC_TARFILE}|" ${DEST_DECOMP}
