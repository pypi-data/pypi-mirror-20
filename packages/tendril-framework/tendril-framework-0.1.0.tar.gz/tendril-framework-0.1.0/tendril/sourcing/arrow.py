...
>>> arrow.avobj.search_vpnos('IC SMD W5300 LQFP100-14')
(['1278-1009-ND'], 'EXACT_MATCH_FFP')

>>> arrow.avobj.search_vpnos('IC SMD LM311 8-TSSOP')
...
>>> arrow.avobj.search_vpnos('CAP CER SMD 22uF/35V 0805')
(['445-14428-2-ND', '445-14428-1-ND', '445-11527-2-ND', '445-11527-1-ND'],
 'CONSENSUS_FP_MATCH')

>>> arrow.avobj.search_vpnos('RES SMD 1.15E/0.125W 0805')
(['311-1.15CRTR-ND', '311-1.15CRCT-ND', '541-1.15CCTR-ND', '541-1.15CCCT-ND'],
'CONSENSUS_FP_MATCH')

>>> arrow.avobj.search_vpnos('RES ARRAY SMD 102E/0.0625W 1206-4')
(['TC164-FR-07102RL-ND', 'AF164-FR-07102RL-ND'], 'NAIVE_FP_MATCH_ALLOW_NS')

>>> arrow.avobj.search_vpnos('CRYSTAL AT 13MHz HC49')
(['887-2036-ND'], 'EXACT_MATCH')


.. seealso::
    Implementation of :func:`VendorArrow._get_pas_vpnos` and the
...
.. seealso::
    :func:`VendorArrow._search_preprocess`
    :func:`VendorArrow._get_device_catstrings`
    :func:`VendorArrow._get_device_subcatstrings`
    :func:`VendorArrow._get_device_subcatstrings`
...
Obtain information for a given Arrow part number :

>>> p = Arrow.ArrowElnPart('800-2146-ND')
>>> p.manufacturer
...
class ArrowElnPart(VendorElnPartBase):
    def __init__(self, arrowpartno, ident=None, vendor=None, max_age=-1):
        """
        This class acquires and contains information about a single Arrow
        part number, specified by the `arrowpartno` parameter.

        :param arrowpartno: The Arrow part number.
        :type arrowpartno: str
        ...
        :param vendor: ``(Optional)`` Allows the caller to 'bind' the part
                       ...
                       for use with the Tendril database.
        :type vendor: :class:`VendorArrow`

        """
        if vendor is None:
            vendor = avobj
        if not arrowpartno:
            logger.error("Not enough information to create an Arrow Part")
        super(ArrowElnPart, self).__init__(arrowpartno, ident, vendor, max_age)
    ...
    def vparturl(self):
        if self._vparturl:
            ...
            return 'https://www.arrow.com/en/products/' + self.vpno

    def _get_data(self):
        """
        This function downloads the part page from Arrow, scrapes the page,
        ...
        try:
            ...
            if e.code == 404:
                logger.error("Got 404 opening Arrow product page : "
                             "" + self.vpno)
...
class ArrowInvoice(customs.CustomsInvoice):
    def __init__(self, vendor=None, inv_yaml=None, working_folder=None):
        if vendor is None:
            vendor = VendorArrow('arrow', 'Arrow Electronics',
                                 'electronics',
        ...
        super(ArrowInvoice, self).__init__(vendor, inv_yaml, working_folder)
...
class VendorArrow(VendorBase):
    ...
    def __init__(self, name, dname, pclass, mappath=None,
        ...
        super(VendorArrow, self).__init__(
            name, dname, pclass, mappath,
            currency_code, currency_symbol, **kwargs
        )
        self.add_order_baseprice_component("Shipping Cost", 0)
...
avobj = VendorArrow('arrow', 'Arrow Electronics',
