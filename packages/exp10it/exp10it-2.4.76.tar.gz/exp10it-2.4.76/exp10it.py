...
/opt/exploit-database")
            ...
            os.system("searchsploit -w -t %s" % cms_value)
            os.system("searchsploit -w -t %s 2>&1 | tee /tmp/searchsploit" % cms_value)
            with open("/tmp/searchsploit","a+") as f:
                searchSploitResult="Below are possible vuls from exploit-db.com,but you should check if it \
exist by yourself coz the cms version is not sure.\n\n"+f.read()+"Upon are possible vuls from exploit-db.com,but you should check if it \
exist by yourself coz the cms version is not sure.\n\n"
