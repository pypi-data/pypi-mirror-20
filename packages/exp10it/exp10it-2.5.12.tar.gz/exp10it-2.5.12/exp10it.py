...
def moduleExist(moduleName):
    ...
    import os
    os.system(
    '''python3 -c "help('%s');" > /tmp/checkModuleExist''' %
     moduleName)
    with open("/tmp/checkModuleExist", "r+") as f:
        out = f.read()
        os.system("rm /tmp/checkModuleExist")
    if re.search(
    r"No Python documentation found for '%s'" %
    moduleName,
    out,
     re.I):
        return False
...
except BaseException: os.system("pip3 install colorama")
    from colorama import *
...
def string2argv(string):
    ...
    returnList = []
    for each in commandList:
        if not re.match("http", each) and re.search(
            "=", each) and each[0:2] == "--":
            tmpList = each.split("=")
...
/tmp && tar -xvzf libssh2.tar.gz -C /usr/src && cd /usr/src/libssh2-1.2.6/ && ./configure && make && make install")
    ...
    # 下面下载并编译
    os.system(
        "wget https://codeload.github.com/jmk-foofus/medusa/tar.gz/2.2 -O /tmp/medusa.tar.gz && \
...
def tab_complete_file_path():
    ...
    if platform.system() == "Linux":
        ...
        except BaseException: os.system("pip3 install readline")
            tab_complete_for_file_path()
    else:
        try:
            import readline
...
class Xcdn(object):
    ...
    def __init__(self, domain):
        ...
        # result = get_request(self.http_or_https + "://" +
        # self.domain,'seleniumPhantomJS')
    ...
    "现在通过修改hosts文件并刷新dns的方法检测ip:%s是否是domain:%s的真实ip" %
     (ip, self.domain))
        ...
        # hosts_changed_domain_title = get_request(self.http_or_https + "://%s"
        # % self.domain, 'seleniumPhantomJS')['title']
...
def figlet2file(logo_str, file_abs_path, print_or_not):
    ...
    except BaseException: if platform.system() == "Linux":
    ...
    if file_abs_path != 0:
        ...
        except BaseException:            print("figlet2file func write to file wrong,check it")
    else:
        pass
    if(print_or_not):
...
def get_request(url, by="MechanicalSoup", proxyUrl=""):
    ...
    if by == "seleniumPhantomJS":
        ...
        except BaseException:            os.system("pip3 install selenium")
            from selenium import webdriver
            from selenium.common.exceptions import TimeoutException
    ...
    '--proxy-type=%s' %
     proxyType]
        ...
        try:
            ...
            # get_http_domain_from_url(url).split("/")[-1] +
            # str(random.random()))
            code = None
            title = driver.title
        ...
        try:
            ...
            if title is not None:
                ...
                title_value = None
        except BaseException:            # 请求次数过多时被目标服务器禁止访问时
            code = 0
            ...
            CLIOutput().good_print(Fore.YELLOW +
     "可能由于访问过多或url错误(eg.scheme error:http|https)导致当前访问被重置,这个访问将不会得到任何内容")
...
os.system's cmd would not find the path")
    ...
    for each in all_files:
        if each[ :7] != "hunxiao" and each[-2:
            ] != "py" and os.path.isdir(each) is False:
...
def execute_sql_in_db(sql, db_name="mysql"):
    ...
    try:
        import MySQLdb
    except BaseException:        # for ubuntu16.04 deal with install MySQLdb error
    ...
    try:
        ...
        return result
    except BaseException:        import traceback
        traceback.print_exc()
        # 发生错误回滚
        conn.rollback()
    finally:
...
def write_string_to_sql(
    ...
    try:
        ...
        if ((table_name == eval(get_key_value_from_config_file(configIniPath, 'default', 'targets_table_name')) or table_name == eval(get_key_value_from_config_file(
            configIniPath, 'default', 'first_targets_table_name')) or table_name[-5:] == "_pang") and column_name == "http_domain") or (table_name[-5:] == "_urls" and column_name == "url"):
            pass
        else:
...
def auto_write_string_to_sql(
    ...
    if (
    table_name == eval(
        get_key_value_from_config_file(
            configIniPath,
            'default',
            'targets_table_name')) or table_name == eval(
                get_key_value_from_config_file(
                    configIniPath,
                    'default',
                     'first_targets_table_name'))) and column_name != "domain":
...
def get_user_and_pass_form_from_html(html):
    ...
    r'''form.*action=('|")?([^'"\s]{,100})('|")?.*>[\s\S]{,1000}?(<input .*type=('|")?password.*>)''',
     re.I)
...
def crack_ext_direct_webshell_url(url, pass_dict_file, ext):
    ...
    def crack_ext_direct_webshell_url_inside_func(url, pass_dict_file, ext):
        ...
        with futures.ThreadPoolExecutor(max_workers=20) as executor:
            ...
            executor.map(ext_direct_webshell_crack_thread, list(
                zip(passwords, urls, exts)), timeout=30)
...
def jieDiQi_crack_ext_direct_webshell_url(url, pass_dict_file, ext):
    ...
    def ext_direct_webshell_crack_thread(param1, param2,param3):
        ...
        if re.search(pattern, html):
            ...
            print(
    Fore.RED +
     "congratulations!!! find webshell password group,now try to get the only one password...")
            if ext !="jsp":
                # html即为密码内容
...
def crack_admin_login_url(
    ...
    sum[0]) +
    '%' +
    ' %s/%s  remain time:%s  %s\r' %
     (try_time[0], sum[0], remain_time[0], USERNAME_PASSWORD))
        sys.stdout.flush()
    ...
    (urls_table_name, string, url), eval(
        get_key_value_from_config_file(
    ...
    def crack_admin_login_url_inside_func(url, username, pass_dict_file):
        ...
        with futures.ThreadPoolExecutor(max_workers=20) as executor:
            executor.map(crack_admin_login_url_thread,
                         list(zip(urls, usernames, passwords)))
    ...
    def crack_allext_biaodan_webshell_url_inside_func(
        ...
        with futures.ThreadPoolExecutor(max_workers=20) as executor:
            executor.map(allext_biaodan_webshell_crack_thread,
                         list(zip(passwords, urls)), timeout=30)
...
def crack_webshell(url, anyway=0):
    ...
    (urls_table_name, strings_to_write, url), eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))
            for each_table in table_name_list:
                auto_write_string_to_sql(
...
def database_init():
    ...
    get_key_value_from_config_file(
        ...
        while 1:
            ...
            if check_string_is_ip(db_server) is True or check_string_is_domain(
                db_server) is True:
...
def crawl_url(url):
    ...
    url_main_target_pang_table_name, eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name'))):
            pang_table_exists = True
            result1 = execute_sql_in_db(
    "select * from `%s` where http_domain='%s'" %
    (url_main_target_pang_table_name, get_http_domain_from_url(url)), eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))
            if len(result1) > 0:
        ...
        if True == exist_table_in_db(
    url_main_target_sub_table_name, eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name'))):
            sub_table_exists = True
            result2 = execute_sql_in_db(
    "select * from `%s` where http_domain='%s'" %
    (url_main_target_sub_table_name, get_http_domain_from_url(url)), eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))
            if len(result2) > 0:
                current_not_main_target_table_name.append(
...
(dll)|(css)|(cab)|(bin)|(js))$", re.I)
    ...
    (target_or_pang_or_sub_urls_table_name, current_url), eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))
                else:
                    execute_sql_in_db(
...
def crawl_scan(target):
    ...
    eval(
        ...
        if len(result) > 0:
            for each in result:
                if each[0] != "":
                    crawl_scaned = get_scan_finished("crawl_scaned", eval(get_key_value_from_config_file(
                        configIniPath, 'default', 'db_name')), target.split( "/")[-1].replace(".", "_") + "_pang", each[0])
                    if crawl_scaned == 0:
                        crawl_url(each[0])
                        set_scan_finished("crawl_scaned", eval(get_key_value_from_config_file(
                            configIniPath, 'default', 'db_name')), target.split( "/")[-1].replace(".", "_") + "_pang", each[0])
                else:
    ...
    eval(
        ...
        if len(result) > 0:
            for each in result:
                if each[0] != "":
                    crawl_scaned = get_scan_finished("crawl_scaned", eval(get_key_value_from_config_file(
                        configIniPath, 'default', 'db_name')), target.split( "/")[-1].replace(".", "_") + "_sub", each[0])
                    if crawl_scaned == 0:
                        crawl_url(each[0])
                        set_scan_finished("crawl_scaned", eval(get_key_value_from_config_file(
                            configIniPath, 'default', 'db_name')), target.split( "/")[-1].replace(".", "_") + "_sub", each[0])
                ...
                set_scan_finished("crawl_scaned",
    eval(get_key_value_from_config_file(configIniPath,
    'default',
    'db_name')),
...
def mail_msg_to(msg, mailto, subject, user, password, format='plain'):
    ...
    if mailto is None:
        ...
        if re.search(r"mailto", content):
            mailto = get_key_value_from_config_file(
    ...
    configIniPath, 'mail', 'mailto', mailto)
        if re.search(r"user", content):
            user = get_key_value_from_config_file(
        ...
        if re.search(r"password", content):
            password = get_key_value_from_config_file(
...
def get_source_main_target_domain_of_sub_url(url):
    ...
    (eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'targets_table_name'))), eval(
                get_key_value_from_config_file(
                    configIniPath, 'default', 'db_name')))
    if len(result1) > 0:
        for each in result1:
    ...
    (eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'first_targets_table_name'))), eval(
                get_key_value_from_config_file(
                    configIniPath, 'default', 'db_name')))
    if len(result2) > 0:
        for each in result2:
...
def get_url_belong_main_target_domain(url):
    ...
    print(
    "get_url_belong_main_target_domain(%s) error,I will try to get it again and exist." %
     url)
    a =0
...
def get_sqlmap_result_and_save_result(url):
    ...
    sql1 = "select http_domain from `%s` where domain='%s'" % ( eval(
    get_key_value_from_config_file(
        configIniPath,
        'default',
        'targets_table_name')),
         source_domain)
    sql2 = "select http_domain from `%s` where domain='%s'" % ( eval(
    get_key_value_from_config_file(
        configIniPath,
        'default',
        'first_targets_table_name')),
    ...
    try:
        ...
        log = f.read()
        f.close()
    except BaseException:        print("%s not exist" % target_folder)
        return ""
...
def getIp(domain):
    ...
    try:
        ...
        return myaddr
    except BaseException:        print("getip wrong")
...
def get_pang_domains(target):
    ...
    (main_target_table_name, target), eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))
    if len(result) > 0:
        if result[0][0] == '0':
        ...
        if len(result) == 0:
            with open(domain_pang_file, "r+") as f:
                for each in f:
                    each = re.sub(r"(\s)$", "", each)
                    if each != target:
                        execute_sql_in_db("insert into `%s`(http_domain,domain) values('%s','%s')" % ( domain_pang_table_name, each, each.split(
                            "/")[-1]), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
...
http_domain varchar(70) not null)" % each_pang_urls_table_name
    ...
    eval(
        ...
        for each in http_domain_list:
            each = re.sub(r"(\s)$", "", each)
            if each != target:
                execute_sql_in_db("insert into `%s`(http_domain,domain) values('%s','%s')" % ( domain.replace(
                    ".", "_") + '_pang', each, each.split('/')[-1]), eval(get_key_value_from_config_file(configIniPath, 'default', 'db_name')))
                # 创建除目标domain外的每个旁站的urls表,因为目标domain的urls表之前已经创建过
                each_pang_urls_table_name = each.split(
                    '/')[-1].replace(".", "_") + "_urls"
                sql = "create table `%s`(url varchar(250) not null primary key,code varchar(50) not null,\
...
def get_sub_domains(target, use_tool="Sublist3r"):
    ...
    "git clone https://github.com/aboul3la/Sublist3r.git %sSublist3r" %
     ModulePath)
            ...
            os.system(
    "cd %sSublist3r && pip install -r requirements.txt" %
     ModulePath)
...
def get_main_target_table_name(target):
    ...
    (eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'targets_table_name')), domain), eval(
                get_key_value_from_config_file(
                    configIniPath, 'default', 'db_name')))
    result2 = execute_sql_in_db(
    "select * from `%s` where domain='%s'" %
    (eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'first_targets_table_name')), domain), eval(
                get_key_value_from_config_file(
                    configIniPath, 'default', 'db_name')))
    if len(result1) > 0:
        return eval(
...
def get_target_table_name_list(target):
    ...
    url_main_target_pang_table_name, eval(
        get_key_value_from_config_file(
            ...
            result1 = execute_sql_in_db(
    "select * from `%s` where http_domain='%s'" %
    (url_main_target_pang_table_name, get_http_domain_from_url(target)), eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))
            if len(result1) > 0:
                current_not_main_target_table_name.append(
    ...
    url_main_target_sub_table_name, eval(
        get_key_value_from_config_file(
            ...
            result2 = execute_sql_in_db(
    "select * from `%s` where http_domain='%s'" %
    (url_main_target_sub_table_name, get_http_domain_from_url(target)), eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))
            if len(result2) > 0:
                current_not_main_target_table_name.append(
...
def single_script_type_scan(target):
    ...
    (each_table, script_type_value, target), eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))
...
def single_dirb_scan(target):
    ...
    if False == os.path.exists(ModulePath + "dirsearch"):
        os.system(
    "git clone https://github.com/maurosoria/dirsearch.git %sdirsearch" %
     ModulePath)
    ...
    (target_urls_table_name, each_url), eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))
        if len(sql_result) > 0:
...
def single_cms_scan(target):
    ...
    (each_table, cms_value, target), eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))
            set_scan_finished(
    "cms_identify_scaned",
...
/opt/exploit-database")
            ...
            os.system(
    "searchsploit -w -j -t %s 2>&1 | tee /tmp/searchsploit" %
     cms_value)
...
exist by yourself coz the cms version is not sure.\n\n"
    ...
    ModulePath + "log/cms_scan_log/dzscan"):
                    os.system(
    "cd %slog/cms_scan_log && mkdir dzscan" %
     ModulePath)
    ...
    "log/cms_scan_log/dzscan/" +
     log_file):
                ...
                os.system(
    "mv %scms_scan/dzscan/%s %slog/cms_scan_log/dzscan/" %
        ...
        get_key_value_from_config_file(
            ...
            "http_domain",
             target)
                ...
                if False == os.path.exists(
    ModulePath + "log/cms_scan_log/joomscan"):
                    os.system(
    "cd %slog/cms_scan_log && mkdir joomscan" %
     ModulePath)
                cms_scaner_list =os.listdir(ModulePath + "cms_scan")
...
&& unzip joomscan-latest.zip -d joomscan && rm joomscan-latest.zip" % ModulePath)
    ...
    "cd %scms_scan/joomscan && perl joomscan.pl update && perl joomscan.pl -u %s -ot" %
     (ModulePath, target))
                ...
                os.system(
    "mv %scms_scan/joomscan/%s log/cms_scan_log/joomscan/ " %
     (ModulePath, log_file))
        ...
        get_key_value_from_config_file(
            ...
            "http_domain",
             target)
                ...
                if False == os.path.exists(
    ModulePath + "log/cms_scan_log/wpscan"):
                    os.system(
    "cd %slog/cms_scan_log && mkdir wpscan" %
     ModulePath)
                cms_scaner_list =os.listdir(ModulePath + "cms_scan")
...
ruby-dev build-essential libgmp-dev zlib1g-dev")
    ...
    "log/cms_scan_log/wpscan/" +
     log_file):
                    ...
                    os.system(
    "mv %scms_scan/wpscan/%s %slog/cms_scan_log/wpscan/" %
     (ModulePath, log_file, ModulePath))
...
def sqlmap_g_human(http_url_or_file, tor_or_not, post_or_not):
    ...
    if re.match("(http://)|(https://)", http_url_or_file):
        ...
        except BaseException:            print("open file error")
...
def sqli_scan(target):
    ...
    sub_domains_sqli_scaned =get_scan_finished(
        ...
        main_target_table_name,
        http_domain)
    if eval(get_key_value_from_config_file(configIniPath, 'default', 'scan_way')
            ) == 1 and http_domain_sqli_scaned == 1 and pang_domains_sqli_scaned == 1:
        return
    if eval(get_key_value_from_config_file(configIniPath, 'default', 'scan_way')
            ) == 2 and http_domain_sqli_scaned == 1 and sub_domains_sqli_scaned == 1:
        return
    if eval(get_key_value_from_config_file(configIniPath, 'default', 'scan_way')
            ) == 3 and http_domain_sqli_scaned == 1 and pang_domains_sqli_scaned == 1 and sub_domains_sqli_scaned == 1:
        return
    if eval(
    get_key_value_from_config_file(
        configIniPath,
        'default',
         'scan_way')) == 4 and http_domain_sqli_scaned == 1:
        return
...
input your number here:''', end='')
    ...
    eval(
        get_key_value_from_config_file(
            ...
            'default',
            'db_name')),
            domain_pang_table_name,
             pang_http_domain_value)
                            set_scan_finished(
    "pang_domains_sqli_scaned",
    eval(
        get_key_value_from_config_file(
    ...
    eval(
        get_key_value_from_config_file(
            ...
            'default',
            'db_name')),
            domain_sub_table_name,
             sub_http_domain_value)
                            set_scan_finished(
    "sub_domains_sqli_scaned",
    eval(
        get_key_value_from_config_file(
...
def single_port_scan(target):
    ...
    (each_table, strings_to_write, target), eval(
        get_key_value_from_config_file(
            configIniPath, 'default', 'db_name')))
...
class MyScanner(object):
        ...
        get_key_value_from_config_file(
            ...
            main_target_table_name,
             target)
                ...
                print(
    "get_scan_finished error in %s func" %
                    ...
                    for each in result:
                        if each[0] != "":
                            ...
                            if xxx_scaned == 0:
                                ...
                                set_scan_finished(xxxValue + "_scaned",
    eval(get_key_value_from_config_file(configIniPath,
    'default',
    'db_name')),
    target.split( "/")[-1].replace(".",
    "_") + "_pang",
     each[0])
                            ...
                            print(
    "%s func's each[0] error in scan_way 1" %
     single_xxx_scan.__name__)
                else:
                    print(
    "%s func's %s error or target has no pang domains" %
    ...
    "_") + "_sub",
     each[0])
                            ...
                            print(
    "%s func's each[0] error in scan_way 2" %
     single_xxx_scan.__name__)
                else:
                    print(
    "%s func's %s error or target has no sub domains " %
     (single_xxx_scan.__name__, sql))
    ...
    "_") + "_pang",
     each[0])
                            ...
                            print(
    "%s func's each[0] error in scan_way 3" %
     single_xxx_scan.__name__)
                else:
                    print(
    "%s func's %s error or target has no pang domains" %
...
def auto_attack(target):
    ...
    try:
        crawl_scan(target)
    except BaseException:        from colorama import init, Fore
        init(autoreset=True)
        print(
            Fore.YELLOW +
