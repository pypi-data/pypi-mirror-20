...
def hwe(controls, cases):
    ...
    for pair in pairs:
        ...
        exp_rr = p**2 * n, 2              #expected homozygous ref
        exp_ra = 2 * p * q * n, 2         #expected heterozygous
        exp_aa = q**2 * n, 2              #expected homozygous alt
        
        s = stats.chisquare([obs_rr, obs_ra, obs_aa], [exp_rr, exp_ra, exp_aa])
        result.append([pair[0], s.statistic, s.pvalue])
    return result
