...
The initial arbitrary value for p was set to 0.25, which is the biologically
...
def ld(snp1, snp2, controls, cases):
    ...
    snp1_final = snp1_total[0]
    snp2_final = snp2_total[0]
    ...
    while True:
        ...
        if abs(p_new - p_old) < 0.01:
            ...
            p_rr = p_new
        #print(p_rr, counter)
    #CALCULATE DOUBLE HAPLOTYPES FROM THE MLE FOR P_RR  (NOT USED EVENTUALLY THOUGH, COULD BE REMOVED)     
    p_ra = p_r1 - p_rr
    p_ar = p_r2 - p_rr
    p_aa = 1 + p_rr - p_r1 - p_r2
    #CALCULATE D
    D = p_rr - p_r1*p_r2
    #CALCULATE D'
    if D >= 0:
