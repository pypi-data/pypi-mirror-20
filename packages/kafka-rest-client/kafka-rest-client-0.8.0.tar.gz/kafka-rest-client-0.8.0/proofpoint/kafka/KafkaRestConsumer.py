...
class KafkaRestConsumer(InfobusConsumer):
    ...
    Keyword Arguments:
        ...
        krp_consumer_timeout (int) (seconds): Kafka Rest proxy consumer timeout. Amount of idle time before a consumer instance is
            ...
            Default: 300
        krp_https_port (int) : Kafka Rest proxy https port.
            Default: 8483
        error_backoff (int) (seconds) : Num seconds to backoff during connection timeouts. This is used as base for exponential backoff
            Default: 8483
        request_failure_retries (int): Number of times to retry in case of  connection timeouts and specific error codes
            default of set of http error codes it retries on 413,429,502,503,504,598,599
            Default: 5
    ...
    def __init__(self, **configs):
        ...
        retry = Retry(
            total=self.config['request_failure_retries'],
            status_forcelist=[413,429,502,503,504,598,599],
            backoff_factor=self.config['error_backoff'],
            raise_on_status=True
        )
        self.__oauth_session.mount('https://', HTTPAdapter(max_retries=retry))
        self.__oauth_session.mount('http://', HTTPAdapter(max_retries=retry))
        ...
        try:
            ...
            try:
                ...
                if int(e.error_code) == 40403:
                    ...
                    log.error(e)
                    raise e
            except (ConnectTimeout, ConnectionError, RetryError) as e:
                log.debug(e.message)
    ...
    def __handle_disconnections(self):
        ...
        log.debug("delta is : " + str(delta.total_seconds()))
    ...
    def __register_consumer(self):
        ...
        # KRP doesn't return an https port because when nginx requests come to KRP
        # the request uri doesn't have the port (comes in with default https port) then
        # the KRP doesn't append the https port. Don't have a better solution other than this
        self.__krp_host_port = '%s://%s:%s' % (url.scheme, url.hostname, self.config['krp_https_port'])
        log.info("Created consumer on " + self.__krp_host_port)
        self.__set_urls()
