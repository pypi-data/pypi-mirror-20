...
class DeviceApi(object):
    ...
    def __do_cleanup(self):
        try:
            ...
            reboot_on_error = self.__opts.get(ClientOption.REBOOT_ON_ERROR)
            if reboot_on_error:
                import RPi.GPIO
                reboot_delay = self.__opts.get(ClientOption.REBOOT_DELAY)
                self.__write_to_lcd('System Offline!', 'Reboot in {0}m...'.format(reboot_delay))
