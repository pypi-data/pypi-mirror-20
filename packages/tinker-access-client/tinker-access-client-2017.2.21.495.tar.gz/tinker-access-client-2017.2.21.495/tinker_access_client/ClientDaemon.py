...
class ClientDaemon:
    ...
    def __should_update(opts, requested_version=None):
        ...
        latest_version = data.get('info', {}).get('version', '0.0.0').encode('utf-8')

        if not requested_version:
            logger.error('current_version: %s', current_version)
            logger.error('latest_version: %s', latest_version)
            logger.error('current_version != latest_version: Will update? %s', current_version != latest_version)
