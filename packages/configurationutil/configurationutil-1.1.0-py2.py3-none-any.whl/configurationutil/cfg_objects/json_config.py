...
class JSONConfig(ConfigObject):
    ...
    def find(self,
        ...
        if key is None:
            ...
            value, _ = self.__retrieve_item(self._cfg, items)
    ...
    def __delitem__(self, key):
        items = self.__split_key(key)
        _, item_obj = self.__retrieve_item(self._cfg, items)

        del item_obj[items[-1]]
    ...
    def __retrieve_item(self,
        ...
        if len(items) == 0:
            ...
            return self.__retrieve_item(item, items)
    ...
    def __add_node(self,
        ...
        """ Add the nodes in the item list to the object and set the value

        :param obj:     The object to load the item from
        :param items:   List of items to traverse to get to the requested value
        :param value:   The value to set the last node to
        """
        ...
        if len(items) == 0:
            ...
            if obj.get(current_item_key, u'Key not found') == u'Key not found':
                obj[current_item_key] = {}
