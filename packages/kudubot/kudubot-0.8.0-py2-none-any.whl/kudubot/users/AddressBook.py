...
class AddressBook(object):
    ...
    def __init__(self, database):
        ...
        self.db.commit()
        logging.info(u"Address Book initialized")
    ...
    def add_or_update_contact(self, contact):
        ...
        old = self.db.execute(u"SELECT * FROM address_book WHERE id=? OR address=?",
                              (contact.database_id, contact.address)).fetchall()

        if len(old) != 1:
            # Increment the ID
            logging.info(u"Address " + contact.address + u" does not exist yet. Inserting into address book.")
            max_id = self.db.execute(u"SELECT CASE WHEN COUNT(id) > 0 THEN MAX(id) ELSE 0 END AS max_id "
                                     u"FROM address_book").fetchall()[0][0]
            contact.database_id = max_id + 1
        elif contact.database_id == -1:
            contact.database_id = self.db.execute(u"SELECT id FROM address_book WHERE address=?",
                                                  (contact.address,)).fetchall()[0][0]

        # Insert into the database
        self.db.execute(u"INSERT OR REPLACE INTO address_book VALUES (?, ?, ?)",
    ...
    def get_contact_for_address(self, address):
        ...
        result = self.db.execute(u"SELECT * FROM address_book WHERE address=?", (address,)).fetchall()
