...
class Producer(object):
    ...
    def _send(self, queue, message, delay_seconds, invisible_seconds):
        ...
        except TApplicationException, ex:
            return False, ex
        ...
        try:
            ...
            if res.failed:
                ...
                for entry in res.failed:
                    msgs.append(messages[int(entry.entryId) - 1])
                print "批量发送失败,重发失败的消息:%s" % str(msgs)
    ...
    def basic_publish(self, queue, task, *args, **kwargs):
        ...
        :return :(tuple)(is_success, res)
        """
        return self.publish(queue, task, self.default_delay_seconds, self.default_limit_seconds, *args,
                            **kwargs)
