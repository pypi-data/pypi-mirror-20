...
# setup stuff
    ...
    def get_index_health(self, index):
        ...
        if current["state"] not in ["complete", "empty"]:
            last = self.get_last_success[index]
            if last["newcommit"] != current["newcommit"]:
                current["last_successful_commit"] = last["newcommit"]
                current["last_successful_publish"] = last["finish"]
        del current["log"]
        for doctype in ["articles", "items", "snippets"]:
            url = self.make_url(index, doctype, "_count")
            current["number_of_" + doctype] = self.request("get", url)
        return current
