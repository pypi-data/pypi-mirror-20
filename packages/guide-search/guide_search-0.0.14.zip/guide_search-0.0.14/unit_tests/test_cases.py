...
def test_buildSort(esearch):
    ...
    assert esearch.buildSort() == {'sort': [{'_score': {'order': 'desc'}}]}
    assert esearch.buildSort(order='desc') == {'sort': [{'_score': {'order': 'desc'}}]}
    assert esearch.buildSort(sort='score') == {'sort': [{'_score': {'order': 'desc'}}]}
    assert esearch.buildSort(sort='score',order='desc') == {'sort': [{'_score': {'order': 'desc'}}]}
    assert esearch.buildSort(sort='date',order='asc') == {'sort': [{'master.lastmodified': {'order': 'asc'}}]}
...
# def test_postLog(esearch, index):
    # seq = '' update["seq"]
    # url = esearch.makeUrl(index, "commits", "{0}-{1}".format(index, seq)).strip()
