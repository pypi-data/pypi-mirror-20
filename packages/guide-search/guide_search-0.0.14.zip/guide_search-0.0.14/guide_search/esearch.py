...
# query functions
    ...
    def get_cluster_list(self, index, **kwargs):
        ...
        try:
            cl = res["hits"]["hits"]
            for c in cl:
                cls[c["_source"]["id"][0]]["title"] = c["_source"]["title"][0]
                cls[c["_source"]["id"][0]]["scope"] = c["_source"]["scope"][0]
        except:
            ResultParseError
...
# setup stuff
    ...
    def get_info(self):
        try:
            ...
            res = self.request("get", url)
            del res["tagline"]
            res["version"] = res["version"]["number"]
            url = self.make_url("_cluster", "health")
            try:
                health = self.request("get", url)
                res["cluster_health"] = health["status"]
                if health["status"] == "red":
                    res["status"] = "alive"
                else:
                    res["status"] = "limping"
            except:
                res["status"] = "not quite alive"
        except:
            res = {"status": "dead"}
        return res
