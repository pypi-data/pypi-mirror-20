...
COMMANDS = {
    ...
    'stats': dict,
    'status': dict,
    # 'sticker_get': ParserStickerItem,
    # 'sticker_set': transform_empty,
    # 'sticker_delete': transform_empty,
    # 'sticker_list': ParserStickerList,
    # 'sticker_find': ParserStickerDict,
...
class Request(asyncio.Future):
    ...
    def set_result(self, result):
        _logger.debug("Got result {} for request {}".format(result, self))
    ...
    def _new_request(executor, name):
        if name in COMMANDS:
            ...
            return lambda commandline: RequestActive(executor, commandline)
        elif name in EVENTS:
            return EVENTS[name]
        else:
...
class RequestCommand(RequestActive):
    def __init__(self, executor, name, *args):
        super().__init__(executor, name + ''.join(' "{}"'.format(str(arg).replace('"', '\\"')) for arg in args))
        self._transform = COMMANDS[name]
    ...
    def _parser(self, success=SUCCESS):
        ...
        while True:
            ...
            if line.startswith(ERROR_PREFIX):
                self.set_exception(errors.ReplyError(line))
                return
            elif line == success:
                self.set_result(self._transform(lines))
                return
            else:
                lines.append(line.split(DELIM, 1))


class RequestIdle(RequestCommand):
    def __init__(self, executor):
        RequestActive.__init__(self, executor, 'idle')
        self._transform = transform_single_list
...
EVENTS = {name: 1 << i for i, name in enumerate(SUBSYSTEM_NAMES + OTHER_EVENTS)}
EVENTS['ANY'] = (1 << len(SUBSYSTEM_NAMES)) - 1
EVENTS['NONE'] = 0
...
class RequestPassive(Request):
    ...
    def __init__(self, executor, event_mask, timeout=None):
        ...
        if timeout is None:
            ...
            self._event_mask |= EVENTS['TIMEOUT']
    ...
    async def _timeout_task(self, timeout):
        try:
            ...
            self.set_result(EVENTS['TIMEOUT'])
        except asyncio.CancelledError:
            pass

    def __repr__(self):
        return 'IDLE({})'.format(' | '.join(name for i, name in enumerate(SUBSYSTEM_NAMES + OTHER_EVENTS) if (1 << i) & self._event_mask))
