...
class Executor(object):
    ...
    async def connect_loop(self, connected_cb=None, disconnected_cb=None):
        ...
        while True:
            try:
                await self.idle(self.CONNECT)
                _logger.debug("Connected")
                connected_cb and connected_cb()
                await self.idle(self.NONE)
...
class Client(object):
        ...
        try:
            ...
            while True:
                if self._active_queue:
                    ...
                    if self._active or self._event(_request.EVENTS['IDLE'], True):
                        continue
                    self._idle_task()
    ...
    def _current_events(self):
        return (_request.EVENTS['IDLE'] if self.is_connected and self._is_idle else 0) | (_request.EVENTS['CONNECT'] if self.is_connected else 0)
    ...
    async def _connect_task(self, welcome):
        ...
        if self._password:
            try:
                ...
                return
        self._event(_request.EVENTS['CONNECT'])
    ...
    async def _idle_task(self):
        ...
        event = sum(_request.EVENTS[subsystem.upper()] for subsystem in await request)
        if event:
            self._event(event)
...
class ServerProperties(object):
        ...
        try:
            while True:
                try:
                    ...
                    await self.ampd.idle(self.ampd.PLAYER)
                    continue
                status = await self.ampd.status()
                if int(status['volume']) == value:
                    break
                await self.ampd.idle(self.ampd.PLAYER | self.ampd.MIXER)
    ...
    async def _connected_cb(self):
        while True:
            ...
            self._unblock()
            await self.ampd.idle(self.ampd.PLAYER | self.ampd.MIXER | self.ampd.OPTIONS | self.ampd.UPDATE, timeout=(int(self.elapsed + 1.5) - self.elapsed) if self.state == 'play' else None)
