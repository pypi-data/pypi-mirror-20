...
CertificateDefaults ClientAuthentication and ServerAuthentication
...
class CPABuilder():
    ...
    def unify_signature(self, acpp, bcpp, context,
        ...
        self.unify_simple_subelement(asec, bsec,security,
                                     ...
                                     intersectifmultiple=False, strictelements=False)
        self.unify_signing_certificate_ref(acpp, bcpp, context,
                                           asec, bsec, security, direction)
    ...
    def unify_signing_certificate_ref(self, acpp, bcpp, context,
                                      asec, bsec, security, direction):

        if direction == 'send':
            signing_certL = asec.xpath('child::cppa:SigningCertificateRef',
                                         namespaces=self.NSMAP)
            default_signing_certL = acpp.xpath(
                'descendant::cppa:CertificateDefaults/cppa:SigningCertificateRef',
                namespaces=self.NSMAP)
            to_append_to = asec
        else:
            signing_certL = bsec.xpath('child::cppa:SigningCertificateRef',
                                         namespaces=self.NSMAP)
            default_signing_certL = bcpp.xpath(
                'descendant::cppa:CertificateDefaults/cppa:SigningCertificateRef',
                namespaces=self.NSMAP)
            to_append_to = bsec
        if len(signing_certL) > 0:
            security.append(deepcopy(signing_certL[0]))
        elif len(default_signing_certL) > 0:
            security.append(deepcopy(default_signing_certL[0]))
            to_append_to.append(deepcopy(default_signing_certL[0]))
    ...
    def unify_encryption(self, acpp, bcpp, context,
        ...
        self.unify_encryption_certificate_ref(acpp, bcpp, context,
                                           asec, bsec, security, direction)
        #self.unify_simple_subelement(asec, bsec, security,
        #                             'cppa', 'EncryptionCertificateRef',
        #                             strictelements=False, required=False)
    ...
    def unify_encryption_certificate_ref(self, acpp, bcpp, context,
                                      asec, bsec, security, direction):
        if direction == 'receive':
            encryption_certL = asec.xpath('child::cppa:EncryptionCertificateRef',
                                         namespaces=self.NSMAP)
            default_encryption_certL = acpp.xpath(
                'descendant::cppa:CertificateDefaults/cppa:EncryptionCertificateRef',
                namespaces=self.NSMAP)
            to_append_to = asec
        else:
            encryption_certL = bsec.xpath('child::cppa:EncryptionCertificateRef',
                                         namespaces=self.NSMAP)
            default_encryption_certL = bcpp.xpath(
                'descendant::cppa:CertificateDefaults/cppa:EncryptionCertificateRef',
                namespaces=self.NSMAP)
            to_append_to = bsec
        if len(encryption_certL) > 0:
            security.append(deepcopy(encryption_certL[0]))
        elif len(default_encryption_certL) > 0:
            security.append(deepcopy(default_encryption_certL[0]))
            to_append_to.append(deepcopy(default_encryption_certL[0]))
    ...
    """
    def unify_compressed_mime_part(self, apart, bpart, context,
                                   acpp, bcpp, direction):
    ...
    """
    def unify_compressed_mime_multipart_related(self, apart, bpart, context,
                                     acpp, bcpp, direction):
        mimepart = lxml.etree.Element(apart.tag, nsmap=self.NSMAP)
        unify_atts(apart, bpart, mimepart, strictatts=True)
        self.unify_mime_part_lists(mimepart, apart, bpart, context,
