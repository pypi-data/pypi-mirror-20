...
hidashApp.controller('ReportsController',function($scope, $http, $window, $location, utils){
  ...
  $scope.download = function() {
    var params_str = '?group=' + $scope.group
    $scope.filters.map(function(filter, index) {
      if($scope.parameters[index].value) {
        params_str += '&' + filter.name + '=' + $scope.parameters[index].value
      } else {
        params_str += '&' + filter.name + '=' + $scope.parameters[index]
      }
    })
    $window.open('/hidash-api/download_docx/' + params_str, '_blank');
  }
})
    ...
    "<div id='dashboard' class='container'>"+
      "<div class='hidash-dashboard-filter row'>"+
        "<div ng-repeat='filter in filters'>"+
          ...
          "</div>"+
        "</div>"+
          "<button id='download-btn' class='btn' ng-click='download()'>Download</button>"+
      "</div>"+
