            ...
            if node.value.is_operation():
                ...
                for i in range(len(node.children)):
                    ...
                    if child.value.is_operation() and \
                       (
                            (
                                (node.value.operation == BinOp.Mult or
                                 node.value.operation == BinOp.Div) and not
                                (child.value.operation == BinOp.Mult or
                                 child.value.operation == BinOp.Div)
                            ) or
                            ...
                            child.value.operation == UnOp.USub or
                            child.value.operation == UnOp.UAdd or
                            node.value.operation == UnOp.USub or
                            node.value.operation == UnOp.UAdd
                       ):
                        result += '(' + dfs(child) + ')'
...
class Rules:
    ...
    def get_possible_rules(self, node, tree):
        result = []
        back_node = Tree(root=node).get_copy()
        for rule in self.data:
            unify_result = node.unify(rule[0].root)
            if unify_result is not None:
                node.replace(rule)
                copy_tree = Tree(root=tree.root).get_copy()
                result.append((copy_tree, rule))
                node.value = back_node.root.value
                node.children = back_node.root.children
        children = [child for child in node.children if child.value.is_operation()]
