...
class DossierService(object):
    def __init__(self, settings, logger, system_token, failure_threshold=5, timeout_default=60, max_timeout=300,
        ...
        self.determine_region_reps_operation = DetermineRegionRepsCommand(
            regioverantwoordelijken_url=self.regioverantwn_url,
            logger=self.logger,
            system_token=self.system_token
        )
        self.retrieve_emails_actoren_operation = RetrieveEmailsActorenCommand(
            logger=self.logger,
            system_token=self.system_token
        )
        self.determine_niscodes_operation = DetermineNiscodesCommand(
            logger=self.logger,
            admin_grenzen_client=self.admin_grenzen_client
        )
        self.retrieve_data_operation = RetrieveDataCommand(
            logger=self.logger,
            system_token=self.system_token
        )
        self.retrieve_masterdata_operation = RetrieveMasterdataCommand(
            logger=self.logger,
            system_token=self.system_token
        )
