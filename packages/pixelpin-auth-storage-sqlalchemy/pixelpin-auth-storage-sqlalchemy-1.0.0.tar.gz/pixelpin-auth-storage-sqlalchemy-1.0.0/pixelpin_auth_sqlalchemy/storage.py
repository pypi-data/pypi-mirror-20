"""SQLAlchemy models for PixelPin Auth"""
import base64
import six
import json
...
from pixelpin_auth_core.storage import UserMixin, AssociationMixin, NonceMixin, \
...
class SQLAlchemyUserMixin(SQLAlchemyMixin, UserMixin):
    ...
    __tablename__ = 'pixelpin_auth_userpixelpinauth'
...
class SQLAlchemyNonceMixin(SQLAlchemyMixin, NonceMixin):
    __tablename__ = 'pixelpin_auth_nonce'
...
class SQLAlchemyAssociationMixin(SQLAlchemyMixin, AssociationMixin):
    __tablename__ = 'pixelpin_auth_association'
...
class SQLAlchemyCodeMixin(SQLAlchemyMixin, CodeMixin):
    __tablename__ = 'pixelpin_auth_code'
...
class SQLAlchemyPartialMixin(SQLAlchemyMixin, PartialMixin):
    __tablename__ = 'pixelpin_auth_partial'
    id = Column(Integer, primary_key=True)
