...
TSV_HEADERS = [
    ...
    # Chromosome
    ({'chromosome', 'chr', 'chrom', 'chromosome_name', '#chrom'}, 'CHROMOSOME', str),
    ...
    # Alternate
    ({'alt', 'tumor_seq_allele1', 'variant'}, 'ALT', str),
...
def __is_known_header(header):
    h_lower = header.lower()
    for valid_headers, field, method in TSV_HEADERS:
        if h_lower in valid_headers:
            return field, method
    return None, None


def tsv_header(line, extra=False):

    header = []
    extra_header = []
    for i, h in enumerate(line):
        field, method = __is_known_header(h)
...
def tsv_parser(lines, header=None, close=False, extra=False):

    for l, line in __base_parser(lines):

        # Initialize header
        if header is None:
            header, inferred_header = tsv_header(line, extra=extra)
        ...
        try:
            ...
            logger.warning("Error parsing line %d", l)
            continue
...
def __parser_generator(file, extra=False):
    ...
    return tsv_parser(fd, close=close, extra=extra)
...
def variants(file, annotations=None, extra=False):
    ...
    if isinstance(file, types.GeneratorType) or isinstance(file, list):
        ...
        for f, a in file:
            for row in variants(f, annotations=a, extra=extra):
                yield row
    else:
        ...
        parser = __parser_generator(file, extra=extra)
