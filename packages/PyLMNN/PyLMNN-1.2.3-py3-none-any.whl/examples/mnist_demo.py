...
def main(autotune=True, load=0):
    ...
    if autotune:
        ...
        t_lmnnParams = time()
        Klmnn, Knn, outdim, maxiter = find_hyperparams(xtr, ytr, xva, yva)
    ...
    lmnn = LargeMarginNearestNeighbor(verbose=True, n_neighbors=Klmnn, max_iter=maxiter, n_features_out=outdim, save='lin_transf',
                                      log_level=10)
    if load == 0:
        # Train full model
