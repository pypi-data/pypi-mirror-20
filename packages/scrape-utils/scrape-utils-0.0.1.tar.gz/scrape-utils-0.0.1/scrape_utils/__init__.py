...
def request(url, **kwargs):
    ...
    r = requests.get(url.strip(), **kwargs)
    if r.status_code != 200:
        raise Exception("Unable to fetch url '%s'. Status Code: %s" % (url, r.status_code))
    return r
...
def load_proxies_list(proxies):
    ...
    """
    _store.update({
        "ttl": int(time.time()) + (60 * 60 * 24 * 30 * 365), # 1 year, so it never gets expires
        "data": proxies
    })
