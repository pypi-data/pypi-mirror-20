...
class MangaSeries(object):
    ...
    def __init__(self, url, root_directory):
        u"""
    ...
    def scrape(self, skip_existing_chapters = False):
        u"""
    ...
    def download_manga(self, update = False, repair = False):
        u"""
        ...
        for volume in self.volumes:
            ...
            for chapter in volume.get_chapters():
                ...
                for page in chapter.get_pages():
                    page_file = os.path.join(chapter_directory, page.get_page_name() + u".jpg")
    ...
    def zip(self, zip_volumes = False, zip_chapters = False):
        u"""
        ...
        for volume in os.listdir(self.root_directory):
            ...
            if volume.startswith(u"Volume ") and os.path.isdir(volume_dir):

                if zip_volumes:
                    if not self.dry_run:
                        shutil.make_archive(volume_dir, u"zip", volume_dir)

                if zip_chapters:
                    for chapter in os.listdir(volume_dir):
                        chapter_dir = os.path.join(volume_dir, chapter)

                        if chapter.startswith(u"Chapter ") and os.path.isdir(chapter_dir):
                            if not self.dry_run:
                                shutil.make_archive(chapter_dir, u"zip", chapter_dir)
    ...
    def set_maximum_thread_amount(self, max_threads):
        u"""
    ...
    def download_file(options):
        ...
        if repair:
            try:
                url_size = int(requests.head(url).headers[u"Content-Length"])
                file_size = os.path.getsize(destination)

                if url_size == file_size:
                    return
                elif verbose:
                    print u"Local file broken, starting repair"
        ...
        if not dry_run:
            with open(destination, u'wb') as destination_file:
                if verbose:
                    print u"Downloading " + destination
