...
class MangaFoxScraper(GenericMangaScraper):
    ...
    @staticmethod
    def scrape_volumes_from_url(manga_url, manga_directory, skip_existing_chapters = False,
        ...
        for volume in volumes:
            ...
            for chapter in chapters:
                chapter_start_url = unicode(chapter).split(u"href=\"")[1].split(u"\"")[0]
                ...
                if chapter_number.is_integer():
                    formatted_chapter_number = unicode(int(chapter_number)).zfill(3)
                else:
                    pre_dot, post_dot = unicode(chapter_number).split(u".")
                    formatted_chapter_number = pre_dot.zfill(3) + u"." + post_dot

                chapter_directory = os.path.join(manga_directory, u"Volume " + unicode(volume_number).zfill(2))
                chapter_directory = os.path.join(chapter_directory, u"Chapter " + formatted_chapter_number)
                ...
                page_amount = int(unicode(chapter_soup.select(u".l")[0]).rsplit(u"of ", 1)[1].split(u"\t", 1)[0])  # Don't ask

                if os.path.isdir(chapter_directory) and skip_existing_chapters:
                    if page_amount == len(os.listdir(chapter_directory)):
                        if verbose:
                            print u"Skipping Chapter " + formatted_chapter_number
                        continue
