...
class Interface(object):
    ...
    def interfaces(self):
        ...
        while (has_more == '') or (has_more == 'true'):
            ...
            rest_root = etree.fromstring(
                re.sub(' xmlns[^ \t\n\r\f\v>]+', '', interface_result.json))
            has_more = rest_root.find('.//has-more')

            for item in rest_root.findall('.//interface'):
                interface_type = item.find('.//interface-type').text
                interface_name = item.find('.//interface-name').text
                last_interface_type = interface_type
                last_interface_name = interface_name
                if "gigabitethernet" in interface_type:
                    interface_role = item.find('.//port-role').text
                    if_name = item.find('.//if-name').text
                    interface_state = item.find('.//if-state').text
                    interface_proto_state = item.find('.//line-protocol-state').text

                    interface_mac = item.find(
                                              './/current-hardware-address').text
        ...
        for interface in rest_root.findall('.//interface'):
            int_type = interface.find( './/interface-type').text
            int_name = interface.find('.//interface-name').text
            if int_type == 'unknown':
                continue

            int_state = interface.find( './/if-state').text
            int_proto_state = interface.find( './/line-protocol-state').text

            ip_address = interface.find( './/ipv4').text
    ...
    def interface_detail(self):
        ...
        while (has_more == '') or (has_more == 'true'):
            ...
            for item in util.findlist(interface_result.json, 'interface'):
                interface_type = item.find('$..interface-type')
                interface_name = item.find('$..interface-name')
                last_interface_type = interface_type
                last_interface_name = interface_name
                if "gigabitethernet" '' in interface_type or \
                    ...
                    if "gigabitethernet" in interface_type or 'ethernet'\
                            in interface_type:
                        interface_role = item.find('$..port-role')
                    else:
                        interface_role = "None"
                    if_name = item.find('$..if-name')
                    interface_state = item.find('$..if-state')
    ...
    def conversational_arp(self, **kwargs):
        ...
        if kwargs.pop('get', False):
            ...
            conversational = item.find('$..conversational')
