...
class Asset(object):
    ...
    def _rest_operation(self, rest_commands=None, yang_list=None, timeout=None):
        ...
        for rest_cmd in rest_commands:
            ...
            self._overall_status.append({self._ip_addr : {'request': {'op_code': rest_cmd[0], 'uri': rest_cmd[1], 'data': rest_cmd[2]}, 'response': {'status_code': self._response.status_code, 'url': self._response.url, 'text': self._response.text, 'json': text_response}}})
    ...
    def _update_fw_version(self):
        ...
        try:
            ...
            fw = rest_root.find('show-firmware-version').find('os-name')
            if fw is not None:
                if 'Network Operating System' in fw.text:
                    self._os_type = 'nos'
                elif 'SLX' in fw.text:
                    self._os_type = 'slxos'
            ...
            ver = rest_root.find('show-firmware-version').find('os-version')

            if ver is not None:
                self._os_ver = ver.text
            full_ver = rest_root.find('show-firmware-version').find('firmware-full-version')
    ...
    def get_dict_output(self):
        ...
        """
        return self._overall_status[0][self._ip_addr]['response']['json']
