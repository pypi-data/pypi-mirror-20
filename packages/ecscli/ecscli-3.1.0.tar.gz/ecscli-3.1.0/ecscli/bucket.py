...
class Bucket(object):
    ...
    def bucket_create(self, bucket_id, fs_enable, headtype, namespace, args):
        ...
        if profVers >= 3.1:
            if (args.tso_read_only is not None):
                parms['tso_read_only'] = args.tso_read_only
            if (args.owner is not None):
                parms['owner'] = args.owner

            if (args.enforce_retention is not None):
                parms['enforce_retention'] = args.enforce_retention
            if (args.minimum_fixed_retention is not None):
                parms['minimum_fixed_retention'] = args.minimum_fixed_retention
            if (args.max_fix_ret is not None):
                parms['max_fix_ret'] = args.max_fix_ret
            if (args.min_var_ret is not None):
                parms['min_var_ret'] = args.min_var_ret
            if (args.max_var_ret is not None):
                parms['max_var_ret'] = args.max_var_ret
...
def create_parser(subcommand_parsers, common_parser):
    ...
    profVers = config.get_profile_version()
    if profVers >= 3.1:
        create_parser.add_argument('-tso_read_only', '-tsoro',
                                    help='flag indicating whether encryption is enabled for bucket. Default is false',
                                    dest='tso_read_only', metavar='<is_tso_read_only>',
                                    default='false',
                                    choices=['true', 'false'])
...
def min_max_gov_exist(args):
    exists = False
    if ((args.enforce_retention is not None) or (args.enforce_retention is not None) or (args.enforce_retention is not None) or
        (args.enforce_retention is not None) or (args.enforce_retention is not None)):
        exists = True
    return exists
