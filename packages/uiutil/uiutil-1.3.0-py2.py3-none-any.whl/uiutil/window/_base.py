...
class BaseWindow(object):
    ...
    def __init__(self,
        ...
        self.configure(background=u'#E7E7E7')
        ...
        self._main_frame = BaseFrame(self, padding=u'10 10 10 10' if self.padding is None else self.padding)
    ...
    def __position(self):
        ...
        if self.parent_geometry:
            ...
            parent_x_center = None
            parent_y_center = None
    ...
    def __validate_position(self):
        ...
        if (self.x + self.width) > self.winfo_screenwidth():
            self.x = self.winfo_screenwidth() - self.width - 10

        if self.x < 0:
            self.x = 0

        if (self.y + self.height) > self.winfo_screenheight():
            self.y = self.winfo_screenheight() - self.height - 10
    ...
    def modal_window(self,
        ...
        window.transient()
        window.grab_set()
        self._main_frame.wait_window(window)
