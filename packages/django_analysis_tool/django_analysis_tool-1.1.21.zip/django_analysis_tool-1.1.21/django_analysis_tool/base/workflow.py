...
class DetectUnzip(object):
    ...
    def runner_zip(self):
        ...
        #todo self.zip支持多个备选项，每个备选项使用“，”分隔
        zip_list = [i.strip() for i in self.zip.split(",") if i.strip() != "" and i.strip() != " "]

        for input_zip in zip_list:
            if (input_zip == 'com.kunpeng.creeper.default') or (self.zip.strip() == ''):
                continue
                # self.archive.close()
                # return

            #此处还是重新利用self.zip 对象，后面的线程池解析流程作为默认参数使用
            self.zip = input_zip
    ...
    def _thread_executor_xls(self, device):
        ...
        pattern = re.compile(self.xls.strip() + "$", re.I)
