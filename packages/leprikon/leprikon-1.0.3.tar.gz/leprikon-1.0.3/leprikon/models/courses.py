...
class Course(Subject):
    ...
    journal_print_setup = models.ForeignKey(PrintSetup, on_delete=models.SET_NULL, related_name='+',
                                            verbose_name=_('journal print setup'), blank=True, null=True)
    ...
    @cached_property
    def final_journal_print_setup(self):
        return (
            self.journal_print_setup or
            self.subject_type.journal_print_setup or
            LeprikonSite.objects.get_current().journal_print_setup or
            PrintSetup()
        )

    def get_journal_pdf(self):
        output = BytesIO()
        self.write_pdf(output)
        output.seek(0)
        return output.read()

    def write_journal_pdf(self, output):
        # get plain pdf from rml
        template = select_template([
            'leprikon/coursejournal/{}.rml'.format(self.subject_type.slug),
            'leprikon/coursejournal/course.rml',
        ])
        rml_content = template.render({'course': self})
        for i, line in enumerate(rml_content.split('\n')):
            print i+1, line
