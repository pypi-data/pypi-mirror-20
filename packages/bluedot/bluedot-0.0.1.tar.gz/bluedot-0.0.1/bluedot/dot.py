...
class BlueDot():
    """
    Represents a bluetooth input in the form of a dot (or button) which 
    communicates when and where it has been touched. 
...
if __name__ == "__main__":
    ...
    try:
        while True:
            #print("is_pressed {}".format(dot.is_pressed))
            sleep(0.1)
    finally:
        dot.stop()
...
"""
    ...
    dot.wait_for_move()
    print("waiting for press timeout")
    if dot.wait_for_press(5):
        print("waited for press")
    else:
        print("timed out waiting for press")
"""