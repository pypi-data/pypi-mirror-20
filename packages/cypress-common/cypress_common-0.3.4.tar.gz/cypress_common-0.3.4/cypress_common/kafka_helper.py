...
class PythonKafkaProducer(CypressBase):
    ...
    def send_message(self, topic, msg, unittest=False):
        """
        ...
        while True:  # loop until the message is send out
            ...
            try:
                if unittest:
                    sleep(2)
                evt_sent = self.kafka_producer_client.send(topic, msg).get(timeout=1)
...
class KafkaProducerBase(Thread, CypressBase):
    ...
    def run(self):
        while self.running:
            topic, msg = self.message_queue.get(True)
            ...
            try:
                ...
                self.logger.debug("Msg is sent to topic[{0}]".format(topic))
                # trigger the io loop to update, timeout value is set to 2.0 seconds.
                evt_sent = self.producer.poll(1.0)
                self.logger.debug("Send message spends {0}s".format(time() - start_time))
                if evt_sent > 0:
                    continue
                # if the message is not sent out, create a new producer.
                self.producer = None
