...
class CypressCache(CypressBase):
    ...
    def add_image_to_profiles(self, image_id, category_id, np_bytes, axis0, axis1, expire=120):
        """
        ...
        :type axis1: int
        :param expire: Set an expire flag on key name(profile:np:<image_id1>) for xx seconds. Default to 120s.
    ...
    def add_image_to_targets(self, image_id, np_bytes, axis0, axis1, expire=120):
        """
        ...
        :param axis1: numpy array dimension, the second element in the tuple
        :type axis1: int
        :param expire: Set an expire flag on key name(target:np:<image_id1>) for xx seconds. Default to 120s.
        :type expire: can be represented by an integer or a Python timedelta object. If it's None, then no expire.
    ...
    def add_to_failed_images(self, task_id, image_id, err_code):
        """
        Add image id to failed image with err_code for db import service.
        :param task_id: the id of the task to add to
        :type task_id: basestring or uuid
        :param image_id: the id of the image to be added into the list
        :type image_id: basestring or uuid
        :param err_code: the error code of the failed image
        :type err_code: int
        :return: True if add successfully, False if add failed
        """
        if err_code not in ERR_CODE:
            self.logger.error("invalid error code. error code can only be 0 to 5")
            return
        status = self.redis.rpush(DATA_IMPORT_HASH_NAME+":{0}:{1}".format(str(task_id), err_code), str(image_id))
    ...
    def set_db_import_status(self, task_id, hash_mapping):
        ...
        :return: True if set successfully, False if set failed
        """
        status = self.set_mkv_to_hash(DATA_IMPORT_HASH_NAME + ":{}".format(str(task_id)), hash_mapping)
        return status == 1 or status == 0

    def delete_failed_images(self, task_id):
        """
        Delete all failed images list for a task from database
        :param task_id: the id of the task to remove
        :type task_id: basestring or uuid
        """
        task_id = str(task_id)
        for err_code in ERR_CODE:
            hash_name = DATA_IMPORT_HASH_NAME + ":{}:{}".format(str(task_id), str(err_code))
            if not self.is_key_exists(hash_name):
                self.logger.warning('failed images for err_code {} does not exist: {}'.format(str(err_code), str(task_id)))
            else:
                self.delete_keys(hash_name)
    ...
    def get_all_db_import_task_id(self):
        ...
        """
        task_ids = []
        for keys in self.redis.scan_iter(match='data_import:*'):
            if not keys.endswith(":0") and not keys.endswith(":1") and not keys.endswith(":2"):
                task_ids.append(keys.split(":")[-1])
        return task_ids

    def get_all_failed_image_by_type(self, task_id, err_code):
        """
        Get a list of  image id of failed images by err_code for db import service.
        :param task_id: the id of the task to add to
        :type task_id: basestring or uuid
        :param err_code: the error code of the failed image
        :type err_code: int
        ...
        if err_code not in ERR_CODE:
            self.logger.error("invalid error code. error code can only be 0 to 5")
            return
        return self.redis.lrange("{}:{}:{}".format(DATA_IMPORT_HASH_NAME, str(task_id), str(err_code)), 0, -1)
