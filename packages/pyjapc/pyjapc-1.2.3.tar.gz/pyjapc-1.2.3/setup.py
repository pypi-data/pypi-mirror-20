...
def pyjapc_version():
    '''Get pyjapc version from local __init__.py'''
    init = os.path.join(os.path.dirname(__file__), 'pyjapc', '__init__.py')
...
class install(_install):
    ...
    def run(self):
        if not self.no_jars:
            ...
            mgr.install('pyjapc')
            print('registered pyjapc with cmmnbuild_dep_manager')
        _install.run(self)
