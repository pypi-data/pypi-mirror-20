...
class Service(object):
    ...
    def __init__(self):
        ...
        if self.name in settings:
            for key, value in settings[self.name].items():
                setattr(self, key, value or self.default_settings.get(key, None))

            supplied_settings = self.__dict__
        else:
            supplied_settings = self.default_settings

        require(msg='Service `{}` requires settings to be set-up before use.'.format(self.name), **supplied_settings)
    ...
    def setup(self):
        ...
        with open(SETTINGS_FILENAME, 'w') as fp:
            dump(data, fp, indent=3)

    @log('List {list_name} IP addresses...', list_name='all')
