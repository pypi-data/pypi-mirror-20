...
def upload_pic(request,f=None,name=None,resize=None,limit_size=20,limit_file_type=('jpg','jpeg','bmp','png','gif','tif')):
    '''上传图片并返回一个字典格式的结果，格式如下：
            返回格式：
            {
            'pic':pic,
    ...
    try:
        ...
        if f:
            ...
            if resize:
                import image
                image.create_small_pic_from_file(file=real_file,dst_w=resize[0],dst_h=resize[1],dst_img=real_file,is_thumbnails=True)
