...
class Scanner():
    ...
    def __init__(self, locale, re_flags=re.IGNORECASE | re.MULTILINE):
        ...
        self.YYYY_MM_DD = scan_product(END, RYEAR, SEP, RMONTH, SEP, RDAY)
        self.YYYYMMDD = scan_product(END, RYEAR, RMONTH, RDAY)
        self.DD_MM_YYYY = scan_product(END, RDAY, SEP, RMONTH, SEP, RYEAR)
        self.AMBIGUOUS_DD_DD_YYYY = scan_product(
            END, RDAY, ["[./ -]"], RDAY, ["[./ -]"], RYEAR + ["[01289][0-9]"])
        self.DDMMYYYY = scan_product(END, RDAY, RMONTH, RYEAR)
        self.YYYY = scan_product(END, RYEAR)
        self.AMBIGUOUS_DD_DD = scan_product(
            ["on ", "the ", "for ", "from ", "until "], RDAY, ["[./ -]"], RDAY, END, ["(?![./0-9-][./0-9-])"])
        self.DDMM = scan_product(END, [r"\d{1,2}"], [" ?..", ""], [
                                 " of ", " ", "-", ""], self.MONTHS)
        self.DDMMS = scan_product(END, [r"\d{1,2}"], [" ?..", ""], [
        ...
        self.scanner = re.Scanner([
            ...
            (pipe(self.UNITS, r'\b', r'\b'), lambda y, x: MetaUnit(
                self.UNITS[x], span=y.match.span())),
