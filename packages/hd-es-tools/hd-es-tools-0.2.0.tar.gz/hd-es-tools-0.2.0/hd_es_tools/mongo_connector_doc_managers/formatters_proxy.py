...
class DocumentFormatterProxy(DocumentFormatter):
    def __init__(self, kwargs):
        self.format_option = kwargs

        def import_default_formatter():
            full_name = "hd_es_tools.mongo_connector_doc_managers.formatters.DefaultDocumentFormatter"
            return import_formatter_by_path(full_name)

        def import_formatter_by_path(path):
            try:
                ...
                if not issubclass(dm_impl, DocumentFormatter):
                    raise TypeError("DocumentFormatterProxy => Formatter must inherit DocumentFormatter.")
                return dm_impl
            except ImportError:
                raise Exception(
                    "DocumentFormatterProxy => Could not import %s " % package)
                sys.exit(1)
            except (AttributeError, TypeError):
                raise Exception(
                    "DocumentFormatterProxy => No definition for Formatter found in %s." % package)
                sys.exit(1)

        if 'formatterClassPath' in self.format_option:
            Formatter = import_formatter_by_path(self.format_option['formatterClassPath'])
        else:
            Formatter = import_default_formatter()

        self.formatter = Formatter(self.format_option)
    ...
    def format_document(self, doc_id, document, namespace):
        res = self.formatter.format_document(doc_id, document, namespace)
        LOG.debug("format_document %r", res)
        return res
...
class DefaultDocumentFormatter(DocumentFormatter):
    ...
    def transform_value(self, value, namespace):
        ...
        if isinstance(value, dict):
            return self.format_document(None, value, namespace)
    ...
    def transform_key(self, key, namespace):
        if namespace in self.format_option:
            fmap = self.format_option.get(namespace, {}).get('rename', {})
            if key in fmap:
...
class DocumentFlattener(DefaultDocumentFormatter):
    ...
    def transform_element(self, key, value, namespace):
        if isinstance(value, list):
            ...
            formatted = self.format_document(None, value, namespace)
