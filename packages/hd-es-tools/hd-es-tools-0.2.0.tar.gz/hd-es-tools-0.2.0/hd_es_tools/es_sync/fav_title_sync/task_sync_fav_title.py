...
SYNC_HOME = '/opt/es/es_sync/fav_title_sync'
SYNC_HOME = dirname
...
ES_HOST = ['elastic:changeme@10.0.10.70:9200', 'elastic:changeme@10.0.10.73:9200', 'elastic:changeme@10.0.10.75:9200']
ES_INDEX = 'fav'
ES_INDEX_TYPE = 'fav'
...
class TaskSyncFavTitle(object):
    ...
    def getObj(self, doc):
        try:
            ...
            if url is None:
                LOG.warn("getObj[%s]|[type[%d] not found]", json.dumps(doc), type)
                return None

            url = url % id
            req = urllib2.Request(url)
            res_data = urllib2.urlopen(req, data=None, timeout=0.5)
            res = res_data.read()
            res_json = json.loads(res)

            if res_json.get("e", {}).get("code", -1) == -102:
                LOG.warn("getObj[%s]|[-102]", json.dumps(doc))
                return None

            data_json = res_json.get("data", None)

            if data_json is None:
                raise Exception('getObj error res[%r]' % res)
            ...
            return data_json.get("title", None)

        except Exception as e:
            LOG.error("getObj[%s]|error[%r]", json.dumps(doc), e)
            raise e

    def syncFromDocs(self):
        for doc in self.docs:
            try:
                objTitle = self.getObj(doc)
    ...
    def saveErrorDoc(self, docs=[], file=None):
        try:
            ...
            if len(error) > 0:
                ...
                LOG.info("--saveErrorDoc count[%d][%s]", len(docs), file)
                fp = open(file, 'a')
                fp.write(error)
                fp.close()
        except Exception as e:
            LOG.error("saveErrorDoc error:[%r][%r]", e, json.dumps(docs))
            pass
            ...
            try:
                ...
                for line in fp.readlines():
                    try:
                        ...
                        LOG.error("doc[%s]|error[%r]", line, err)
                fp.close()
                os.remove(self.file)
                self.syncFromDocs()
            except Exception as err:
