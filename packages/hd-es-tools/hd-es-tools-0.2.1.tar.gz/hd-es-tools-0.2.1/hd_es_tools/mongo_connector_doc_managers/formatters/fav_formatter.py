...
class FavDocumentFormatter(DefaultDocumentFormatter):
    ...
    def format_document(self, doc_id, document, namespace):
        ...
        try:
            if doc_id is not None and 'objId' in res and 'objType' in res:
                try:
                    objTitle = self.sync.getObj(res)
                    if objTitle is not None:
                        res['objTitle'] = objTitle
                except Exception as e:
                    self.sync.saveErrorDoc([{"_id": doc_id, "objId": res['objId'], "objType": res['objType']}], None)
