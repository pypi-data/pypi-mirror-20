...
class Cli:
    ...
    Examples:
        ...
        4. %s cc cluster3 --notification my-mns-topic:OnClusterDeleted,OnInstanceActive    # set notification
        5. %s cc cluster4 --resource_type Spot  # creating with spot resource type''' % (
                                     COMMAND, CMD, CMD, CMD, CMD, CMD, IMG_ID, INS_TYPE),
                                     spliter=' ',
                                    # spliter='\n    -----%s----------------' % blue('create, update, delete'),
    ...
    Examples:
        ...
        7. %s sub --notification my-mns-topic:OnTaskStopped,OnJobFailed           # set notification
        ''' % (
