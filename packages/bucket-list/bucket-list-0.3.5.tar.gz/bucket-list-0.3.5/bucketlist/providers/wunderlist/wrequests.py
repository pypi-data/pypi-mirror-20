...
class WRequests:
    ...
    def get(*args, **kwargs):
        ...
        for _ in range(WRequests.MAX_RETRIES):
            try:
                resp = requests.get(*args, **kwargs, headers=headers).json()
    ...
    def post(*args, **kwargs):
        ...
        for _ in range(WRequests.MAX_RETRIES):
            try:
                resp = requests.post(*args, **kwargs, headers=headers).json()
            except json.decoder.JSONDecodeError:
                resp = None
            else:
                break
    ...
    def patch(*args, **kwargs):
        ...
        for _ in range(WRequests.MAX_RETRIES):
            try:
                resp = requests.patch(*args, **kwargs, headers=headers).json()
            except json.decoder.JSONDecodeError:
                resp = None
            else:
                break
