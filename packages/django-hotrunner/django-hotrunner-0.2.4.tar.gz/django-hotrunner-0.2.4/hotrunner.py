...
class HotRunner(DjangoTestRunner):
...
class HotRunnerTestResult(TextTestResult):
    ...
    def getDescription(self, test):
        ...
        parts = re.findall('[\w_]+', smart_unicode(test))
        name = u"{testname:s} ({app:s}.{suite:s}.{testname:s})".format(
            app=parts[-3], suite=parts[-1], testname=parts[0])
        
        if self.descriptions and description:
            name = u"{name:s}\n{description:s}".format(
                name=name, description=smart_unicode(description))
        return name
    ...
    def _add_tb_to_test(self, test, test_result, err):
        ...
        test_result.set('message', smart_unicode(exc_value))
        test_result.text = smart_unicode(tb_str)
