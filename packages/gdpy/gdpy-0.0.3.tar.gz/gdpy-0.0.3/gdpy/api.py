...
class Tasks(_Base):
    ...
    def get_task(self, id):
        ...
        """
        super(Tasks, self).__init__(self.auth, self.endpoint, 'tasks', None)
    ...
    def active_workflow(self, param_file, workflow_name, workflow_version, workflow_owener=None):
        """
        usage::
            >>> resp = task.active_workflow('workflow_param_file', 'workflow_name', 'workflow_version')
        """
        if not workflow_owener:
            workflow_owener = self.res_account_name
        ...
        try:
            ...
            data["workflow_owener"] = workflow_owener
...
class Workflows(_Base):
    ...
    def list_workflows(self):
        ...
        super(Workflows, self).__init__(self.auth, self.endpoint, 'workflows', None)
    ...
    def list_exc_workflows(self):
        ...
        """
        super(Workflows, self).__init__(self.auth, self.endpoint, 'executable-workflows', None)
        resp = self.__do_workflow('GET')
        return self._parse_result(resp, yml_utils.parse_list_workflows, ListWorkflowsResult)
...
class Tools(_Base):
    ...
    def get_tool(self, name, version=None):
        ...
        if name is None:
            raise ValueError("Expect a name(str) started with alphabet and under 128 characters")
        super(Tools, self).__init__(self.auth, self.endpoint, 'tools', None)
    ...
    def get_tool_param(self, name, version=None):
        ...
        if name is None:
            raise ValueError("Expect a name(str) started with alphabet and under 128 characters")
        super(Tools, self).__init__(self.auth, self.endpoint, 'toolparameters', None)
        resp = self.__do_tool('GET', name, version)
