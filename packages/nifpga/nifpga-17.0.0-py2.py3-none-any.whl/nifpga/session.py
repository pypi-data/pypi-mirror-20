...
class Session(object):
    ...
    FIFOs are accessed directly via a _FIFO object obtained from the session::
        ...
        data, elements_remaining = myFpgaToHostFifo.read(number_of_elements=4,
                                                         timeout_ms=0)

        myP2PFifo = session.fifos["MyP2PFifo"]
        p2p_handle = myP2PFifo.get_peer_to_peer_endpoint("MyP2PFifo")

    IRQs can be used like this::

        (irqs_asserted, timed_out) = session.wait_on_irqs(irqs=[0, 17, 31],
                                                          timeout_ms=2)
        session.acknowledge_irqs(irqs=[0, 17, 31])
    """
