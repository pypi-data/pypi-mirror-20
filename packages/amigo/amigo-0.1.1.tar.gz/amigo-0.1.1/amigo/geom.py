...
def theta_sort(R,Z,origin='lfs',**kwargs):
    ...
    if origin == 'lfs':
        ...
        theta = np.arctan2(R-xo[0],xo[1]-Z) 
...
def rotate_vector2D(V,theta):
    ...
    R = rotate2D(theta)[0]
    Vhat = (R*np.matrix([[V[0]],[V[1]]])).getA1()/Vmag
    return Vhat
...
def offset(R,Z,dR,min_steps=5,close_loop=False):
    R,Z = order(R,Z)  # enforce anti-clockwise
    dr_max = np.mean(dR)/min_steps  # maximum step size
    if np.mean(dR) != 0:
...
def cut(r,z):  # trim gaps
    for _ in range(len(r)-1):
        dl = np.diff(length(r,z))
