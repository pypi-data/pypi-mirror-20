...
import gdpy
from gdtools import login
from gdtools import authpolicy
...
CONFIGFILE = os.path.join(os.path.expanduser('~'), '.gdtools_configuration')
SECTION = 'Defaults'
...
def read_configuration(configfile):
    ...
    if not read_ok:
        ...
        print('Using: "gdtools config" or "gdtools config -e [endpoint] -i [access_id] -k [access_key] -a [account_name] -u [user_name]".', file=sys.stderr)
        sys.exit(1)
...
def setup_configuration(args):
    ...
    if args.ls:
        ...
        _logger.warning('Missing parameters in command-line gdtools config.')
        print('Missing parameters in command-line gdtools config!', file=sys.stderr)
        print('Using: gdtools config -e [endpoint] -i [access_id] -k [access_key] -a [account_name] -u [user_name]', file=sys.stderr)
        sys.exit(-1)
...
def list_task(args):
    ...
    try:
        resp = task.list_tasks(params={'size': args.size})
        table = json2table(resp.response.text, 'task_list', ['task_id', 'name', 'status', 'progress_rate', 'startTime', 'endTime'], None)
...
def update_tool(args):
    ...
    try:
        ...
        if e.status // 100 == 5:
            ...
            print('Please check the content of the input "config file" or use "gdtools tool get" command to obtain the tool original configuration file and remodify it', file=sys.stderr)
    except ValueError as e:
...
def active_workflow(args):
    ...
    try:
        ...
        print('Failed to active the workflow!\nstatus: {}\nerror: {1}'.format(e.status, e.error_message.decode('utf-8')), file=sys.stderr)
        print('NetWork Connection Error, please check your network and retry.', file=sys.stderr)
    except gdpy.exceptions.ServerError as e:
...
def update_workflow(args):
    ...
    try:
        ...
        if e.status // 100 == 5:
            ...
            print('Please check the content of the input "config file" or use "gdtools workflow get" command to obtain the workflow original configuration file and remodify it', file=sys.stderr)
    except ValueError as e:
...
def main():
    parser = argparse.ArgumentParser(description='GeneDock command-line toolkit for interacting with GeneDock platform.')
