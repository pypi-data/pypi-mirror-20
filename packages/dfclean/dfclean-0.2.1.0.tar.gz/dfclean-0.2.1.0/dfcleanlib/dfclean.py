...
def clean():
    ...
    for file_path, file_size in files_for_clean():
        if can_use_f_string():
            print(f'[ {human_size(file_size):>10} ] {file_path}')
        else:
    ...
    print('=' * 120)
    if can_use_f_string():
        print(f'[ {human_size(total_size):>10} ] Total size')
        print(f'[ {human_size(removed_size):>10} ] Removed size')
    else:
