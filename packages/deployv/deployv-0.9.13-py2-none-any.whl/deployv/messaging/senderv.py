...
from deployv.messaging.rabbitv import BaseRabbitConfiguration
from deployv.messaging import messagev
...
class RabbitSenderV(object):
    ...
    def __init__(self, sender_configuration, sender_id):
        ...
        self.__config = sender_configuration
        self.__id = sender_id
    ...
    def amq_send_message(self, message, send_to=None):
        ...
        logger.debug('Sending message (%s): %s', routing_key, message)
        retry = 0
        while retry <= 3:
            try:
                ...
                retry += 1
                logger.info('Connection failed, retry %s', retry)
                if retry == 3:
                    raise
                else:
                    self.reconnect()
    ...
    def send_message(self, message, send_to=None):
        ...
        """
        assert isinstance(message, messagev.BasicMessage) or isinstance(message, dict)

        if isinstance(message, messagev.BasicMessage):
