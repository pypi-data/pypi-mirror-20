...
def download_torrent_file(url, destination, filename):
    ...
    if '?' in url:
        url = str(url.split('?')[0])
    rdata = requests.get(url, headers=http_header, proxies=_proxies)
...
def download_rss_feed(url):
    ...
    :raises: BSError
    """
    global _logger, http_header, _proxies
    if url.startswith('http') is False:
        raise BSError('Not a url', url)
    try:
        rdata = requests.get(url, headers=http_header, timeout=5, proxies=_proxies)
...
def cli(**kwargs):
    ...
    """
    global _logger, config, _proxies, __cfgfile__
    if kwargs['debug']:
    ...
    try:
        ...
        if sorted(_proxies.keys()) != ['http', 'https']:
            raise click.UsageError('Proxy requires one of: (http, https)')
        _logger.debug('Proxies: ' + str(_proxies))
    except (KeyError, ValueError):
        pass

    try:
        ipv6 = bool(config['app']['ipv6'])
        if not ipv6:
