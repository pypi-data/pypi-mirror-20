...
class Expr(_coconut.object):
    ...
    def contradicts(self, other, **kwargs):
        ...
        if isinstance(other, Not):
            raise _coconut_tail_call((other.contradicts), self)
        else:
...
class Atom(Expr):
    ...
    def substitute(self, subs):
        try:
            ...
            raise _coconut_tail_call(self.substitute_elements, subs)
        else:
            if wff(sub):
                return sub
            elif sub is True:
...
class FuncAtom(Atom):
    ...
    def __repr__(self):
        return self.name + "(" + ", ".join((repr(x) for x in self.args)) + ")"
    def __eq__(self, other):
...
class Predicate(FuncAtom):
    ...
    def substitute_elements(self, subs):
        try:
            ...
            raise _coconut_tail_call((_coconut.functools.partial(Predicate, self.name)), *(_coconut.functools.partial(map, _coconut.operator.methodcaller("substitute", subs)))(self.args))
        else:
...
class Function(Term, FuncAtom):
    ...
    @_coconut_tco
    def substitute_elements(self, subs):
        raise _coconut_tail_call((_coconut.functools.partial(Function, self.name)), *(_coconut.functools.partial(map, _coconut.operator.methodcaller("substitute", subs)))(self.args))
    @_coconut_tco
    def rename(self, name):
...
class UnaryOp(Expr):
    ...
    @_coconut_tco
    def substitute(self, subs):
        raise _coconut_tail_call(self.__class__, self.elem.substitute(subs))
    @_coconut_tco
    def find_unification(self, other):
...
class Quantifier(Expr):
    __slots__ = ("var", "elem")
    def __repr__(self):
        return self.__class__.__name__ + "(" + repr(self.var) + ", " + repr(self.elem) + ")"
    def __str__(self):
        return self.opstr + str(self.var) + ". " + quote(self.elem)
    def __len__(self):
    ...
    def substitute(self, subs):
        try:
            ...
            var = self.var
        raise _coconut_tail_call(self.__class__, var, self.elem.substitute(subs))
    @_coconut_tco
    ...
    def simplify(self, **kwargs):
        ...
        if simp_elem == simp_elem.substitute({self.var: self.var.prime()}):
            ...
            raise _coconut_tail_call(self.__class__, self.var, simp_elem)
...
class BinaryOp(Expr):
    ...
    @_coconut_tco
    def substitute(self, subs):
        raise _coconut_tail_call((self.__class__), *(_coconut.functools.partial(map, _coconut.operator.methodcaller("substitute", subs)))(self.elems))
    ...
    def prenex(self, **kwargs):
        ...
        for i, x in enumerate(self.elems):
            if isinstance(x, Quantifier):
                ...
                elems += (free_x.elem,)
                raise _coconut_tail_call((_coconut.functools.partial(free_x.__class__, free_x.var)), self.__class__(*elems).simplify(**kwargs))
        return self
