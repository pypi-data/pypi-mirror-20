...
Aegea Batch Lambda supervisor
-----------------------------
- runs on event: batch job entered terminal state OR batch job has exceeded timeout
- cleans up unattached EBS volumes
- enters job metadata into dynamo

  ``batch ls job-id``

- retrieve job metadata from dynamo

Aegea multi-user environment Lambda supervisor
----------------------------------------------
- runs periodically or when cloudwatch alarm on EBS usage is triggered
- dynamically expands EBS volumes
- 
