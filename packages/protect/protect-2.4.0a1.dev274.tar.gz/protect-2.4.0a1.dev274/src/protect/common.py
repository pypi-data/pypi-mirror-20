...
def get_file_from_s3(job, s3_url, encryption_key=None, per_file_encryption=True,
    ...
    elif parsed_url.scheme == 's3' or parsed_url.scheme == 'S3':
        download_url = s3_url
    else:
...
def get_file_from_url(job, any_url, encryption_key=None, per_file_encryption=True,
    ...
    try:
        ...
        if (parsed_url.netloc).startswith('s3') or (parsed_url.netloc).startswith('S3'):
            job.fileStore.logToMaster("Detected https link is for a encrypted s3 file.")
            filename = get_file_from_s3(job, any_url, encryption_key=encryption_key,
