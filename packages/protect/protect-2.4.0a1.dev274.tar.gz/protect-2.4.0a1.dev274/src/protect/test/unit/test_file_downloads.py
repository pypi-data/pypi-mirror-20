...
class TestFileDownloads(ProtectTest):
    ...
    def test_file_downloads_from_https_fail(self):
        """
        Test the functionality of get_file_from_url https
        """
        a = Job.wrapJobFn(self._download_https_files_fail)
        Job.Runner.startToil(a, self.options)

    @staticmethod
    def _download_https_files_fail(job):
        """
       Attempts to download an unencrypted file, a file encrypted with a key.

        """
        http_base = 'https://thisprettymuchdoesnotleadanywhere.com'
        get_file_from_url(job, http_base, write_to_jobstore=False)
...
# noinspection PyProtectedMember
_download_https_files_fail = TestFileDownloads._download_https_files_fail
# noinspection PyProtectedMember
