...
def get_file_from_url(job, any_url, encryption_key=None, per_file_encryption=True, write_to_jobstore=True):
    """
    Downloads a supplied URL that points to a file. The file
    ...
    filename = '/'.join([work_dir, os.path.basename(any_url)])

    try:
        urllib.urlretrieve(any_url, filename)
    except:
        if 'S3' in any_url:
            get_file_from_s3(job, any_url, encryption_key=encryption_key, per_file_encryption=per_file_encryption,
