...
class CustomChoiceTypeField(serializers.Field):
  '''
    Serializer field for ChoiceTypeField, returns a tupple with both real value on [0] and get_{field_name}_display on [1].
    to_internal_value uses only de first value os the tupple if a tupple, or just the value if not a tupple
  '''
...
class CustomNestedSerializer(serializers.Field):
    """
      Uses the seriazlizer to create the object representation, but only uses its pk to get
      object to its internal value.
    """
