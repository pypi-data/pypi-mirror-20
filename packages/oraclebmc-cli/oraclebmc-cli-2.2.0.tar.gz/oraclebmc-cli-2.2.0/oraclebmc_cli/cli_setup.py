...
from .packages.oraclebmc.regions import is_region
from .packages.oraclebmc import config
...
upload_public_key_instructions = """
    \n
    If you haven't already, follow these instructions to upload your public key:
...
password_file_prefix = 'file:'
password_pass_prefix = 'pass:'
...
@setup_group.command('keys', help="""Generates an RSA key pair. Prompts for a passphrase to (optionally) encrypt the private key.""")
...
def generate_key_pair(key_name, output_dir, overwrite):
...
def validate_public_key_file(filename):
    if not os.path.isfile(filename):
        raise click.BadParameter('No file found at: {}'.format(filename))
    else:
        with open(filename, 'rb') as f:
            try:
                return serialization.load_pem_public_key(data=f.read(), backend=default_backend())
            except ValueError:
                raise click.BadParameter('Could not load file as public key. File: {}'.format(filename))
