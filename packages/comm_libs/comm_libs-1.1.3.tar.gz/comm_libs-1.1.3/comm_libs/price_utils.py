...
def fen2yuan(price_fen):
    """
    分转元
    """
    yuan_part = int(price_fen) / 100

    fen_part = int(price_fen) % 100

    return "%d.%02d" % (yuan_part, fen_part)


# 人民币大写转换类
# 代码来自互联网
# 使用例子:
# from price_utils import cnumber
#
# cnumber().cwchange("11.20")
# 注意参数必须是 xx.yy 格式
class cnumber:
    cdict = {}
    gdict = {}
