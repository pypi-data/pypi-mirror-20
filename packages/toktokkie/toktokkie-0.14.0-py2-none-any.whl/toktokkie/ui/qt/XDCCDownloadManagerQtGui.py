...
class XDCCDownloadManagerQtGui(QMainWindow, Ui_XDCCDownloadManagerWindow):
    ...
    progress_updater_signal = pyqtSignal(float, float, int, int, name=u"progress_updater")
    download_queue_refresh_signal = pyqtSignal(name=u"download_queue_refresh_signal")
    spinner_updater_signal = pyqtSignal(QPushButton, unicode, name=u"spinner_updater")
    show_download_completed_signal = pyqtSignal(dict, name=u"show_download_completed")
    ...
    def browse_for_directory(self):  # pragma: no cover
        u"""
    ...
    def start_download(self):
        ...
        def handle_download():
            ...
            results = MultipleServerDownloader(u"random").download(packs, progress)
            ...
            if self.iconize_check.checkState():
                ...
                if iconization_method != u"":  # pragma: no cover
            ...
            self.directory_edit.textChanged.emit(u"")
    ...
    def update_progress(self, single_progress, total_progress, current_speed, average_speed):
        u"""
    ...
    def start_spinner(self, spinner_type):
        ...
        def spin():
            ...
            if search and not self.searching:
                self.spinner_updater_signal.emit(self.search_button, u"Search")
            if download and not self.downloading:
                self.spinner_updater_signal.emit(self.download_button, u"Download")

        Thread(target=spin).start()

    # noinspection PyMethodMayBeStatic
    def generate_message(self, icon_type, window_title, text):
        u"""
    ...
    def show_download_completed_message_box(self, packs):
        u"""
        ...
        :return:      None
        """
        message = self.generate_message(QMessageBox.Information, u"Download Complete",
                                        u"The following packs were downloaded:")
