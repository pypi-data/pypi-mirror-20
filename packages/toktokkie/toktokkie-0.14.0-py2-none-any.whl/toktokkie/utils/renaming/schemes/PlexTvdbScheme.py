...
class PlexTvdbScheme(GenericScheme):
    ...
    def apply_scheme(self):
        ...
        episode_number = unicode(self.episode_number).zfill(2) if self.episode_number >= 0 \
            else unicode(self.episode_number).zfill(3)
        season_number = unicode(self.season_number).zfill(2) if self.season_number >= 0 \
            else unicode(self.season_number).zfill(3)

        return self.series_name + u" - S" + season_number + u"E" + episode_number + u" - " + episode_name
