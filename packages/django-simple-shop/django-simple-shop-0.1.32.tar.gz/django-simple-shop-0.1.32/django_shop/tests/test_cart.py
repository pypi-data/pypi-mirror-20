...
class CartTestCase(TestCase):
    ...
    def test_has_basket_data(self):
        session = {Cart.CART: [(self.p1.id, 1.3), (self.p2.id, 1)]}
        basket = Cart(session)
        ...
        self.assertEqual(session, {Cart.CART: [(self.p1.id, 1.3), (self.p2.id, 1)]})
    ...
    def test_basket_modification(self):
        ...
        self.assertEqual(session, {Cart.CART: [(self.p1.id, 1.5)]})
        ...
        self.assertEqual(basket.get_amount(self.p2), 1)
        self.assertEqual(session, {Cart.CART: [(self.p1.id, 1.5), (self.p2.id, 1)]})

        basket.set(self.p1, Decimal(1))
        self.assertEqual(basket.get_amount(self.p1), 1)
        self.assertEqual(session, {Cart.CART: [(self.p1.id, 1), (self.p2.id, 1)]})

        basket.remove(self.p2)
        self.assertEqual(basket.get_amount(self.p2), 0)
        self.assertEqual(session, {Cart.CART: [(self.p1.id, 1)]})
        ...
        self.assertEqual(session, {Cart.CART: [(self.p1.id, 1), (self.p2.id, 3)]})
