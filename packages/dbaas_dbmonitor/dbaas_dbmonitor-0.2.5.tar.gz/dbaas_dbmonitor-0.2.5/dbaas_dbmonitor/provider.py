...
class DBMonitorProvider(object):
    ...
    def create_instance(self, cursor, databaseinfra, dbmonitor_infraid, instance, instance_number, tipo_mongodb='2'):
        ...
        if sgbd == SGBD_MONGODB:
            sql = """INSERT INTO dbmonitor_instancia (database_id, nome, maquina, dns, porta, ativo, tipo_mongodb)
                    VALUES (%(database_id)s, '%(nome)s', '%(maquina)s', '%(dns)s', '%(porta)s', %(ativo)s, '%(tipo_mongodb)s')
            """ % instance_dict
        else:
            sql = """INSERT INTO dbmonitor_instancia (database_id, nome, maquina, dns, porta, ativo)
