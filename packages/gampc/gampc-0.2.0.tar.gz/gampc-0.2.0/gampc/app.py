...
class App(Gtk.Application):
    def __init__(self):
        ...
        self.add_main_option('dark', ord('d'), GLib.OptionFlags.NONE, GLib.OptionArg.NONE, _("Dark display (implies --non-unique)"), None)
        self.add_main_option('debug', 0, GLib.OptionFlags.NONE, GLib.OptionArg.NONE, _("Debug messages"), None)
    ...
    def do_handle_local_options(self, options):
        if options.contains('non-unique') or options.contains('dark'):
    ...
    def do_startup(self):
        ...
        self.shared.connect('notify::safe-mode', self.set_inhibit)
        ...
        self.add_action(action.Action('quit', self.quit_cb))
        ...
        self.sigint_source = GLib.unix_signal_add(GLib.PRIORITY_DEFAULT, signal.SIGINT, self.quit_cb)

    def do_command_line(self, command_line):
        options = command_line.get_options_dict().end().unpack()
        if 'dark' in options:
            css = Gtk.CssProvider.get_named('Adwaita', 'dark')
    ...
    def edit_profiles_cb(self, *args):
        ...
        value = struct.dialog(self.get_active_window(),
                              self.shared.config['server']['profiles'],
                              self.shared.config.setdefault('server', {}).setdefault(self.shared.CONFIG_EDIT_DIALOG_SIZE, [500, 500]))
        if value:
    ...
    async def action_notify_cb(self, *args):
        ...
        await self.ampd.idle(self.ampd.IDLE)
    ...
    def set_inhibit(self, *args):
        if self.shared.ampd_server_properties.state == 'play':
            ...
            self.systemd_inhibit_fd = self.systemd_inhibit_fd or obj.Inhibit('handle-lid-switch', __program_name__, _("Playing in safe mode"), 'block', dbus_interface='org.freedesktop.login1.Manager')
        else:
    ...
    async def idle_output(self, client):
        ...
        for output in outputs:
            ...
            self.add_action(action.Action('output.' + output_id, self.action_output_activate_cb, state=GLib.Variant.new_boolean(int(output['outputenabled']))))
        while True:
            await self.ampd.idle(self.ampd.OUTPUT)
            outputs = await self.ampd.outputs()
            for output in outputs:
    ...
    @ampd.task
    async def THIS_IS_BAD_cb(self, action, param):
        for i in range(1000):
            self.BAD2()
        print(5)
