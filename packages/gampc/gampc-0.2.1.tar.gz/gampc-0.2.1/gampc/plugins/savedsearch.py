...
class SavedSearch(songlist.SongListWithSeparator, songlist.EditableFileSongList, module.PanedModule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.config.setdefault(self.shared.CONFIG_EDIT_DIALOG_SIZE, [500, 500])
    ...
    def action_search_new_cb(self, *args):
        value = self.ssde_struct.dialog(self.win, size=self.config[self.shared.CONFIG_EDIT_DIALOG_SIZE], scrolled=True)
        if value:
    ...
    def edit_search(self, p):
        ...
        new_value = self.ssde_struct.dialog(self.win, value, size=self.config[self.shared.CONFIG_EDIT_DIALOG_SIZE], scrolled=True)
        if new_value:
        ...
        for i in reversed(range(len(songs) - 1)):
            for j in reversed(range(sep)):
                if songs[i][sort[j][0]] != songs[i + 1][sort[j][0]]:
                    songs[i + 1:i + 1] = (sep - j) * [self.separator_song]
                    break
        self.set_songs(songs, False)

    def action_reset_cb(self, *args):
...
class SearchDatabase(object):
    def __init__(self, shared):
        super(SearchDatabase, self).__init__()
        self.shared = shared
        self.get_connection()
        self.setup_database()

    def get_connection(self):
        self.connection = self.shared.sqlite_connection(SavedSearch.name)
    ...
    def repair_database(self):
        with self.connection:
            for name in self.get_search_names():
                search = self.get_search(name)
                changed = self.repair_strings(search['search'])
                if self.repair_strings(search['sort']):
                    changed = True
                if changed:
                    self.delete_search(name)
                    self.add_search(search)

    def repair_strings(self, l):
        changed = False
        for i, obj in enumerate(l):
            if isinstance(obj, list):
                changed = self.repair_strings(obj)
            elif i == 0 and isinstance(obj, str):
                if obj not in self.shared.songlist_fields.names and obj.capitalize() in self.shared.songlist_fields.names:
                    l[i] = obj.capitalize()
                    changed = True
        return changed
