...
class Tanda(module.PanedModule):
    ...
    def __init__(self, shared):
        ...
        self.problem_button = Gtk.ToggleButton(image=Gtk.Image(icon_name='dialog-warning-symbolic'), visible=True, can_focus=False, active=self.shared.safe_mode)
        ...
        self.signal_handler_connect(self.shared, 'notify::safe-mode', lambda shared, param_spec: shared.safe_mode and self.problem_button.set_active(True))
    ...
    def db_missing_song_ok_cb(button, db, search_module, song_file):
        ...
        if path:
            ...
            song = search_module.store.get_row(i).get_data()
...
class TandaSubModule(module.Module):
    ...
    def tandaid_cursor_changed_cb(self, treeview):
        ...
        self.current_tandaid = path and self.tandaid_store.get_row(self.tandaid_store.get_iter(path)).tandaid
...
class TandaEdit(TandaSubModule, songlist.EditableSongList):
    ...
    def __init__(self, shared):
        ...
        self.actions.add_action(action.Action('tanda-merge', self.action_tanda_merge_cb))

        self.tanda_treeview = data.DictTreeView(self.db.fields, self.tanda_data_func, True)
        ...
        scrolled_tanda_treeview = Gtk.ScrolledWindow(visible=True)
        scrolled_tanda_treeview.add(self.tanda_treeview)

        self.treeview.set_vexpand(False)
        self.scrolled_treeview.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.NEVER)

        self.box.pack_start(scrolled_tanda_treeview, True, True, 0)
        super().add(self.box)
    ...
    async def client_connected_cb(self, client):
        while True:
            ...
            await self.ampd.idle(self.ampd.PLAYLIST)

    @staticmethod
    def tanda_data_func(column, renderer, store, i, j):
        tanda = store.get_row(i)
        if tanda._modified:
    ...
    def set_current_tanda(self):
        ...
        if self.current_tanda_path is None:
            ...
            self.current_tanda = self.tanda_store.get_row(self.tanda_store.get_iter(self.current_tanda_path))
            self.find_duplicates(self.queue + self.current_tanda._songs, ['Title'])
    ...
    def editing_done_cb(self, editable, path, name):
        ...
        if value != self.current_tanda.get_data().get(name, ''):
            if value:
                self.current_tanda.__setattr__(name, value)
            else:
                self.current_tanda.__delattr__(name)
            self.set_modified()
    ...
    def action_tanda_delete_cb(self, action, parameter):
        ...
        tanda = self.tanda_store.get_row(i)
    ...
    def get_filenames(self, selection=True):
        if selection:
            ...
            return sum(([song['file'] for song in store.get_row(store.get_iter(path))._songs] + [songlist.SongListWithSeparator.SEPARATOR_FILE] for path in paths), [songlist.SongListWithSeparator.SEPARATOR_FILE])
    ...
    def action_save_cb(self, action, parameter):
        ...
        store, paths = self.tanda_treeview.get_selection().get_selected_rows()
        for path in paths:
            tanda = store.get_row(store.get_iter(path))
    ...
    def action_tanda_field_cb(self, action, parameter):
        ...
        tanda = self.tanda_store.get_row(self.tanda_store.get_iter(path))
        ...
        if name in alt_tanda:
            tanda.get_data()[name] = alt_tanda[name]
        elif reset:
            try:
                del tanda.get_data()[name]
            except:
                pass
        if not reset:
            self.set_modified()
    ...
    def action_copy_delete_cb(self, action, parameter):
        ...
        if focus == self.tanda_treeview and action.get_name() == 'copy':
            ...
            if column:
                name = column.field.name
                Gtk.Clipboard.get(Gdk.SELECTION_CLIPBOARD).set_text(repr({name: self.tanda_store.get_row(self.tanda_store.get_iter(path)).get_data()[name]}), -1)
    ...
    def clipboard_paste_cb(self, clipboard, raw, before):
        ...
        if focus == self.tanda_treeview:
            ...
            if not isinstance(to_paste, dict):
                return
            for path in paths:
                tanda = self.tanda_store.get_row(self.tanda_store.get_iter(path)).get_data()
    ...
    def action_tanda_merge_cb(self, *args):  # XXXXXXXXXXXXXXXXXX
        store, paths = self.tanda_treeview.get_selection().get_selected_rows()
        tandas = [store.get_row(store.get_iter(path)) for path in paths]
        for target in tandas:
            if all(tanda == target or tanda.Problems == 'NOT FOUND' for tanda in tandas):
                break
        else:
            return
        target._songs = sum((tanda._songs for tanda in tandas), [])
        for tanda in tandas:
            tanda._modified = True
            if tanda != target:
                tanda._songs = []
            if tanda == self.current_tanda:
                self.set_songs(tanda._songs)
        self.tanda_treeview.queue_draw()
        self.set_current_tanda()
...
class TandaView(TandaSubModule, songlist.SongListWithSeparator):
    ...
    def set_tandas(self, tandas):
        songs = [self.separator_song]
        songs[0]['tandaid'] = None
        for tanda in tandas:
            tandaid = tanda['tandaid']
            for song in tanda['_songs'] + [self.separator_song]:
...
class TandaDatabase(GObject.Object):
    ...
    def __init__(self, shared):
        ...
        self.shared = shared
        self.config = shared.config.setdefault(Tanda.name, {})
        self.ampd = self.shared.ampd_client.executor.sub_executor()

        self.fields = data.FieldFamily(self.config.setdefault('fields', {}))
        self.fields.register_field(data.Field('Artist', _("Artist")))
        self.fields.register_field(data.Field('Genre', _("Genre")))
        self.fields.register_field(data.Field('_years_min', visible=False, get_value=lambda tanda: min(song.get('Date', '').split('-', 1)[0] for song in tanda['_songs']) or '????' if tanda.get('_songs') else None))
        self.fields.register_field(data.Field('_years_max', visible=False, get_value=lambda tanda: max(song.get('Date', '').split('-', 1)[0] for song in tanda['_songs']) or '????' if tanda.get('_songs') else None))
        self.fields.register_field(data.Field('Years', _("Years"), get_value=lambda tanda: ('\'{}'.format(tanda['_years_min'][2:]) if tanda['_years_min'] == tanda['_years_max'] else '\'{}-\'{}'.format(tanda['_years_min'][2:], tanda['_years_max'][2:])) if '_years_min' in tanda and '_years_max' in tanda else '????'))
        ...
        self.fields.register_field(data.Field('time', _("Duration"), get_value=lambda tanda: data.format_time(sum((int(song['Time'])) for song in tanda.get('_songs', [])))))
    ...
    def _get_tanda_from_tuple(self, t):
        ...
        self.fields.song_set_fields(tanda)
        return tanda
    ...
    async def action_tanda_verify_cb(self, action, param):
        ...
        await self.ampd.idle(self.ampd.UPDATE)
        await self.ampd.idle(self.ampd.UPDATE)
        unused = []
        updated = []
        replaced = []
...
class TandaExtension(plugin.ExtensionWithCss):
    ...
    def activate(self):
        ...
        self.provides['app']['menubar_items'] = [
            omenu.Item('20#edit/90#tanda/10', 'mod.tanda-reset-field', _("Reset tanda field"), ['<Control><Shift>r']),
            omenu.Item('20#edit/90#tanda/20', 'mod.tanda-fill-field', _("Fill tanda field"), ['<Control><Shift>f']),
            omenu.Item('20#edit/90#tanda/30', 'mod.tanda-merge', _("Merge"), ['<Control><Shift>m']),
            omenu.Item('20#edit/90#tanda/40', 'mod.tanda-delete', _("Delete tanda")),
