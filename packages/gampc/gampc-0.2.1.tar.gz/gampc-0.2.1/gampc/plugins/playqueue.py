...
class PlayQueue(songlist.SongListWithTotals):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.actions.add_action(action.Action('playqueue-shuffle', self.action_shuffle_cb, unsafe=True, shared=self.shared))
    ...
    def data_func(self, column, renderer, store, i, j):
        ...
        if self.shared.ampd_server_properties.state != 'stop' and store.get_row(i).Id == self.shared.ampd_server_properties.current_song.get('Id'):
    ...
    async def action_replace_playlist_cb(self, action, parameter):
        filenames = self.get_filenames(False)
        ...
        if not value:
            return
        await self.ampd.playlistclear(value)
        await self.ampd.command_list(self.ampd.playlistadd(value, filename) for filename in filenames)
    ...
    def new_song(self, store, i):
        ampd.Task(self.ampd.addid(store.get_row(i).file, store.get_path(i).get_indices()[0]))

    def delete_song(self, store, i):
        song_id = store.get_row(i).Id
        m = int(store.get_string_from_iter(i))
        for n in range(store.iter_n_children()):
            if n != m and store.get_row(store.iter_nth_child(None, n)).Id == song_id:
                ampd.Task(self.ampd.command_list((self.ampd.swap(n, m), self.ampd.delete(m))))
                store.remove(i)
                return
        if not (self.shared.safe_mode and self.shared.ampd_server_properties.state == 'play' and self.shared.ampd_server_properties.current_song.get('pos') == song_id):
            ampd.Task(self.ampd.deleteid(song_id))
            store.remove(i)

    @ampd.task
    async def treeview_row_activated_cb(self, treeview, p, column):
        if not self.shared.safe_mode:
            await self.ampd.playid(self.store.get_row(self.store.get_iter(p)).Id)
    ...
    def activate(self):
        ...
        self.provides['songlist']['actions'] = [
            action.ActionModel('playqueue-ext' + verb + shuffle, action_playqueue_add_replace_cb,
                               unsafe=(verb == '-replace' or shuffle), parameter_type=GLib.VariantType.new('b'), shared=self.shared)
