...
class CoreExtension(plugin.Extension):
    def activate(self):
        ...
        self.provides['app']['actions'] = [
            action.PropertyAction('safe-mode', self.shared),
            ...
            action.ActionModel('play-or-pause', self.play_or_pause_cb, unsafe=True),
            action.ActionModel('absolute-jump', self.absolute_jump_cb, unsafe=True, parameter_type=GLib.VariantType.new('i')),
            action.ActionModel('relative-jump', self.relative_jump_cb, unsafe=True, parameter_type=GLib.VariantType.new('i')),
        ] + [
            action.ActionModel(name, self.mpd_command_cb, unsafe=True) for name in (
        ...
        self.provides['app']['menubar_items'] = [
            omenu.Item('10#gampc/20#misc/50', 'app.safe-mode', _("Safe mode"), ['<Control><Alt><Shift>s']),
    ...
    async def fade_to_action_cb(self, action_, parameter):
        ...
        try:
            ...
            for i in range(N):
                ...
                reply = await self.ampd.idle(self.ampd.PLAYER, T / N)
                if self.ampd.PLAYER & reply:
                    break
            else:
