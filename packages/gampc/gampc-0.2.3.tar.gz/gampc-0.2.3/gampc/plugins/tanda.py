...
class TandaEdit(TandaSubModule, songlist.EditableSongList):
    ...
    def get_filenames(self, selection=True):
        if selection:
            ...
            return sum(([song['file'] for song in store.get_record(store.get_iter(path))._songs] + [songlist.SongListWithSeparator.SEPARATOR_FILE] for path in paths), [songlist.SongListWithSeparator.SEPARATOR_FILE])
