...
class Swinger(object):
    ...
    @staticmethod
    def best_word_features(word_list, word_features):
        ...
        best_words = find_best_words(word_features, 1500) #特征维度1500
    ...
    def create_word_features(bigram, pos_data, neg_data):
        ...
        # pickle.dump(word_features, open('word_features.pickle','wb'))
    ...
    def load(self, name):
        try:
            self.classifier = pickle.load(open(name + '.pickle', 'rb'))
            ...
            pickle.dump(self.classifier, open(name + '.pickle','wb'))
        # pred = classifier.classify_many(self.test) #对开发测试集的数据进行分类，给出预测的标签
        # return accuracy_score(self.tag_test, pred) #对比分类预测结果和人工标注的正确结果，给出分类器准确度
...
if __name__ == '__main__':
    s = Swinger(pos=sys.argv[1], neg=sys.argv[2])
    s.load('NuSVC')
    for i in enumerate(json.load(open(sys.argv[3]))['articles']):
        print(i)
        print(s.swing(jieba.lcut(i['content'])))
