...
_ccp4_encode_init(_ccp4_encode *self, PyObject *args) {
    geometry *geo;
    PyObject *par_struct = NULL, *slice = NULL, *tmp;
    int type;

    _destroy_ccp4(self->geo);
    if (!PyArg_ParseTuple(args, "OOi", &par_struct, &slice, &type))
        return -1;
    ...
    PyObject_GetBuffer(self->slice_struct, &self->slice_buf, PyBUF_C_CONTIGUOUS);
    if (self->par_buf.len != sizeof(parfile)) {
        PyErr_SetString(PyExc_TypeError, "Parfile structure cannot be interpreted");
        return -1;
    }
    if (self->slice_buf.len != sizeof(t_slice)) {
        PyErr_SetString(PyExc_TypeError, "Slice cannot be interpreted correctly");
        return -1;
    }
    self->slice = (t_slice *)self->slice_buf.buf;
    self->par = (parfile *)self->par_buf.buf;

    Py_BEGIN_ALLOW_THREADS
    geo = _init_ccp4(self->par, self->slice, type);
...
_ccp4_encode_getbuffer(PyObject *obj, Py_buffer *view, int flags) {
    ...
    if (self->geo->type == LAYER_3D) {
        ...
        view->shape = NULL;
        view->strides = NULL;
    } else if (self->geo->type == LAYER_2D) {
        ...
        view->format = "f";
        view->ndim = 2;
        view->shape = self->geo->shape;
        view->strides = self->geo->strides;
    } else {
        return -1;
    }
    Py_INCREF(self);
...
static PyMethodDef _ccp4_encode_methods[] = {
    {"add", (PyCFunction)_ccp4_encode_add, METH_VARARGS, "Add an array to the CCP4 map"},
    {NULL}  /* Sentinel */
};
