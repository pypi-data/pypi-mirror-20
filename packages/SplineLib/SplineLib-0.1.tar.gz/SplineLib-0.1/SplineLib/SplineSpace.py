...
class SplineSpace(object):
    ...
    def evaluate_basis(self, res=1000, knot_range='active'):
        """
        Evaluates the basis functions on the entire knot interval or over the
        interval with full support.
        :knot_range: 'active' for full support, 'all' for the entire knot range.
    ...
    def _evaluate_non_zero_basis_splines(self, x, mu, derivative=0, eps=1.0e-14):
        """
    ...
    def function(self, coefficients, derivative=0):
        ...
        def f(x):
            mu = self._index(x)
            non_zero_b_splines = self._evaluate_non_zero_basis_splines(x, mu, derivative=derivative)
