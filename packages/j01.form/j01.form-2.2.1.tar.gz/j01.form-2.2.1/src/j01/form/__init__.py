...
class FormMixin(LayoutHelperMixin, j01.jsonrpc.HistoryStateMixin):
    ...
    def applyWidgetIgnoreRequest(self):
        """Apply ignore request marker"""
        for name, state in self.widgetIgnoreRequests.items():
            try:
                self.widgets[name].ignoreRequest = state
                # we need to udpate the widget again, since the widget
                # get updated without the ignoreRequest marker
