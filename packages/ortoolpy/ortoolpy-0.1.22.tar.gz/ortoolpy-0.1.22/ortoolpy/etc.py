...
class CacheDict:
    ...
    def get(self, key, default=None):
        return self[key] if key in self._dct else default
    def items(self):
...
class MultiKeyDict:
    ...
    def _hier_dict(v1, iskey, conv):
        ...
        for k2, v2 in v1.items():
            if iskey and iskey(k2):
                dct[k2] = MultiKeyDict._hier_dict(v2, None, conv)
            else:
                lst.append((k2, conv(v2) if conv else v2))
        return lst, dct
    def __init__(self, dc, iskey=None, conv=None):
        """親子情報ペアを値とする辞書"""
        self._dct = {}
        for k1,v1 in dc.items():
            self._dct[k1] = MultiKeyDict._hier_dict(v1, iskey, conv)
    @staticmethod
    def _hier_get(tpl, ky):
        """
        親子情報ペアに対する子キーをkyとした情報
        tpl: 親子情報ペア
        ky: 子キー
        """
        yield from tpl[0]
        if ky in tpl[1]:
            yield from tpl[1].get(ky,[[]])[0]
    @lru_cache(maxsize=2048)
    def __getitem__(self, key):
        """
        親キー内の情報と親キー.子キーの情報の両方を取る
        key: 親キー,子キー
        """
        ky1, ky2 = key if isinstance(key, tuple) else (key,None)
        return dict(MultiKeyDict._hier_get(self._dct.get(ky1,([],{})), ky2))
    def __repr__(self):
        """表現"""
