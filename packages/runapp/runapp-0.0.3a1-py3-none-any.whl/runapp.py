...
arguments:
    ...
    ${BOLD}--help|help${ENDC}: This help
    ${BOLD}--version|version${ENDC}: Show ${UNDERLINE}runapp${ENDC} version
    ''')
...
def except_hook(exctype, value, traceback):
    if exctype == RunException:
        print('{}{}{}'.format(BColors['FAIL'], value[0] if isinstance(value, list) else value, BColors['ENDC']))
        use()
        sys.exit(1)
    else:
...
def main():
    ...
    if '--version' in sys.argv or 'version' in sys.argv:
        print('{}runapp {}{}{}'.format(BColors['OKBLUE'], BColors['OKGREEN'], pkg_resources.get_distribution('runapp').version, BColors['ENDC']))
        sys.exit(0)
