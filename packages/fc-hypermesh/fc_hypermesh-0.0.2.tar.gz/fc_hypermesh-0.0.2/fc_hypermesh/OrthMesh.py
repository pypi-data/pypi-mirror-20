...
from fc_hypermesh.EltMesh import EltMesh,LabelBaseName
...
class OrthMesh:
  def __init__(self,d,N,**kwargs):
    ...
    if mapping is not None:
      ...
      my_mapp=trans
    self.Mesh=EltMesh(d,d,my_mapp(q),me,None,label=1)
    self.set_box()
    self.Faces=[]
    for m in np.arange(d-1,m_min-1,-1):
      ...
      for j in np.arange(nsTh):
        Fh.append(EltMesh(d,m,my_mapp(sTh[j].q),sTh[j].me,sTh[j].toGlobal,label=j+1,color=colors[j]))
