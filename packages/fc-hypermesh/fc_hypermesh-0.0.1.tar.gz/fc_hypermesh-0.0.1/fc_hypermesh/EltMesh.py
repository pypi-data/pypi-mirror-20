...
class EltMesh:
  ...
  def plotmesh(self,**kwargs):
    ...
    #PlotOptions=deepcopy(kwargs)
    #if 'color' in PlotOptions.keys():
      #color=PlotOptions.get('color')
      #PlotOptions.pop('color',None)
    PlotOptions=deepcopy(kwargs)
    color=PlotOptions.pop('color',self.color)
    if self.type==0:
      return eval("PlotmeshSubTh"+str(self.m)+"simp"+str(self.d)+"D(self,color,**PlotOptions)")
    else:
      return eval("PlotmeshSubQh"+str(self.m)+"orth"+str(self.d)+"D(self,color,**PlotOptions)")
