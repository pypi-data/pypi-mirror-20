...
NAMESPACE = getattr(settings, setting_name('URL_NAMESPACE'), None) or 'Pixelpin'
...
def _do_login(backend, user, Pixelpin_user):
    ...
    if backend.setting('SESSION_EXPIRATION', False):
        ...
        # by setting. Use last Pixelpin-auth instance for current
        # provider, users can associate several accounts with
        # a same provider.
        expiration = Pixelpin_user.expiration_datetime()
        if expiration:
            try:
