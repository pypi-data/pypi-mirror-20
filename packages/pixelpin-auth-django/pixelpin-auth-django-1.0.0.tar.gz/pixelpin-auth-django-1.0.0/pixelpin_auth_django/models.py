"""Django ORM models for Pixelpin Auth"""
import six
...
from .managers import UserPixelpinAuthManager
...
class AbstractUserPixelpinAuth(models.Model, DjangoUserMixin):
    """Abstract Pixelpin Auth association model"""
    user = models.ForeignKey(USER_MODEL, related_name='pixelpin_auth',
                             on_delete=models.CASCADE)
    provider = models.CharField(max_length=32)
    uid = models.CharField(max_length=UID_LENGTH)
    extra_data = JSONField()
    objects = UserPixelpinAuthManager()
...
class UserPixelpinAuth(AbstractUserPixelpinAuth):
    ...
    class Meta:
        ...
        db_table = 'pixelpin_auth_userpixelpinauth'
...
class Association(models.Model, DjangoAssociationMixin):
    ...
    class Meta:
        app_label = "pixelpin_auth_django"
        db_table = 'pixelpin_auth_association'
        unique_together = (
            ('server_url', 'handle',)
        )
...
class Partial(models.Model, DjangoPartialMixin):
    ...
    class Meta:
        app_label = "pixelpin_auth_django"
        db_table = 'pixelpin_auth_partial'
