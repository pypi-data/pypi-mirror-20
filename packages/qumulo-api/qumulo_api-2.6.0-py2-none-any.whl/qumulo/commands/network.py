...
class MonitorNetworkCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument("--version", type=int, default=2,
            choices=range(1, 3), help="API version to use (default 2)")
...
def print_connection_counts(connlist):
    def initial_counts():
        return {'CONNECTION_TYPE_NFS': 0, 'CONNECTION_TYPE_SMB': 0}
    ...
    print 'Total: SMB {} NFS {}'.format(
        totals['CONNECTION_TYPE_SMB'], totals['CONNECTION_TYPE_NFS'])
    for node in sorted(per_node.keys()):
        print 'Node{}: SMB {} NFS {}'.format(node,
