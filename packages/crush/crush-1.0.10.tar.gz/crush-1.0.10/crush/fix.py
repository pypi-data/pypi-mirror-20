...
class SimplePolicyFamily(BasePolicyFamily):
    ...
    def sample(self, params):
        ...
        for r in range(1, self.R):
            ...
            selection.append(i)
        print('LLLLLLLLLL ' + np.array(selection))
...
#        pol = CrushPolicyFamily(cap, R)
        sim = run_sim(len(cap), pol.build_selector(cap))
        # print('Before: All replicas on each hard drive')        
        # report(sim, cap/np.sum(cap), 1/R*np.mean(sim > 0, axis=0))
        # print('initial ' + str(pol.c))
#        pol.init_crushmap(pol.p_1)
#        sim = run_sim(len(cap), lambda: pol.sample(None), n=20000)
#        print('Before: First replica on each hard drive')
#        report(sim, np.repeat(1/len(cap), len(cap)), np.mean(sim == 1, axis=0))
        print('Before: All replicas on each hard drive')        
        report(sim, cap/np.sum(cap), 1/R*np.mean(sim > 0, axis=0))

        sys.exit(0)
        opt = optimal_params(pol, constraints=constraints, verbose=1)
        pol.init_crushmap(opt)
        sim = run_sim(len(cap), lambda: pol.sample(None), n=20000)
#        print('After: First replica on each hard drive')
#        report(sim, np.repeat(1/len(cap), len(cap)), np.mean(sim == 1, axis=0))
        print('After: All replicas on each hard drive')        
        report(sim, cap/np.sum(cap), 1/R*np.mean(sim > 0, axis=0))
        print('Sample:')
        print(sim[:10, :])
        print()
